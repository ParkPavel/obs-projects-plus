...
- **The 4-gate verification, tiered (economical) verification, MCP memory protocol, anti-hallucination rules, and creative-judgment clause in `CLAUDE.md` are binding on every step below.**

## Operating posture

You run autonomously but you are not a rule-following zombie. If a subagent's plan is flawed, a ticket conflicts with the architecture, or a simpler solution exists, surface it and adjust. Every report you relay must be backed by **raw command output**, never a subagent's unverified paraphrase.
...
Never work on main/master — hooks will block commits anyway.
...
**Gate 5 — full 4-gate, evidenced by raw output:**
- 5a — `npm run build` → 0 errors.
- 5b — `npm test` → ≥ 134 suites / 2020 tests PASS.
- 5c — `npm run lint` → 0 errors.
- 5d — `npm run svelte-check` → 0 errors.
