...
1. Query the `memory` MCP graph for the component's modules/registries; read Svelte files only where memory is thin.
...
Design within Svelte-3 limits and the invariants, but propose better patterns when you see them. Flag a questionable request rather than planning it verbatim.
...
- **To `senior-designer`**: if new visual pattern with no existing token → state "Design token decision needed."
