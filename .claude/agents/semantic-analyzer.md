...
- READ-ONLY. No `Edit`, no `Write`, no `Bash` writes, no destructive operations.
- Findings only — no fixes, no code changes.
- Every claim carries a `file:line` citation — never assert a symbol/path you have not matched.
