...
## PRESET E — Formula Builder

**Название**: *"Пресет: Конструктор формул"*

```
PROMPT:

Dashboard layout wireframe for Formula Builder preset showing the visual formula editor,
white background, flat design. Aspect ratio 16:10.

Top bar: "База: Проекты" | Preset: [Обзорный пресет] [**Конструктор формул★**]
The "Конструктор формул" tab is active.

This preset has a 3-panel layout:

LEFT PANEL (30%) — "Formula Fields" sidebar:
White, 1px border, list of formula field definitions:
  [fx] revenue — "=sum(@estimate)"  [edit]
  [fx] margin — "=@revenue * 0.3"  [edit]
  [fx] status_label — "=IF(progress>0.9, 'Done', IF(progress>0.5, 'On Track', 'At Risk'))"  [edit]
  [fx] risk_score — "=CORREL(@estimate, @progress)"  [edit]
  [ + Add formula field ]

Each formula field shows:
  - Field name (bold)
  - Formula expression (monospace, truncated with ...)
  - Return type inferred: [number] or [string] or [boolean]

MIDDLE PANEL (45%) — "Formula Visual Editor" (FormulaVisualEditor.svelte):
A visual node-based editor (not code, but connected boxes):
Currently editing: "margin = @revenue * factor"

Shows 3 node boxes connected by thin lines:
  [@revenue] → [× multiply] → [OUTPUT: margin]
                ↑
            [0.30 constant]

Below the node editor: text input "=@revenue * 0.30" (the formula as text, editable)
Below: function autocomplete dropdown showing: multiply, minus, plus, divide

Toolbar above editor: [Save] [Preview] [Reset] | Field type: [Number ▾]

RIGHT PANEL (25%) — "Preview & Debug":
Header: "Preview (5 rows)"
Small table showing 5 rows:
  name | @revenue | factor | margin (computed)
  Alpha | $4,500 | 0.30 | **$1,350** (bold, amber)
  Beta | $2,200 | 0.30 | **$660**
  ...
  
Below preview table:
"Function reference" section:
  Small cards for 4 functions with hover-expanded docs:
  [sum()] [IF()] [CORREL()] [MAP()]

Bottom of right panel:
"115+ functions available" with a "Browse all →" link

Bottom strip:
"FormulaVisualEditor.svelte · FormulaBar.svelte · lib/formula/index.ts"
"Formula coverage: 140% of Notion parity 🟢"

Color: violet for formula nodes, amber for computed output values, green for "healthy" indicators.
```
