...
- Status: ✅ DONE — `R0.16` реализован, ветка `fix/167-rem-in-container-ratchet` от `09fef14`,
  коммиты `eef8c92` test / `49026c4` fix (правки по аудиту + docs). Ждёт мёржа.
...
- **Что сделано.** Абсолютным инвариантом это сегодня быть не может — сам тикет мерил порядки
  ...
  - **Что нашёл аудит Codex и что с этим сделано (CRITICAL, подтверждено чтением кода; отчёт — `codex-reports/CX-AUDIT-167.md`).** Первая
    ...
    **перемерен** (806 → 807), а не увеличен. Граница названа явно: у `"{STRIP_HEIGHT_REM}rem"`
    цифр в тексте нет, такую единицу счётчик увидеть не может — но читает её теперь именно
    читатель, так что ограничение принадлежит регулярке, а не охвату. Дополнительно проверено и
    закреплено тестом: под областью нет ни одного `.css` (единственный в дереве —
    `ui/tokens/tokens.css`, это уровень 1 на `:root`, привязка к корню там намеренная) и ни одного
    `rem` в `.ts` Dashboard.
