...
- Status: 🔄 ПОЧТИ — хранилище и разрешение сделаны, источники стали адресуемыми; **создание
  выборки из живого фильтра вынесено в #184** (см. ниже, почему именно там) | Milestone: M-MATRYOSHKA | Priority: **P1** | Complexity: M (решение)
...
- Status: 📋 TODO — вынесен из #170 2026-09-03 | Milestone: M-MATRYOSHKA | Priority: P2 | Complexity: M
- analysis_required: false — решение принято в #170, здесь только поверхность
...
- **Что уже готово к этому и не требует работы:** `sourceLabel` даёт имя источнику без имени,
  `sourceExists` отличает удалённый источник от пустого результата, `resolveDerived` уже различает
  `ok / empty / broken / pending` — то есть UI есть чем рисовать «сломана» отдельно от «ничего не
  нашла». Тесты на всё это лежат в `datasources/__tests__/`.
...
- **Работа:** действие в полосе фильтра вида, диалог имени, запись `DerivedDataSource` в
  `additionalSources` проекта, отображение состояния `broken` там, где выборка выбрана источником.
