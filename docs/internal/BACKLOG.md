...
- **T2 (`#180b`) — проценты: число как данность, «NN%» как запись; пустая совокупность даёт прочерк.**
  §3.2 пункты 2 и 3 закрыты. `value` у `percent_empty` / `percent_not_empty` / `percent_true` теперь
...
- **Найдено при доделке T2 трассировкой потребителей ядра, а не гейтом:** поле-роллап с процентом
  складывает результат в значения записи (`ui/app/rollupColumns.ts`), где обобщённый рендерер ячейки
  не знает про единицы — голое число превратило бы ячейку «57%» в «57». Текст сохранён для этого
  семейства, и только когда значение есть: пустая совокупность остаётся `null` и рисует пустую
  ячейку вместо прежнего «0%». Список `PERCENT_FUNCTIONS` объявлен рядом с кодом, который их
  форматирует, — это вопрос к ядру, а не второй список. `CrossProjectRollupResult` теперь несёт
  `formattedValue`. Закреплено двумя тестами в `rollupColumns.test.ts`.
...
- **Не тронуто T2:** `PCT_EMPTY`/`PCT_NOT_EMPTY` в `transformExecutor.ts` (словарь конвейера, это
  адаптер T3) и предикат «пустоты» ядра, где `false` считается пустым, тогда как решение
  пользователя D4 говорит обратное — унификация счётного семейства это T4, и менять предикат под
  одними процентами значило бы поссорить ядро с самим собой.
