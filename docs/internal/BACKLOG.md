...
- Status: ⚠ КОД ГОТОВ, живая проверка не сделана (2026-08-30) | Milestone: (next) | Priority: P2 | Complexity: S
...
- **Чего НЕ сделано:** демо не пересоздавалось в OBStests, поэтому утверждение «первое открытие
  не пишет на диск» держится на тестах, а не на живом прогоне. Закрывать тикет до этого нельзя —
  ровно на таком выводе он и был ложно закрыт в прошлый раз.
