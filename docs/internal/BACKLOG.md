...
- Status: 📋 ПЛАН ГОТОВ, ЖДЁТ РЕШЕНИЯ ПОЛЬЗОВАТЕЛЯ (2026-09-02) | Milestone: (next) | Priority: P2 | Complexity: M (план: S в исполнении)
...
- **ВОПРОС ПОЛЬЗОВАТЕЛЮ (без ответа не выполняется):** удалить три файла, сохранив их текст как датированный
  архивный документ — да/нет? Рекомендуемый ответ: **да**. Что сделало бы план неверным: если Unified
  DataEngine всё ещё намеченное направление, а не оставленное.
...
- Status: 📋 ПЛАН ГОТОВ, ЖДЁТ РЕШЕНИЯ ПОЛЬЗОВАТЕЛЯ (2026-09-02) | Milestone: (next) | Priority: P2 | Complexity: M
- **План архитектора: `PLAN_180_AGGREGATION_ADAPTER_2026-09-02.md`.** Рекомендация — вариант 1, адаптер:
...
- **ВОПРОСЫ ПОЛЬЗОВАТЕЛЮ (без ответов не выполняется), рекомендуемые ответы в скобках:**
  1. `AVG` от `["", "10"]` сегодня `5`, у ядра `10` — что поставляем? (**ядро: пустая ячейка — не ноль**)
  2. `SUM` от `["12abc"]` сегодня `0`, у ядра `12` — что поставляем? (**ядро, ради одного правила коэрции**)
  3. `MIN`/`MAX`/`RANGE` от пустой группы — сегодня `null`, ядро даёт `0`. Оставить `null`? (**да**)
  4. `COUNT` считает null-ы (≡ `count_total`). Оставить? (**да** — `chartDataPipeline.ts:29` на этом стоит**)
...
- Status: 🟡 ШАГ 1 СДЕЛАН, НЕ СЛИТ (2026-09-02, ветка `feat/166-step1-container-roots`, коммиты
  `08ede44` feat / `dbe51c0` test / `a31423f` chore, база `main` = `f425812`); шаги 2–3 ждут
  решений пользователя | Milestone: M-MATRYOSHKA | Priority: **P1** | Complexity: L
- **Шаг 1 — что сделано и чем это доказано.** `.ppp-database-root` и `.ppp-database-canvas`
  ...
  svelte-check 0/0.
- **🔴 БЛОКЕР МЁРЖА, найден грепом риска 4 (не исправлен, решение за пользователем):**
  `TemplateConfirmDialog.svelte:51` — оверлей `position: fixed; inset: 0`, отрисованный **внутри**
  `.ppp-database-root` (`DashboardCanvas.svelte:163`) и **не** портированный в `<body>`. Сегодня
  его containing block — `ViewContent` (у неё `container-type` с до-#166 времён), то есть видимая
  область; после шага 1 им становится `.ppp-database-root`, высота которого — весь прокручиваемый
  дашборд. На дашборде выше экрана диалог центрируется в контенте, а не на экране, и может уехать
  под фолд. Варианты: портировать диалог как `FloatingPopup`, вынести его из `.ppp-database-root`,
  либо заменить `align-items: center` на отступ сверху. Это единственное, что в шаге 1 двигается.
...
- **ВОПРОСЫ ПОЛЬЗОВАТЕЛЮ (шаги 2–3 без ответов не выполняются), рекомендуемые ответы в скобках:**
  1. Сохранённые ширины колонок остаются буквальными, пропорциональные колонки — отдельный тикет? (**да**)
  2. После шага 2 чарты на существующих дашбордах изменят пропорции и плотность подписей — принять как
     исправление без настройки? (**да**: нынешнее поведение — безграничное масштабирование, это и есть дефект)
  3. Удалить не запрошенный никем `db-table` (`DataTableContent.svelte:255-256`), а не искать ему потребителя? (**да**)
