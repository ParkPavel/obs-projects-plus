...
- **Не тронуто T2, и это отложено осознанно — но противоречие видно уже сегодня, так что записано
  как долг, а не как «мелочь»:**
  - `PCT_EMPTY`/`PCT_NOT_EMPTY` в `transformExecutor.ts:867` отвечают `0` на пустую совокупность.
    Это словарь конвейера, предмет адаптера **T3**; менять его до адаптера — правка тех же строк дважды.
  - Предикат «пустоты» ядра (`aggregate.ts:115,118,124,130`) считает `false` пустым, тогда как
    решение пользователя D4 говорит, что `false` — значение. **Проверено:** предикат общий у
    `count_values`, `count_empty`, `percent_empty`, `percent_not_empty`, поэтому правка под одними
    процентами поссорила бы ядро с самим собой. Видимое следствие сегодня: над `[false]` ядро даёт
    100% пустых, подвал — 0%. Унификация счётного семейства это **T4**.
  - `median` пустого списка возвращает `0` (`aggregate.ts:160`), где §3.1 требует `null`. **T3 или
    отдельный тикет** — меняет ответ поставляемого оператора.
