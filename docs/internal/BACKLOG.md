...
- Status: 🔄 IN-PROGRESS — шаг 1 СЛИТ в `main` (2026-09-02, merge `010a59d`; коммиты `08ede44` feat /
...
- **ВОПРОСЫ ПОЛЬЗОВАТЕЛЮ (шаги b/c; шаг (a) их не ждёт), рекомендации в скобках:**
  1. Поверхность пика — внутренняя выезжающая панель или лист сайдбара? (**панель**)
  2. Режим обычного клика после (b) — одинаковый для всех видов или свой у каждого? (**одинаковый**)
  3. Пик редактируемый или только для чтения на первом шаге? (**редактируемый** — переиспользует
     `EditNote`; «только чтение» — это новый код, чтобы отнять возможность)
  4. Заводить настройку `defaultRecordOpenMode`? (**нет** — ключ навсегда, пять файлов, а
     модификаторы оставляют старое поведение доступным)
  5. Выпускать (b) до focus-trap из #169 или блокироваться на нём? (**блокироваться** — панель
     объявляет себя модальной и не берёт фокус, для клавиатуры это регресс)
