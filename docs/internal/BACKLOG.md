...
- **ВОПРОСЫ ПОЛЬЗОВАТЕЛЮ до T1 (рекомендации в скобках):** (1) `sum` пустой группы — `0` или «—»? (**0**);
  (2) `"1,5"` — число или текст? (**текст**); (3) `"1e3"` → 1000? (**да**); (4) `false` — значение для
  «Count values»? (**да**); (5) «Count numbers» только для Number/текстовых полей? (**да**).
