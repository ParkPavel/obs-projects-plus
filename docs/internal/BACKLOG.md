...
- Status: ✅ DONE 2026-09-03 — закрыт через пик #168(c); `invalid-field` был закрыт 2026-08-28
- **Закрыто 2026-09-03 шагом (c) тикета #168.** Записанная здесь причина «нельзя» была верна для
  ячейки таблицы и перестала быть верной для пика: он показывает обогащённую запись, где спутник
  `__resolved__<field>` уже лежит рядом с сырыми ссылками. Неразрешённые ссылки называются поимённо,
  разрешённые молчат.
...
- Status: ✅ DONE 2026-09-03 — решено и закрыто; **бандлы остаются в дереве, но перестают сливаться**
- analysis_required: true — **закрыто разбором в основной сессии 2026-09-03**
...
- **РЕШЕНО 2026-09-03. Ничего не снимается с учёта, и это не уклонение от вопроса.** Корневой
  `main.js` — это то, чем пользователь ставит плагин прямо из репозитория, а `releases/*/main.js` —
  та же возможность для старых версий. Строка `.gitignore`, которая исключает бандл и тут же
  возвращает релизные копии, **намеренная, а не недосмотр**; ни один из двух workflow закоммиченный
  файл не читает — оба собирают свой.
- **Чинится то, что действительно болело.** Из перечисленных издержек настоящая ежедневная — одна:
  неразрешимый конфликт в минифицированном файле между любыми двумя собиравшимися ветками. Git
  пытался слить построчно 2,8 МБ сгенерированного вывода, и любой такой слив бессмыслен: файл не
  написан, а произведён. `.gitattributes` с `-merge` это прекращает — конфликтующий бандл
  сообщается и остаётся нетронутым вместо склеенного мусора, а разрешение всегда одно:
  `npm run build`. `-diff` заодно перестаёт печатать мегабайты минифицированного кода в `git log -p`.
- **Вес клона не трогается, и это названо, а не умолчано.** 13 МБ в `releases/` и 61 МБ в `.git`
  остаются: удаление из рабочего дерева `.git` не уменьшит, а возможность поставить старую версию
  из репозитория уберёт. Это изменение того, что репозиторий предлагает наружу, и оно не окупается
  ничем, кроме размера чекаута.
- **R0.20** закрепляет: `.gitattributes` — это файл, а файлы удаляют по невнимательности, и тогда
  боль вернулась бы молча. Ратчет проверяет, что каждый отслеживаемый бандл исключён из слияния,
  что «противоречие» в `.gitignore` всё ещё читается как задумано, и что оба workflow собирают сами.
...
- Status: ✅ **DONE 2026-09-02** (ветка `fix/178-delete-engine-contracts` от `main` = `6c6a82f`;
  `2fece5f` — удаление + архив, следом docs-коммит в той же ветке; **не слито, не запушено**)
  | Milestone: (next) | Priority: P2 | Complexity: M (план: S в исполнении)
- **Сделано.** Удалены все три файла (`lib/engine/contracts.ts`, `lib/relations/contracts.ts`,
  `lib/colors/contracts.ts`, 515 строк). Тикет называл один файл — удалено три, и это не расширение
  объёма, а вынужденность: два других не имеют импортёров вовсе и при этом являются единственными
  импортёрами первого, так что множество удаляется только целиком. Текст сохранён дословно в
  `docs/internal/archive/ENGINE_CONTRACTS_V4_DESIGN.md` — под исходными путями и рядом с причинами,
  по которым движок не построили. Заодно зафиксировано, что оба документа, на которые ссылается
  шапка удалённого файла (`docs/ARCHITECTURE_V4.md`, `docs/PHASE_3_TICKETS.md`), **не существуют** в
  репозитории и не существовали до этой правки.
- **Условие пользователя выполнено до удаления.** Карта использований по вызовам в коде, а не только
  по экспортам: `docs/internal/codex-reports/CX-MAP-178.md` (Codex `code-mapper`, read-only, база
  `6c6a82f`) — ноль динамических импортов, `require`, `jest.mock`, реэкспортов, `typeof`/`keyof` и
  declaration merging; ни одного `const/let/var/function/class/enum` ни в одном из трёх файлов.
  Обе строки-импортёра (`colors/contracts.ts:17`, `relations/contracts.ts:18`) перепроверены
  вручную перед удалением, а не приняты на веру.
- **Проверено:** `npx tsc -noEmit -skipLibCheck` = 0 ошибок — это и есть доказательство, что типы
  никто не потреблял. Четыре гейта зелёные: build 0, jest **182 suites / 2526 tests** (базовая линия
  держится ровно — ни один тестовый файл не добавлен и не удалён), lint 0 ошибок, svelte-check 0/0.
  `main.js` и `styles.css` побайтово не изменились (`git diff --stat` пуст; статус `M` — это только
  CRLF, в коммит не попало). Предупреждений lint стало 109 вместо 112: у каждого из трёх удалённых
  файлов был тег `@since 4.0`, который tsdoc не знает.
- **Правки текста (логика не менялась):** шапки `dashboard-engine/transformTypes.ts` и
  `R0_14_duplicateExportedTypeNames.test.ts` переписаны в прошедшее время по образцу «HISTORICAL» из
  R0.4 — ратчет пережил свой предмет и теперь стережёт СЛЕДУЮЩУЮ коллизию;
  `docs/ARCHITECTURE_V5.md` §3.6 записывает, что R5-005 сдан как `stores/palettes.ts`
  (`Favorite { color, name }`) и контракт не ожил, плюс убраны две строки таблиц, одна из которых
  (`contracts.ts | unit tests`) была неверна и до этого тикета — файла `contracts.test.ts` не
  существует; `RELATION_CONTRACT_ANALYSIS_111.md:21` фиксирует, какой из двух вариантов выбран.
- **Остаток, названный вслух:** `git grep "engine/contracts" -- src/` возвращает две строки —
  синтетические строковые литералы в `R0_14_…test.ts:140,154`, которыми тест реконструирует
  до-#179 дерево. Это значения ассертов, а не ссылки на файл; править их значило бы менять логику
  ратчета ради красоты grep. Грепом по рёбрам импорта (`from`/`import`/`require`/`jest.mock`)
  не находится ничего.
- **План архитектора: `PLAN_178_ENGINE_CONTRACTS_2026-09-02.md`.** Рекомендация — вариант B с архивной
  половиной C: удалить `lib/engine/contracts.ts` и оба его единственных импортёра
  (`lib/relations/contracts.ts`, `lib/colors/contracts.ts` — сами полностью мёртвые, 515 строк, ноль
  живых потребителей, ноль хранимых данных), сохранив текст в `docs/internal/archive/`. Поправка к
  посылке тикета: `ProjectId` определён в `settings/base/settings.ts:6`, `contracts.ts` его лишь
  реэкспортирует, и ни один из ~21 потребителей не импортирует его отсюда; у `RecordId` живых
  потребителей ноль. Вариант A (достроить) — это второе представление фильтра, инвариант 2.
- **RESOLVED 2026-09-02 (пользователь): удалить**, с условием — перед удалением проверить утечки использования
  не только по экспортам (выходные данные), но и по вызовам в коде: динамические импорты, строковые ссылки
  на пути и имена, `typeof`/`keyof` по типам, тесты, моки, документы, конфиги сборки. Порядок:
  `code-mapper` (Codex, read-only) строит полную карту использований → `lead` удаляет → `auditor`.
  Текст файлов сохраняется в `docs/internal/archive/ENGINE_CONTRACTS_V4_DESIGN.md`.
...
- Status: ✅ DONE 2026-09-02 (ветка `fix/179-transformstep-name`, `c861cd8`; слито в `main` 2026-09-02, merge `5a99f8e`)
...
- **Сделано:** переименован **мёртвый** тип — `contracts.ts` теперь экспортирует `TransformStepIR`
  (суффикс не украшение: соседи по файлу уже `FilterIR`, `SortIR`, `GroupIR`, `ComputeIR`,
  `AggregateIR`). Живой `TransformStep` в `transformTypes.ts` не тронут намеренно: это формат
  хранения, его имя — часть контракта с уже созданными пользователем дашбордами, а не локальный
  выбор. Внутри `contracts.ts` обновлена единственная ссылка (`DataEngineRequest.steps`);
  импортов этого типа вне файла не было — проверено 2026-09-02, наружу из `contracts.ts` уходят
  только `RecordId` и `ProjectId`.
- **Шапки обоих файлов** переписаны под состояние ПОСЛЕ переименования. Предупреждение
  «UNCONSUMED» в `contracts.ts` сохранено: это предмет #178, а не #179.
- **Ратчет:** `src/__tests__/R0_14_duplicateExportedTypeNames.test.ts` (5 тестов). Скан имён —
  чистая функция `(text) → names`, доказанная на синтетике в обоих состояниях, как R0.13; сверх
  того подсаженный дубликат в реальном дереве (`src/lib/engine/__r014_planted_fixture.ts`) уронил
  оба «древесных» теста и был удалён. Проверка деклараций, а не реэкспортов; слияние деклараций
  внутри одного файла законно и не считается коллизией.
- **#178 не предрешён.** Файл не удалён, не перенесён и не «достроен» — переименован один тип.
- **Найдена вторая коллизия того же класса и НЕ исправлена:** `ValidationError` объявлен дважды —
  `src/lib/helpers/formulaParser.ts:794` (`{ message, position? }`, реэкспортируется через
  `lib/formula/index.ts`) и `src/lib/types/validation.ts:15` (`{ field, message, code? }`,
  импортёров не найдено). Это другой владелец и другой модуль; втягивать его в P1/S-переименование
  значило бы превратить маленький тикет в нерецензируемый. R0.14 **декларирует** её списком
  `KNOWN_COLLISIONS_OUTSIDE_ENGINES`, поэтому третья коллизия уронит сьют, а починка этой
  потребует осознанной правки списка. Отдельный тикет не заводился — если возьмётесь, начните
  отсюда: похоже, что `types/validation.ts` мёртв целиком.

### #180 — Третий словарь агрегаций: `computeAggFn` не зовёт ядро
- Status: ✅ DONE 2026-09-03 — **T1–T5 слиты**; тикет закрыт
  | Milestone: (next) | Priority: P2 | Complexity: **L** (было M)
- **T2 (`#180b`) — проценты: число как данность, «NN%» как запись; пустая совокупность даёт прочерк.**
  §3.2 пункты 2 и 3 закрыты **в ядре и в подвале** — и только там. Формулировка сужена по итогам
  adversarial-ревью, которое верно указало: пока `PCT_EMPTY`/`PCT_NOT_EMPTY` конвейера отвечают `0`
  на пустую совокупность, заявлять пункт закрытым целиком нельзя. Конвейер — предмет адаптера T3. `value` у `percent_empty` / `percent_not_empty` / `percent_true` теперь
  число 0-100, `formattedValue` — прежний текст, побайтно тот же. Над пустой совокупностью все они
  дают `null` и «—»: ноль процентов это утверждение о совокупности, а её нет, и в ячейке подвала
  «0%» неотличимо от настоящего нуля процентов — единственного прочтения, по которому пользователь
  стал бы действовать. Подвал (`aggregation.ts`) выровнен по тому же правилу: четыре его ветки
  процентов отвечали `0`/«0%», тогда как соседняя функция в том же файле уже отвечала `null` —
  снова две реализации в одном файле, та же форма дефекта, что нашло adversarial-ревью T1.
- **Ошибка, которую сделала основная сессия и поймало adversarial-ревью — запись поучительна.**
  Поле-роллап складывает результат в значения записи (`ui/app/rollupColumns.ts`), а рендерер ячейки
  не знает про единицы, поэтому голое число превратило бы «57%» в «57». Первая попытка сохранила там
  форматированный текст. Ревью показало, что это худший размен: **кадр фильтруется и сортируется
  ПОСЛЕ этой свёртки** (`View.svelte`), с диспетчеризацией по типу значения во время выполнения —
  строка не попадает в числовую ветку, поэтому `процент > 50` не работает вовсе, а сортировка
  сравнивает «33%» как текст, где 33.1 и 33.4 неразличимы. И строка там лежала **и до #180b**, то
  есть фильтр по процентному роллапу был сломан уже тогда.
  **Итог: в значения идёт число.** Потерянный знак процента — пробел слоя отображения, записанный в
  T4, а не оплаченный строкой в данных; это ровно та подмена данных представлением, ради устранения
  которой существует тикет. Спекулятивный API (`PERCENT_FUNCTIONS`, `formattedValue` в
  `CrossProjectRollupResult`) добавлялся только ради откаченной правки и удалён вместе с ней.
  Закреплено двумя тестами в `rollupColumns.test.ts`.
- **Известный пробел, записан, а не починен мимоходом:** `median` пустого списка всё ещё возвращает
  `0`, хотя §3.1 ставит её в колонку `null` рядом с `avg`/`min`/`max`/`range`. T1 перевёл четыре и
  до медианы не дошёл, T2 ограничен семейством процентов. Это меняет ответ поставляемого оператора
  и заслуживает своего ревью — **T3 или отдельный тикет**.
- **T3-долги закрыты в `#180c` (2026-09-03), все три разом — иначе они остались бы дырами:**
  - `PCT_EMPTY`/`PCT_NOT_EMPTY` конвейера отвечали `0` на пустую совокупность → теперь `null`.
    Это же делают рядом стоящие `MIN`/`MAX`, так что конвейер теперь согласен и сам с собой.
    Остальные его пустые ответы (`AVG`, `STD_DEV` всё ещё `0`) — предмет адаптера T3.
  - `median` пустого списка возвращала `0` → теперь `null` и «—», как остальные числовые.
  - Предикат «пустоты» ядра считал `false` пустым вопреки D4 → один общий `isFilled` на четыре
    оператора (`count_values`, `count_empty`, `percent_empty`, `percent_not_empty`), и `false`
    заполняет ячейку. Видимое расхождение снято: над `[false]` ядро давало 100% пустых, подвал — 0%.
    `count_checked` / `percent_true` отвечают на другой вопрос и не затронуты.
  Три утверждения перевёрнуты с записанной причиной, добавлен блок «ядро и подвал согласны».
- **Остаётся у T3/T4 (не дыра, а объём):** адаптер `computeAggFn` → ядро, унификация трёх словарей
  и семейство счётчиков с интерфейсом; плюс слой отображения не знает единиц, поэтому процентный
  роллап рисует «57» вместо «57%» (см. запись выше про `rollupColumns.ts`).
- **T5 (`#180e`) — R0.18 не даёт словарям разойтись снова.** Три правила, по одному на каждый
  способ, которым это уже случилось: математика живёт в одном месте (ветка `case "SUM"` вне ядра —
  это то, как появился `computeAggFn`); хранимый словарь только пополняется (удалить член — осиротить
  все конфиги, которые его записали, и `count` тут поучителен: переименован, оставлен в союзе, не
  обработан в switch, молча рисовался ничем); пикер читает общую таблицу (шесть поверхностей держали
  шесть списков и расходились в том, как называется `count_total`).
  Проверено, что падает: подсаженная четвёртая реализация в `rollup.ts` и удалённый член `count`
  дают по красному тесту. Заодно порядок опций роллап-пикера переехал в общий модуль — иначе два
  диалога продолжали бы держать списки имён, а именно так шесть списков и завелись.
- **T4 (`#180d`) — семейство счётчиков и интерфейс, то есть ваш четвёртый пункт.**
  - **Сущности разведены и различимы.** Добавлены `count_numeric` (значения, с которыми можно
    считать) и `count_empty` (пустые). Над одной колонкой из шести записей пять счётчиков дают пять
    разных чисел: 6 / 4 / 3 / 4 / 2 — это и есть проверка, что они отвечают на разные вопросы.
  - **Живой дефект из §4.3 закрыт.** `count` — устаревшее написание, переименованное в R5-004; союз
    типа его сохранил, а в `switch` ветки не было, поэтому сохранённое значение падало в `default:`
    и рисовалось прочерком **без предупреждения**. Одна строка, три значения, одно из них — тишина.
  - **Ни одна опция не предлагается без последствия.** Одна таблица `aggregationOptions.ts` даёт имя
    и строку последствия каждой; голого «Count» больше нет нигде — его нет ни в одном референсе,
    и выбравший его не может знать, входит ли пустая ячейка в ответ. Последствие показывается в
    `title` опции, то есть выбор проверяем до того, как сделан.
  - **Нельзя ошибочно выбрать то, чего не показывают.** `aggregationOptionsFor(field)` — привычка,
    которая была только в меню заголовка таблицы, теперь общая: текстовому полю не предлагают `sum`,
    числовому — `count_checked`, дате — арифметику.
  - **Шесть списков стали одним.** Меню заголовка делегирует, Stats-карточка берёт список и подписи
    оттуда же, значок виджета читает ту же таблицу — он печатал `COUNT` и для `count_total`, и для
    `count_values`, возвращая ровно ту двусмысленность, которую пикер только что снял.
  - Приёмка: `A180d_countFamily.acceptance.test.ts`, 14 тестов.
- **T3 (`#180c3`) — конвейер зовёт ядро; третьего словаря больше нет.** `computeAggFn` стал
  адаптером: десять операторов уходят в ядро по замороженной карте имён, а внутри остаются только
  те, которых у ядра нет — `FIRST`/`LAST` (позиционные) и `STD_DEV`. Хранимые ключи не тронуты:
  переименование `COUNT` потребовало бы миграции по `widget.transform`, то есть по единственной
  структуре, которая молча проглатывает битый шаг.
- **Изменение сделано обозримым, а не заявленным.** `__tests__/aggregationParity.test.ts` записал
  снимком ответы конвейера на восьми краевых наборах ДО делегирования, поэтому расхождения — это
  диф, а не утверждение. Сдвинулись ровно три ответа и ровно те, которых требовала §3.2: `AVG`,
  `MEDIAN` и `STD_DEV` над набором без чисел дают `null`, а не `0`. Остальные 88 значений совпали
  побайтно — включая `PCT_EMPTY`, где пришлось нормализовать пустой массив в `null` на границе:
  для конвейера это пустая ячейка многозначного поля, для ядра — значение.
- **Удалён `extractNumericValues`** (18 строк): после делегирования его звать некому.
  Наблюдаемый результат тикета выполнен буквально — `grep engine/aggregate transformExecutor.ts`
  даёт импорт, `grep extractNumericValues` даёт ноль.
- **T1: два ревью Codex, две верные находки, обе закрыты до слияния** (`codex-reports/CX-AUDIT-180a.md`,
  `CX-ADV-180a.md`; вердикты BLOCKED и BLOCK).
  - **Adversarial P1 — T1 сам делает достижимой политику пустого ввода, которую откладывали в T2.**
    Мусор в числовом поле раньше становился `NaN` и доживал до свёртки, поэтому среднее «ни из чего»
    печатало `NaN`. После T1 значение отбрасывается, список пуст по-настоящему, и `0` из ядра печатал
    бы число, похожее на ответ — в ячейке подвала неотличимое от настоящего нуля. В `aggregation.ts`
    нашлись **две реализации**: `computeAggregateValue` подстраховку для `avg` имела,
    `computeAggregations` (её зовёт подвал, `DataTableContent.svelte:155`) — нет.
    **Сделано:** `avg`/`min`/`max`/`range` возвращают `null` и «—» из самого ядра, обе ветки согласны;
    `sum` остаётся `0` (тождество сложения, RESOLVED 2026-09-02). Шесть утверждений, закреплявших
    старую политику, перевёрнуты с записанной причиной. **Проценты по-прежнему возвращают строку —
    эта половина T2 (`#180b`) не тронута.**
  - **Audit P1 — дефект в объявленной слепой зоне R0.15, через день после того, как её записали как
    терпимую.** `CreateField.svelte` показывал значение по умолчанию числового поля через `parseInt`:
    `1.5` отображалось как `1`, а записывалось `1.5`. Обоснование «эти обработчики читают элементы
    формы, а не данные записи» здесь не работало — это были данные записи.
    **Сделано:** ратчет читает разметку; маркер внутри выражения шаблона пишется блочным комментарием
    (HTML-комментарий между атрибутами тега невозможен), блоки `<style>` исключены как CSS (иначе
    `calc(var(--x) * 1)` и класс `.heat-0` читаются как арифметика — три ложных срабатывания в дереве),
    четыре обработчика настроек виджетов размечены. Сам маркер ужесточён: он принимал закрывающую
    последовательность комментария за причину.
  - **Найдено основной сессией при чтении диффа, не ревью:** сложение с отсутствующим операндом давало
    строку (`a + 1` → `"1"`), тогда как вычитание давало число (`a - 1` → `-1`). Плюс разошёлся с
    остальной арифметикой. Excel и Airtable читают пустую ячейку в арифметике как ноль, а правило
    «пустое — не ноль» отвечает на другой вопрос: что считает знаменатель среднего. Исправлено,
    четыре теста.
  - **Дымовой прогон в OBStests** (`UNTESTABLE_FEATURES_2026-09-02.md`): с записями `mrr: 12abc` и
    `mrr: ""` плагин грузится и дашборд открывается. Что показывает агрегат, REST не видит.
- **План архитектора: `PLAN_180_AGGREGATION_ADAPTER_2026-09-02.md`** (остаётся как анализ расхождений). Рекомендация — вариант 1, адаптер:
  `computeAggFn` делегирует ядру `engine/aggregate.ts` операторы, которые у ядра есть (`SUM`, `AVG`,
  `MEDIAN`, `MIN`, `MAX`, `RANGE`, `COUNT`→`count_total`, `COUNT_DISTINCT`→`count_unique`), оставляет
  inline `FIRST`/`LAST`/`STD_DEV`/`PCT_*` (у ядра нет эквивалента или другой тип результата), хранимые
  ключи не трогает — миграции нет. Дифференциальный тест пишется ДО изменения. Это изменение
  поведения (коэрция `Number` против `parseFloat`), поэтому нужен adversarial-ревью.
- **RESOLVED 2026-09-02 (пользователь), по четырём вопросам:**
  1. Пустые ячейки в `AVG` — привести к **универсальному** правилу, если реальный сценарий использования
     действительно подразумевает такую возможность: сценарии проверить, а не предположить.
  2. `"12abc"` → 12 **не принимается**. Требуется точное отслеживание всей секции числовой коэрции: по
     документации такой ошибки образовываться не должно было. Одно строгое правило (строка целиком —
     число, иначе не число), задокументированное и закреплённое тестом; префиксный `parseFloat` — дефект.
  3. Пустые группы и прочие краевые случаи — **рассмотреть сценарии подробнее и привести всю математику
     проекта к уровню Excel-таблиц**: надёжно и разнообразно в использовании. Это спецификация по
     референсам (Excel, Google Sheets, Notion, Airtable), а не выбор между двумя сегодняшними значениями.
  4. `COUNT`: числовое значение существует для своей сущности, и для каждой сущности у пользователя должны
     быть **очевидные способы переключения** (все / непустые / уникальные / числовые) и **интерфейсные
     решения, предотвращающие логические ошибки**. Молчаливая семантика одного `COUNT` не принимается.
  **Следствие:** тикет становится спецификацией `SPEC_MATH_SPREADSHEET_2026-09-02.md` (принципы через
  обзор референсных решений) → затем реализация по её плану; адаптер из плана архитектора входит в неё
  как один из шагов, но не поставляется отдельно.
- **Спецификация написана 2026-09-02: `SPEC_MATH_SPREADSHEET_2026-09-02.md`.** Порядок работ перевёрнут:
  адаптер — последний шаг (T3), потому что делегирование сейчас перенесло бы в конвейер `parseFloat("12abc") = 12`.
  Пять тикетов: T1 одно правило коэрции `engine/numeric.ts` + таблица контракта (начиная с ingest —
  `datasources/helpers.ts:41` парсит Number-поля через `parseFloat` ещё на входе, проверено); T2 пустой
  ввод → `null`/«—», проценты возвращают число; T3 адаптер; T4 семейство счётчиков + интерфейс
  (`count_numeric`, `case "count"` для сохранённого значения, общий `aggregationOptionsFor(field)`,
  одна таблица подписей, живой предпросмотр); T5 ратчеты. Хранимые ключи не переименовываются.
  Референсы с URL: Excel/Sheets игнорируют текст, не парсят; ни один продукт не имеет голого «Count»;
  Notion и Airtable без значений-ошибок → `#DIV/0!` отвергнут в пользу «—» с причиной.
- **RESOLVED 2026-09-02 — пять открытых вопросов спецификации, решение делегировано пользователем
  («реши сам»), принято по рекомендациям спецификации:** (1) `sum` пустой группы → **`0`** (тождество
  сложения; все остальные числовые операторы → `null`/«—»); (2) `"1,5"` → **текст** (frontmatter — YAML,
  запятая делает `[1,5]` и `"1,5"` неразличимыми); (3) `"1e3"` → **1000** (грамматика с экспонентой);
  (4) `false` — **значение** для «Count values» (снятая галочка — ответ; для другого вопроса есть
  `count_checked`); (5) «Count numbers» — **только для Number и текстовых/Unknown полей**.
  T1 (`#180a`) стартует на этих ответах; они входят в таблицу контракта `NUMERIC_COERCION_CASES`.
- **T1 (`#180a`) DONE 2026-09-03**, ветка `feat/180a-numeric-coercion` (`4ef2388`, `a58cbe7`,
  `2ea043d`, `2161352`). `src/lib/engine/numeric.ts` — единственное правило; таблица контракта
  `NUMERIC_COERCION_CASES` в `numericContract.test.ts`, её импортируют потребители, а не повторяют.
  Ratchet — `R0_15_oneNumericCoercion.test.ts` (I-2), оба посева из спецификации проверены красными
  до фиксации зелёного. Подробности и дельта к инвентарю §2.3 — в спецификации, §2.3a.
  Ключевое: ingest (`datasources/helpers.ts`) больше не пишет `NaN` — не-число становится `null`;
  `extendedEvaluator.ts` расклассифицирован полностью (UNKNOWN спецификации §9 закрыт), 111 сайтов
  — коэрция аргументов, все маршрутизированы, ни одного `UNCLASSIFIED`. Одно намеренное
  расхождение задокументировано в коде (`operandNumber`): для ОПЕРАТОРА булево — 0/1.
  Найдено сверх инвентаря (он грепал только `src/lib`): операнды `is-last-n-days`/`is-next-n-days`,
  спарклайн `StatsCard`, `EditableCell`, два рендерера rollup и — самое разрушительное —
  таблица конверсии типов поля в `CreateField.svelte`, которая писала `parseInt`-результат на диск.
  Пустой ввод (`avg([])`, проценты) НЕ трогался — это T2.
...
### #182 — Мобильный bottom sheet не портируется, и минимумы внутри него переполняют виджет
- Status: ✅ DONE 2026-09-03 — но **диагноз тикета опровергнут измерением**, см. ниже | Milestone: M-MATRYOSHKA
  | Priority: P2 | Complexity: S
- `FloatingPopup` ветвится по `$isMobile`. Десктопная ветка (`FloatingPopup.svelte:285-289`) несёт
  `use:portal` → `document.body.appendChild` (`:255-256`); мобильная (`:265-283`) рендерится **на
  месте**, поэтому её `.ppp-popup-backdrop { position: fixed; inset: 0 }` позиционируется не по
  окну, а по `.ppp-widget-host` (`container-type: inline-size` — это containing block), и лист
  выходит шириной с виджет. Внутри него `.ppp-blockfilter-popover { min-width: 22rem }`
  (`BlockFilterBar.svelte:83`) на узком виджете переполняет лист.
- **ОПРОВЕРГНУТО ИЗМЕРЕНИЕМ 2026-09-03. Механизма, описанного выше, не существует.**
  `A182_mobileSheet.acceptance.test.ts` в Chrome: containing block для `position: fixed` создают
  **только** `transform` и `contain: paint`. Ни `overflow: hidden`, ни `container-type: inline-size`,
  ни `container-type: size` — не создают. Настоящий `.ppp-widget-host` с настоящим
  `.ppp-popup-backdrop` внутри: лист шириной с экран, а не с виджет, и он **не обрезается** —
  отвечает на hit-test в 700px по горизонтали, далеко за пределами виджета 300×100.
  **Так сказали три источника подряд** — `lead` при определении портала, `CX-ADV-166-step3` и
  комментарий самого #112 в коде (`FloatingPopup.svelte:250`). Ни один не измерил.
- **Следствие: предложенный портал мобильной ветки откачен, а не поставлен.** Он не чинит ничего:
  правка, оправданная несуществующим механизмом, — ровно та форма, которую здесь ловят на ревью.
  Комментарий #112 про десктопную ветку не тронут — его portal, возможно, решал другую задачу
  (сдвиг раскладки), но заявленное в нём обоснование этим измерением тоже не подтверждается.
  **Это стоит отдельного взгляда, а не тихой правки.**
- **Что было настоящим и починено:** `min-width: 22rem` внутри телефона 320px выталкивал содержимое
  за край. Теперь `min(22rem, 100%)` — `100%`, а не `100cqi`: родитель поповера это лист, а
  контейнерная единица мерила бы виджет, к которому лист отношения не имеет.
- **Это преднамеренно не чинилось в #166 шаг 3.** Дефект существовал до тикета: фикс #112 был
  только десктопным. И «очевидная» правка `min(22rem, 100cqi)` его **не закрывает** — `cqi` меряет
  content box `.ppp-widget-host`, а внутри листа доступно на его `padding: 0.5rem` меньше, то есть
  минимум всё равно окажется шире примерно на 1rem. Кандидаты: `min(22rem, 100%)` (не зависит от
  контейнера вообще) либо починить containing block в самом `FloatingPopup` — то есть территория
  #112, а не #166.
- analysis_required: **false** — но нужен прогон на мобильной раскладке, иначе проверить нечем
- Vision scene: 7 | User outcome: фильтр блока на узком экране не уезжает за край листа
- Нашли независимо два прохода: `lead` при определении портала для шага 3 и `CX-ADV-166-step3` (P1).

### #181 — Ратчеты рабочего стека не переживают собственный worktree
- Status: ✅ DONE 2026-09-02 (ветка `fix/181-ratchet-worktree-exclusion`, `dbb43b8`, `f30f8f7`, `fbcd0c9`;
  слито в `main` 2026-09-02 вслед за #165) | Milestone: (инфраструктура) | Priority: **P1** | Complexity: XS
- analysis_required: false
- Vision scene: none | User outcome: делегирование по маршрутизации перестаёт красить гейты
- **Конфликт при слиянии был запланирован и сработал.** Тикет заведён на `feat/165` (`7178122`), фикс
  писался от `main`; обе редакции столкнулись в одном месте, и победила эта — она описывает
  выполненную работу. Разрешено 2026-09-02 на интеграционной ветке, `main` двинут fast-forward.
...
- **Причина одна на три файла:** `R0_7:57`, `R0_10:77`, `R0_11:58` — три одинаковых рекурсивных
  обхода без единого исключения, а `.claude/worktrees/` это полная копия репозитория.
- **Почему не заметили раньше:** внутри worktree у ратчетов нет своего `.claude/`, и сьюты там
  **скипаются**. Главное дерево видит ложные падения, агент — ложную тишину; ни одна из сторон не
  видит правды, что и есть определение дыры в гейте.
- **Сделано:** обход один на троих — `src/__tests__/support/configScan.ts`. Исключения разделены
  по причине, а не для удобства: `worktrees` — это **место** (только прямой потомок сканируемого
  корня, потому что `isolation: worktree` кладёт чекаут ровно туда), `node_modules` и `.git` —
  машинерия, исключается на любой глубине. `.git` ловится по имени, а не по типу: в связанном
  worktree это **файл** (`gitdir: …`), и проверка по типу пропустила бы ровно тот случай, ради
  которого всё делается. Ссылки (в том числе junction) раскрываются, как и до #181; конечность
  даёт вход в каждый каталог ровно один раз по реальному пути.
- **Граница исключения закреплена в обе стороны** — `src/__tests__/configScanBoundary.test.ts` на
  временной фикстуре, живой `.claude/` не трогается. Подсаженный файл в `worktrees/` находки не
  даёт, побайтово тот же файл в `agents/` — даёт.
- **Две редакции были неверны, и обе поймал аудит Codex — четыре гейта были зелёными на
  всех трёх вариантах.** (1) Исключение `worktrees` по имени на любой глубине спрятало бы
  обычный конфиг-каталог `.claude/agents/worktrees/` от всех трёх ратчетов сразу (`f30f8f7`).
  (2) Пропуск всех каталогов-симлинков — задуманный против junction на `node_modules` — заодно
  ослеплял ратчеты к привязанному конфиг-каталогу, который старые обходы читали; ссылки снова
  раскрываются, а конечность даёт вход в каждый каталог ровно один раз по реальному пути
  (`fbcd0c9`). Оба случая закреплены фикстурами в `configScanBoundary.test.ts`.
  Ровно то, о чём предупреждал сам тикет: исключение, написанное шире своего предмета, — это
  способ перестать быть гейтом.
- **Смежная операционная ловушка** записана в протокол, а не в память:
  `WORKING_STACK_DESIGN_2026-09-01.md` §4 «Операционный протокол worktree» — junction на
  `node_modules` снимается ДО `git worktree remove --force`, иначе удаление проходит по junction и
  вычищает реальный каталог зависимостей.

### #183 — Дерево legacy Table/DataGrid не собрано и не используется
- Status: ✅ DONE 2026-09-03 (ветка `fix/183-delete-legacy-table`) | Milestone: (next) | Priority: P2 | Complexity: M
- analysis_required: **true — ГЕЙТ ЗАКРЫТ 2026-09-03**: `codex-reports/CX-MAP-183.md`
- Vision scene: none — сопровождаемость | User outcome: в дереве нет поверхности, которой нет в продукте
- **Найдено при планировании #168 (2026-09-03), проверено в основной сессии.** `src/view.ts:14-17`
  регистрирует четыре вида: Board, Calendar, Dashboard, Gallery. Вида Table среди них нет.
  `src/ui/views/Table/` — 37 файлов, 22 Svelte-компонента, около 3000 строк — и **ни одного
  импорта снаружи каталога**: единственные упоминания `DataGrid` вне его — комментарии в
  `contextMenu.ts:10,170` и `RollupCellRenderer.svelte:10`, а `helpers.ts` импортирует только его
  собственный `helpers.test.ts`.
- **Почему это не копия #178.** Там были три файла только с типами: `tsc` в ноль доказывал, что их
  никто не потреблял, а бандл оставался побайтно тем же. Здесь код рантайма — удаление обязано
  сопровождаться картой использований (`code-mapper`), сравнением бандла до/после и проверкой, что
  из дерева не тянут отдельные компоненты (`GridCell/*`, `RollupCellRenderer` соседствует с ним).
- **Сделано 2026-09-03.** Карта использований пришла однозначной: у каждого экспорта и каждого из 22
  компонентов снаружи дерева — NONE, любым путём (статический импорт, `import()`, `require`,
  `jest.mock`, строка пути, реестр). Единственный корень — бочка `DataGrid/index.ts`, которую никто
  не импортирует; `helpers.ts` достижим только собственным тестом. Общего файла с живой Table V2
  дашборда нет, общего CSS нет — `.ppp-rollup-cell` это совпадение имён, каждая сторона объявляет своё.
- **Доказательство сильнее, чем требовал тикет: бандл побайтно тот же.** `main.js` до удаления и
  после — 2 778 073 байта, контрольная сумма совпадает. Дерево никогда в сборку и не попадало:
  esbuild вытряхивал его, потому что на него нет ни одной ссылки. То есть у пользователя ничего не
  меняется, и это измерено, а не выведено.
- **База тестов падает на один сьют и четыре теста** (186/3013 → 185/3009) — это `helpers.test.ts`,
  собственный тест удалённого дерева, проверявший `sortFields`. Прецедент #160, где база упала с
  177 до 174 вместе с моделью sub-base. Падение названо здесь, а не спрятано.
- **Аудит Codex (`codex-reports/CX-AUDIT-183.md`) — вердикт BLOCKED, две находки, обе верны и закрыты.**
  Он подтвердил главное независимо: все 37 удалённых файлов присутствуют в архиве побайтно, ни одного
  лишнего и ни одного пропущенного; `main.js` — тот же git-blob до и после; диф состоит только из
  удалений. Находки были не про удаление, а про то, что вокруг него осталось:
  (1) база тестов падает, а инвариант в `CONTEXT.md` говорит «держится или растёт» — исключение
  теперь **записано** там же: тест, чей предмет удалён, это не покрытие (прецедент #160);
  (2) три документа продолжали указывать на удалённое дерево — `PROJECT-INFO.md`,
  `ARCHITECTURE_V5.md` («под удаление») и ADR по #168 — сопровождающий пришёл бы по ссылке к
  отсутствующим файлам. Исправлены; в ADR добавлена шапка, помечающая все его пути к Table
  историческими разом.
- **UNKNOWN аудита, оставлен как есть:** статический разбор не доказывает, что сторонний плагин не
  дотянется до дерева динамически. `view.ts:145-155` действительно позволяет чужим плагинам
  регистрировать виды, но внутренний неэкспортируемый компонент этим путём недостижим.
- **Текст сохранён:** `docs/internal/archive/LEGACY_TABLE_DATAGRID.md` — 37 файлов дословно, с
  запиской о причине. Одно наблюдение вынесено вперёд: `GridRollupCell` повторял разбор процентов из
  `RollupCellRenderer`, и оба рисовали `null` как видимые «0%». Сам этот рендерер тоже никем не
  смонтирован; когда #180d даст слою отображения единицы, привычку возвращать не следует.
- **Решение за пользователем:** удалить с архивной копией текста, как #178, или восстановить вид
  Table как продуктовую поверхность. Рекомендация: **удалить** — вид не зарегистрирован ни в одной
  версии настроек, а Table V2 дашборда покрывает сценарий.
...
- Status: ✅ DONE 2026-09-04 — слито в `main` вместе с #181; пилотное правило и R0.13 на месте (ветка
...
- Status: ✅ DONE 2026-09-03 — три шага слиты, **приёмка автоматизирована** (`A166_matryoshkaSizing.acceptance.test.ts`); шаг 1 слит 2026-09-02 (merge `010a59d`; коммиты `08ede44` feat /
  `dbe51c0` test / `a31423f` chore / `2bb8e62` fix диалога); **шаг 2 РЕАЛИЗОВАН, ждёт ревью и мёржа**
  (ветка `feat/166-step2-chart-width` от `6c6a82f`, коммиты `c17d358` feat / `139970a` test, плюс
  этот docs-коммит и пересборка бандла); **шаг 3 РЕАЛИЗОВАН, ждёт мёржа** (ветка
  `feat/166-step3-minimums` от `d2f2bbd`, коммиты `1ac1f96` feat / `f81a7d5` test)
  | Milestone: M-MATRYOSHKA | Priority: **P1** | Complexity: L
- **Шаг 3 — что сделано и чем это доказано.** `gridTemplate` дописывает один хвостовой трек `1fr`;
  фиксированные треки колонок не двигаются (проба: `272 176 176` и при 800px, и при 300px), так что
  UT-R2 #083 не открывается заново, а сохранённые `widthRem` не меняют смысла (решение 1).
  Побочно вскрылось, что «один общий шаблон» был полуправдой: `TableHeader` дописывал свой трек
  `2rem` под кнопку `[+]`. Теперь заголовок читает `var(--ppp-dt-columns)` как строка и футер, а
  `[+]` живёт в филлере с `width: 2rem` — замер: 0px после последней колонки при обеих ширинах,
  то есть кнопка осталась на месте, а слак открывается справа от неё. Контейнер `db-table` удалён
  (решение 3). Поповер фильтра оставлен буквальным `22rem`: `FloatingPopup` портирует десктопный
  попап в `<body>` (`FloatingPopup.svelte:255`), контейнера-предка нет, и замер показывает, что
  `min(22rem, 100cqi)` там разрешается в те же 352px — правка была бы доказуемым no-op.
  Свидетель R0.13 «скан читает дерево, а не список» переехал с `.ppp-dt-content`, который шаг 3
  удаляет, на `.ppp-widget-config`. Гейты на `f81a7d5`: build 0, jest **183/2549** (база 183/2544,
  +5), lint 0 ошибок / 109 предупреждений, svelte-check 0/0. Проба —
  `docs/internal/probes/166-minimums.html`, числа в ADR.
- **Тикет закрыт НЕ БУДЕТ до прогона в хранилище.** Все три шага реализованы, но каждое утверждение
  о размере — это headless Chrome на реконструкции правил, а не Obsidian на настоящей таблице;
  jsdom контейнерные запросы не видит, и зелёный jest сюда не доказательство (ADR, RISKS 1).
  Не проверено вживую: вид и зона попадания `[+]` после замены `flex`→`grid` в центрировании,
  филлер под сгруппированной таблицей и под строкой агрегатов футера, и перетаскивание границы
  колонки при наличии филлера.
- **Шаг 1 — что сделано и чем это доказано.** `.ppp-database-root` и `.ppp-database-canvas`
  объявлены контейнерами с именами; `cqi`-форма `--ppp-local-text-sm` уехала из `:root` в правило
  с селектор-списком объявленных корней, а имя в `:root` осталось на **полу** clamp без `cqi`.
  R0.13 получил `CONTAINER_ROOTS` и +5 тестов, проверяющих список в обе стороны; обе подсадки
  падают адресно (`:root` и корень без `container-type`). Пробы прогнаны подряд в одном headless
  Chrome, root font-size 16px:

  | Ячейка | `165-cqi-in-custom-property.html` | `166-no-container-fallback.html` |
  |---|---|---|
  | контейнер 200px | 16px | 16px |
  | контейнер 800px | 18.4px | 18.4px |
  | **без контейнера** | **20px** (потолок) | **16px** (пол) |

  Пилот (`ChartWidget`) сохраняет вычисленное значение — «ничего не двигается» выполнено.
  Гейты: build 0, jest 182/2526 (было 182/2521), lint 0 ошибок / 112 прежних tsdoc-warning,
  svelte-check 0/0.
- **Блокер мёржа из грепа риска 4 — найден и устранён до слияния.** `TemplateConfirmDialog.svelte:51`,
  оверлей `position: fixed; inset: 0`, рисовался **внутри** `.ppp-database-root`; после шага 1 корень
  стал бы его containing block, и на дашборде выше экрана диалог центрировался бы в контенте, а не на
  экране. Аудитор Codex нашёл то же независимо. Устранено выносом диалога на уровень `ViewContent`
  (сосед корня): containing block снова `ViewContent`, геометрия до-шага-1 сохранена побайтно.
  Портирование в `<body>` отвергнуто — оно сменило бы скрим с размера вью на размер окна, то есть
  сдвинуло бы. Диалог читает только переменные Obsidian, каскад корня ему не нужен.
- **Шаг 2 — что сделано и чем это доказано.** `ChartWidget` измеряет свою обёртку и прибивает
  viewBox к измеренной ширине; шесть `width={480}` заменены измерением, круговые/donut получают
  `min(измеренное, height)`, ноль (свёрнутый виджет) удерживает последнюю ненулевую ширину —
  нулевой viewBox это деление на ноль во всех scale-функциях. Комментарий пилота переписан: он
  обещал провести подписи SVG через шкалу, а Q1 ADR это отвергает.
  Проба `166-chart-viewbox-scale.html`, тот же headless Chrome, четыре ячейки в одном прогоне:
- **Шаг 2 — два ревью Codex (`CX-AUDIT-166-step2.md`, `CX-ADV-166-step2.md`), четыре находки, все
  верны и закрыты до слияния:** пирог всё ещё растягивался (`width: 100%` при квадратном viewBox) —
  атрибуты `width`/`height` на SVG + `max-width: 100%`, тест; Scatter и Progress держали фиксированное
  число подписей — `tickCountFor(plotW)` (2 при 160px, 6 при 480, 8 при 1600) и усечение подписи
  Progress; первая проба не несла реальных CSS-правил корней — вторая проба
  `166-chart-root-sizing.html` на 160/480/1600px: масштаб 1 у всех корней, пирог 240×240 по центру.
  Открыто и записано: прогон в vault с реальными данными на трёх ширинах.

  | Контейнер | СЕЙЧАС (viewBox 480) | ПОСЛЕ (viewBox = контейнер) |
  |---|---|---|
  | 240px | 5.5 CSS px | 11 CSS px |
  | 960px | 22 CSS px | 11 CSS px |
  | разброс узкий→широкий | **4×** | **1×** |

  Гейты: build 0 ошибок, jest **183 сьюта / 2539 тестов** (было 182/2526: +1 сьют, +13 тестов),
  lint 0 ошибок / 112 прежних tsdoc-warning, svelte-check 0/0. R0.3 держится на 151 — первая
  редакция комментария содержала строку `11px` в прозе, ратчет поймал 152 > 151.
- **Найдено по ходу и починено: `PieChart` считал `CX/CY/R` через `const`, один раз при монтировании.**
  Пока ширина была константой на монтирование, это не всплывало (хотя правка высоты в конфиге уже
  дотягивалась); после шага 2 ширина меняется на каждом ресайзе, и устаревший центр рисовался бы
  против живого viewBox — пирог мимо собственной середины, ровно в узких виджетах, ради которых
  тикет и делается. Четыре объявления стали `$:`, геометрия передаётся в `computeSlices`
  аргументами (Svelte отслеживает только те идентификаторы, которые названы в реактивном выражении).
- **Отклонение от Q2 ADR, осознанное: `bind:contentRect` вместо `bind:clientWidth`.** ADR выбрал
  `clientWidth` ровно за то, чего у него нет: в Svelte 3.59.2 это биндинг через
  `add_iframe_resize_listener` — вставленный `<iframe>` плюс запись `style.position`, — тогда как
  `contentRect` компилируется в `resize_observer_content_box.observe` и отдаёт **content**-бокс,
  то есть ровно ту коробку, которую заполняет `<svg>` (padding обёртки не попадает в масштаб).
  Проверено в собранном бандле: `ResizeObserver` есть, `iframe` нет ни одного.
- analysis_required: **true** (было false — L требует архитектора) — **ГЕЙТ ЗАКРЫТ 2026-09-02**:
  `ADR_MATRYOSHKA_SIZING_2026-09-02.md` | Depends on: #165 ✅
- **План — три шага, порядок принудительный:** (1) цепочка `dashboard-root` → `dashboard-canvas` объявляет
  себя контейнером, а `cqi`-форма `--ppp-local-*` уезжает из `:root` под селекторы объявленных корней,
  так что потребитель без контейнера получает **пол**, а не потолок; R0.13 получает список
  `CONTAINER_ROOTS`. Ничего не двигается. (2) чарт берёт ширину из `bind:clientWidth`, viewBox
  прибивается к измеренной ширине — 11 единиц становятся ~11px при любой ширине вместо
  безграничного геометрического роста; CSS `font-size` на SVG `text` **отвергнут**, потому что
  `axisLabels.ts:77-107` считает геометрию из того же числа и рендер разошёлся бы с раскладкой.
  (3) `min-width: min(22rem, 100cqi)` у поповера фильтра, хвостовой `1fr`-трек в `gridTemplate`
  таблицы (колонки остаются фиксированными — UT-R2 #083), неиспользуемый контейнер `db-table` удаляется.
- **Приёмка больше не ждёт человека у экрана (2026-09-03).** Три шага #166 оставили после себя
  пункты вида «закрыть может только прогон в хранилище», и они накапливались. Все они — утверждения
  о каскаде и раскладке, а об этом можно спросить браузер. `src/__tests__/support/renderProbe.ts`
  запускает headless Chrome как обычную зависимость теста, а `A166_…acceptance.test.ts` собирает
  страницу **из самих исходников**: `tokens.css` дословно и блок `<style>` компонента дословно.
  Это ответ на замечание adversarial-ревью шага 2: прежние пробы были реконструкциями и могли
  согласиться с убеждением вместо продукта.
- **Что закрыто автоматически:** токен даёт разный размер в узком и широком контейнере;
  потребитель без контейнера получает **пол**, а не потолок; подпись чарта одного размера при 240 и
  960px, **и контрольный тест на старую форму доказывает, что разброс был четырёхкратным** — иначе
  зелёный тест не отличить от теста ни о чём; пирог не растягивается шире своей высоты; `min()`
  сжимает минимум в узком контейнере и не трогает широкий; хвостовой `1fr` забирает остаток и не
  двигает фиксированные треки (это #083, ставший тестом, а не комментарием).
  Проверено, что сьют **падает**: возврат `width: 100%` пирогу и возврат `cqi`-формы в `:root` дают
  по красному тесту каждый.
- **Чего он не закрывает, прямо:** красиво ли это выглядит. Чарт с одинаковыми подписями и
  центрированным пирогом может быть уродлив на 1600px — это суждение человека, и теперь оно
  единственное, что осталось человеку.
- **Если Chrome не найден, сьют не молчит:** он печатает предупреждение, что приёмка #166 в этом
  прогоне не выполнялась. Исчезнувший сьют неотличим от пройденного.
- **RESOLVED 2026-09-02 (пользователь): все три — да.** (1) ширины колонок остаются буквальными,
  пропорциональные — отдельный тикет; (2) чарты меняют пропорции без настройки; (3) `db-table` удаляется.
  Общий принцип, заданный пользователем для этого и остальных решений: приоритет — описание принципов
  решения через обзор референсных решений в других приложениях (как `REFERENCE_NOTION_UI_2026.md`).
...
- R0.3 сторожит букву («мало px») и пропускает смысл: `rem` — это привязка к корню, то есть все
  матрёшки одного размера независимо от вложенности. Тест не заметил, что принцип потерян.
  (Прежняя редакция этого тикета называла здесь конкретное значение `PX_BUDGET`; оно к тому моменту
  уже устарело. Каноничное значение — константа в `src/__tests__/R0_3_pxBudget.test.ts`, и
  переписывать его сюда не надо: см. правило «Numbers».)
- **Что сделано.** Абсолютным инвариантом это сегодня быть не может — сам тикет мерил порядки
  разницы между `rem` и `em`, — поэтому правило легло **храповиком** в форме R0.3:
  `REM_IN_CONTAINER_BUDGET` в `src/__tests__/R0_16_remInContainer.test.ts`, который можно только
  понижать, и понижать удалением кода.
  - **Стартовое значение 807 — измерено, а не выбрано** (56 файлов в области на `09fef14`).
    Храповик, чей потолок выбран выше измерения, не видит удаления: у R0.3 потолок оторвался от
    дерева на 23 единицы, и в этот зазор поместился мёртвый файл токенов, который нашли руками
    в #165. Подсадка одного объявления обязана ломать бюджет — это утверждение в тесте, и вместе
    с `<=` оно прибивает потолок **к** измерению.
  - **Область — правило по дереву, а не список файлов.** Компонент считается, если лежит под
    `ui/views/Dashboard/` (цепочка контейнеров, которую R0.13 проверяет поимённо) **или** сам
    пишет `@container` (уже объявил, что контейнер решает его брейкпоинты, — значит не вправе брать
    размер от корня документа; этой веткой в область входят три компонента Calendar). Оба пункта
    проверяются в обе стороны, так что область нельзя тихо сузить, чтобы погасить падение.
  - **Список исключений — часть теста.** `TemplateConfirmDialog` рендерится вне
    `.ppp-database-root` намеренно, `FloatingPopup` портирует себя в `<body>`; ADR фиксирует этот
    разрыв родства как несущий. Тест требует, чтобы каждая запись была реальным файлом и чтобы
    диалог без неё попадал в счёт, — список не может стать декоративным.
    **Намеренно НЕ освобождены** `DashboardBlockPalette`, `WidgetToolbar`, `BlockFilterBar`: каждый
    держит в одном файле и триггер внутри контейнера, и портированное тело попапа. Освобождать файл
    целиком ради его попап-половины значит освободить и ту половину, что реально внутри виджета.
    По итогам аудита проверка усилена: одного существования файла мало — запись обязана
    **предъявить механизм**, на который ссылается (собственный `position: fixed` в стилях либо
    портал), и длина списка ограничена. Это сужает дыру, но не закрывает её, и так и записано в
    тесте: принадлежность контейнеру выведена из каталога и объявленного запроса, а не из реальной
    цепочки предков, — статически последнее недоказуемо (ADR), потому #166 и положил гарантию
    в каскад.
  - **Комментарии не считаются, фолбэки считаются.** У R0.3 обратное соглашение, и в его журнале
    четыре записи «comment scrub» — конверсии, не сдвинувшие на экране ничего. `var(--x, 0.375rem)`
    — это размер, иначе бюджет гасится оборачиванием литералов в `var()`. Инлайновый стиль тоже
    считается, иначе бюджет падает переносом кода, а не его удалением, — и **во всех четырёх
    формах**: `style="…"`, `style={…}`, `style:name="…"`, `style:name={…}`.
  - **Что нашёл аудит Codex и что с этим сделано (CRITICAL, подтверждено чтением кода; отчёт — `codex-reports/CX-AUDIT-167.md`).** Первая
    редакция читала только закавыченный `style="…"`. `StatsCard.svelte:68` всё это время отдавал
    `0.1875rem` через `style={…}`, а `AllDayEventStrip.svelte:122-125` задаёт размеры директивами
    `style:` — оба внутри области и оба невидимы. Это не недосчёт, а выданный способ понижать
    бюджет переносом объявления, то есть ровно то, ради отказа от чего бюджет и существует.
    Читатель переписан на разбор всех четырёх форм со сбалансированными скобками; бюджет
    **перемерен** (806 → 807), а не увеличен. Граница названа явно: у `"{STRIP_HEIGHT_REM}rem"`
    цифр в тексте нет, такую единицу счётчик увидеть не может, и ограничение принадлежит
    регулярке, а не охвату. Дополнительно проверено и закреплено тестом: под областью нет ни
    одного `.css` (единственный в дереве — `ui/tokens/tokens.css`, это уровень 1 на `:root`,
    привязка к корню там намеренная) и ни одного `rem` в `.ts` Dashboard.
  - **Второй проход аудита (HIGH x2, обе верны) и почему счётчик стал читать файл целиком.**
    Codex нашёл ещё два маршрута: сокращённые формы `{style}` (`FloatingPopup.svelte:291`) и
    `style:width` (`SlideInPanel.svelte:54`), а также случай, когда литерал поднят в скрипт и
    в разметке стоит только имя переменной. Сокращённая форма не несёт значения у элемента
    вообще — значение живёт в скрипте, — поэтому ни один читатель, построенный вокруг
    привязок стиля, не мог бы их сосчитать. Важно не то, что синтаксисов оказалось больше:
    важно, что это второй аудит подряд находит маршрут, который читатель не моделирует, и
    каждая моя правка была уже дыры. Поэтому читатель перестал перечислять маршруты и
    считает **весь текст компонента** без комментариев — ровно так R0.3 всегда считал `px`.
    Перенос перестаёт помогать по построению, а не по полноте перечисления. Перемер при
    смене правила: 807 → 807 — названные маршруты реальны, но сегодня по ним ничего не
    ездит, так что дыра закрыта, а число не сдвинулось (в журнале это записано: перемер с
    тем же значением — это свидетельство, а незаписанный выглядит как «ничего не было»).
    Проверка подсадкой ровно по сценарию аудита: `const style = "gap: 0.75rem"` в скрипте
    `StatsCard.svelte` и `{style}` в разметке роняют бюджет `808 > 807`.
    Вторая находка: проверка «предъяви механизм» принимала само слово `FloatingPopup`,
    которое стоит в заголовке этого же файла, — то есть исключение пережило бы удаление
    `use:portal`. Тест, который компонент проходит потому, что *назван* в честь того, что
    он якобы делает, — не тест. Теперь требуется `use:portal` вне комментариев либо
    `position: fixed` в настоящем правиле, и предикат доказан в обе стороны на синтетике,
    включая три почти-попадания.
  - `svelteStyles` / `stripCssComments` / `collectStyled` вынесены из R0.13 в
    `src/__tests__/support/cssScan.ts` — два определения «CSS этого компонента» дали бы двум
    храповикам разойтись в том, что они сторожат (та же причина, по которой этажом выше живёт
    `configScan.ts`, #181). R0.13 в остальном не изменился и проходит те же 20 тестов.
- **Доказательства.** Две подсадки в реальные файлы, обе сняты, дерево зелёное:
  `gap: 0.75rem` в `<style>` у `WidgetShell.svelte` роняло `807 > 806` с `WidgetShell` на 24
  вместо 23 (до правки читателя); после правки `style:padding={\`0.625rem\`}` у `StatsCard.svelte`
  — форма, которую аудит назвал невидимой, — роняет `808 > 807`. Сообщение об ошибке печатает
  десятку нарушителей и число чтений контейнерной шкалы. Гейты после правки: build 0 ошибок,
  jest **184 / 2559** (база `main` 183 / 2549 — +1 сюита / +10 тестов), lint 0 ошибок при
  109 предупреждениях (совпадает с базой), svelte-check 0/0.
- **Что этим НЕ доказано.** Храповик считает единицы в тексте — он не открывал Obsidian и не мерил
  ни одного пикселя. Он говорит, сколько корневой привязки осталось внутри контейнеров и в какую
  сторону её убирать; он не говорит, что убранное выглядит правильно. Верхние цели для следующего
  понижения: `HeaderStripsSection` (136), `PipelineEditor` (102), `Day` (51), `WidgetConfigShell`
  (43), `AllDayEventStrip` (31).
- **Инвариант 13 в `CLAUDE.md` не добавлен:** файл локальный и не отслеживается git, править его
  из пайплайна нельзя. Строку «внутри объявленного контейнера размер не берётся от корня документа
  (R0.16)» в список инвариантов добавляет пользователь.

### #168 — Пик вместо ухода: открытие записи не должно терять контекст
- Status: ✅ DONE 2026-09-03 — шаги (a), (b) и (c) сделаны
  | Milestone: M-MATRYOSHKA | Priority: **P1** | Complexity: M
- analysis_required: **true** (было false — контракт пересекает 5 поверхностей) —
  **ГЕЙТ ЗАКРЫТ**: `ADR_RECORD_OPEN_CONTRACT_2026-09-02.md` + трассировка `codex-reports/CX-MAP-168.md`
- **Буква тикета расходится с кодом, и это выяснилось до реализации.** «Поверх существующего
  `VisualizerPane`» построить нельзя: панель следует за активным файлом
  (`VisualizerPane.svelte:74-85`, подписка на `active-leaf-change`), поэтому показать в ней запись
  можно только открыв её во вкладке (`main.ts:450-464`) — ровно тот уход, который тикет запрещает.
  Зато готовая поверхность пика уже есть и мертва: `RecordCardView.svelte` (это `EditNote` внутри
  `SlideInPanel`), у неё нет ни одного продакшн-потребителя. **Решение: пик — это она, а не лист
  сайдбара**; лист остаётся возможной второй поверхностью («закрепить запись справа»).
- **Контракт:** один модуль `src/lib/record/openRecord.ts` с `openRecord(target, mode, deps)` и
  `modeFromEvent` (shift → окно, ctrl/meta → вкладка, обычный клик → `PLAIN_MODE`). Переключение
  поведения — **одна константа в одном файле**, поэтому шаг (b) откатывается, не трогая места вызова.
- **Трассировка Codex поправила план в трёх местах (перепроверено в основной сессии):**
  (1) **вида Table не существует** — `src/view.ts:14-17` регистрирует только Board, Calendar,
  Dashboard, Gallery, а весь `src/ui/views/Table/components/DataGrid/**` не импортируется ниоткуда;
  «клик по строке» из тикета — это Table V2 дашборда (`TableRow.svelte:60` →
  `DataTableContent.svelte:135`). Пробел, который план считал блокирующим шаг (a), не существует;
  мёртвое дерево вынесено в **#183**. (2) класс A — **24** места, а не 18: шесть мест, отнесённых к
  классу B, получают `record.id` через `InternalLink` с фиксированными входами
  (`CardList.svelte:126`, `EventList.svelte:92`, `GalleryView.svelte:188`). (3) `helpers.ts:152,156`
  (`showMobileNavMenu`) — **третья категория**: параметры произвольные, мигрируются в шаге (a).
- **Три шага:** (a) контракт + миграция 24 мест + ратчет, **поведение не меняется**
  (`PLAIN_MODE = "same"`); (b) переключение на пик + панель «поле ↔ ключ frontmatter» (это и есть
  содержание сцены 3, которую сегодня не закрывает ни одна поверхность); (c) статус ссылки (#151)
  и клавиатура, вместе с #169.
- **Шаг (a) сделан 2026-09-03, поведение не изменилось.** `src/lib/record/openRecord.ts` — контракт;
  соглашение о модификаторах прочитано во всех трёх поставленных реализациях, они совпадают, поэтому
  воспроизведено, а не переизобретено. `PLAIN_MODE = "same"` — та самая одна константа, которую
  переключит шаг (b). Мигрированы все места класса A в Board, Calendar, Gallery, Inspector и
  Table V2 дашборда; `showMobileNavMenu` принимает запись вместо произвольной пары «ссылка + путь»,
  так что вызывающий заявляет, что открывает.
- **Шаг (b) сделан 2026-09-03 и оказался одной строкой, как и было задумано.** `PLAIN_MODE` стал
  `"peek"` — переключение поведения на всех пяти поверхностях без единой правки в местах вызова.
  Откат — тоже одна строка, и это закреплено тестом, чтобы он был видимым диффом, а не тихой сменой
  поведения.
- **Пик монтируется один раз на `View.svelte`, а не в каждом виде, как предлагал ADR.** Дашборд
  держит несколько табличных виджетов, и вариант «у каждого свой пик» вынужден отвечать соглашением
  на вопрос «что если открыты два». Хранилище отвечает типом: значение одно, второе открытие
  заменяет первое, «двух пиков» не бывает конструктивно. Отступление от плана названо здесь, а не
  оставлено молча.
- **Запись ищется в отсортированном кадре вида**, а не подгружается: пик — это ближе рассмотреть то,
  что на экране, а не обход фильтра.
- **Сцена 3 закрыта тем, чего в продукте не было.** Её критерий готовности — «инспекция показывает
  ключ frontmatter»; редактор показывал поля вида, боковая панель — сырые ключи, и ни одна не
  говорила, что это одно и то же. А это одно и то же: `dataApi` пишет `frontmatter[field.name]`.
  Пик теперь перечисляет ключи с их значениями так, как они лягут в файл; отсутствующий ключ пишется
  прочерком, а не пустой строкой, похожей на пустое значение.
- **Adversarial-ревью шага (b) (`codex-reports/CX-ADV-168b.md`) — вердикт BLOCK, четыре находки,
  все верны. Три критические исправлены, четвёртая записана с точным условием.**
  - **P1 — строки внешнего источника перестали делать что-либо.** Табличный виджет дашборда умеет
    читать чужой источник, чьих записей в кадре вида нет вовсе; поиск по одному идентификатору
    оставлял панель закрытой, то есть клик без результата — хуже, чем уход, который он заменил.
    **Исправлено двумя способами сразу:** цель пика теперь может нести саму запись и её поля (у
    вызывающего они есть), а если запись всё-таки не разрешилась — вид не молчит, а открывает
    заметку, то есть возвращается к прежнему поведению. Ратчет R0.17 тут же поймал первую версию
    этого отката за прямой вызов рабочей области мимо контракта.
  - **P1 — закрытие пика могло выбросить набранное.** `EditNote` откладывает сохранение на 300 мс и
    **отменял** таймер при размонтировании. В модалке это было незаметно — её закрывают осознанно;
    в пике, который выглядит предпросмотром и закрывается мимоходом, правка терялась. Теперь таймер
    не отменяется, а сбрасывается: то же сохранение, только сразу.
  - **P1 — «ключи frontmatter» было ложью для производных полей.** `dataApi` их не пишет, поэтому
    формулы, роллапы и спутники отношений `__resolved__…` — не ключи. Исключены; панель перестала
    говорить о файле неправду.
  - **P2 — хранилище пика одно на модуль, а листов Projects может быть несколько.** Записано как
    известное ограничение: последствие — панель откроется в обоих листах, где запись есть, а не
    потеря данных, и переносимая запись (P1) убирает половину проблемы «не откроется нигде».
    Ограничение по владельцу — отдельный тикет, если два листа окажутся обычным сценарием.
- **Шаг (c) сделан 2026-09-03 — и он закрыл открытую половину #151.** Тикет сам записал, почему
  не может: пометить неразрешённую ссылку **в ячейке таблицы** нужен целевой кадр в самой ячейке, а
  его там нет. В пике он есть — пик показывает **обогащённую** запись, поэтому спутник
  `__resolved__<field>` лежит рядом с сырыми вики-ссылками, и разница между ними и есть ответ.
  Ни нового пути данных, ни повторного разрешения.
  Называются только сломанные ссылки: галочка рядом с каждой рабочей — это шум, а одна строка про
  ту, что не нашлась, и есть то, что просит сцена 4. Сопоставление по пути и по имени файла — по тем
  же двум ключам, по которым индексирует сам контракт.
- **Приёмка (`A168_peekNotLeaving.acceptance.test.ts`):** обычный клик открывает пик и **ни разу** не
  просит рабочую область что-либо открыть; ctrl и shift по-прежнему дают вкладку и окно, то есть
  ничего не стало недостижимым; поверхность со своей панелью спрашивают вместо хранилища; второе
  открытие заменяет первое; мост от старой булевой формы следует той же константе, иначе переключение
  было бы частичным. Проверено, что откат константы даёт пять красных тестов.
- **Исключений осталось четыре, а не шесть:** `GridFileCell` и `TextLabel` ушли вместе с мёртвым
  деревом в #183. Оставшиеся четыре берут ссылку из отрисованного Markdown или разобранного
  вики-линка, и цель может не быть записью вовсе.
- **Ратчет R0.17** закрепляет это в обе стороны: прямой вызов рабочей области вне контракта и
  списка — падение с путём и строкой; **и** объявленное исключение, которое перестало быть
  исключением, — тоже падение, иначе список тихо расширяется. Оба подсадка прогнаны.
- **Поймано гейтом, а не мной:** `BoardView.svelte` был мигрирован прерванным прогоном без импорта.
  `tsc` этого не видит — он не проверяет разметку Svelte, — а `svelte-check` увидел. Ещё одна
  причина не считать четыре гейта взаимозаменяемыми.
- **Перевёрнуто одно утверждение:** guard-тест #142 закреплял написание вызова
  (`openLinkText(record.id, record.id, false)`) внутри ветки read-only. Смысл гарантии тот же —
  read-only галерея открывает заметку, а не редактор, — но шаблон теперь следует контракту, иначе
  шаг (b) сменил бы поведение везде, кроме этой ветки.
- **RESOLVED 2026-09-03 — решение делегировано пользователем («решай весь стек»), принято по
  рекомендациям ADR:** (1) пик — **внутренняя выезжающая панель** (`RecordCardView`), лист сайдбара
  остаётся возможной второй поверхностью; (2) режим обычного клика **одинаков для всех видов**;
  (3) пик **редактируемый** — переиспользует `EditNote`, «только чтение» это новый код, чтобы отнять
  возможность; (4) настройку `defaultRecordOpenMode` **не заводить** — ключ навсегда и пять файлов,
  а модификаторы оставляют прежнее поведение доступным; (5) шаг (b) **блокируется на focus-trap**
  из #169: панель объявляет себя модальной и не берёт фокус, для клавиатуры это регресс.
  Следствие порядка: **#169 выполняется перед шагом (b)**.
...
- Status: ✅ DONE 2026-09-03 — клавиатурная часть, шкала слоёв и визуальный вес сделаны,
  приёмка автоматизирована в браузере
  | Milestone: M-MATRYOSHKA | Priority: P2 | Complexity: M
- analysis_required: false
...
- **Сделано 2026-09-03.** `src/lib/a11y/focusTrap.ts` — одно действие, а не две копии: та же ошибка
  в двух файлах это форма, за которую проект уже платил. `SlideInPanel` и `SettingsMenuPopover`
  объявляли `aria-modal="true"` и отпускали Tab наружу — теперь фокус входит внутрь, ходит по кругу
  и возвращается тому, кто его отдал, но **не отбирается**, если вызывающий увёл его намеренно.
- **Скрытые действия ушли из табуляции:** `opacity: 0` их в ней оставляла, поэтому обход дашборда
  спотыкался четыре раза на виджет о кнопки, которых не видно. Теперь `visibility: hidden`.
- **Первая попытка была неверна, и это записано в тесте, а не забыто:** я открывал их по
  `:focus-within`, чтобы они «оставались доступны с клавиатуры». Так не бывает — элемент с
  `visibility: hidden` нефокусируем, фокус внутрь никогда не попадёт и правило не сработает.
  Клавиатурный путь — всегда видимая кнопка меню, которую собственный комментарий компонента
  называет единственным входом.
- **Приёмка автоматизирована.** jsdom вообще не перемещает фокус по Tab, поэтому проверка там была
  бы утверждением о тесте, а не о продукте. Сьют собирает `focusTrap.ts` esbuild-ом в отдельном
  процессе (в jsdom esbuild не запускается — подменён `TextEncoder`) и гоняет его в headless Chrome
  над настоящей разметкой; правила видимости берутся из `<style>` самого компонента. Проверено, что
  падает: снятие цикла в Tab и возврат `opacity`-only дают красные тесты.
- **Шкала слоёв (2026-09-03).** Единая шкала `--ppp-z-*` уже была объявлена в `tokens.css`; чего не
  было — соблюдения. **Найден и закрыт настоящий дефект:** один и тот же слой писался как
  `var(--ppp-z-overlay, 49)` в `FloatingPopup` и `var(--ppp-z-overlay, 1200)` в `SettingsMenuPopover`,
  плюс `var(--ppp-z-modal, 1210)` там же. Fallback срабатывает ровно тогда, когда токена нет — то
  есть в единственном случае, когда эти два разошлись бы на 1151. Все три приведены к объявленным
  значениям, и R0.19 запрещает такое противоречие **без бюджета**: это не вопрос степени.
- **Сырые значения ратчетятся, а не переписываются разом.** Их 71, и переписать всё одной правкой —
  это изменение стекинга по всему дереву, проверить которое можно только глазами на каждой
  поверхности. Ровно та форма, за которую только что наказал #182. Поэтому число измерено и может
  только падать, как в R0.3, и падает переносом значения на шкалу, а не правкой константы.
  Сообщение о падении называет пять худших файлов — куда резать.
- **Визуальный вес: спецификация написана 2026-09-03** — `SPEC_WIDGET_WEIGHT_2026-09-03.md`
  (проход `designer`, как требует маршрутизация).
- **Половина этой части уже построена, и тикет держал её открытой зря.** `BlockFilterBar` —
  постоянно видимая полоса фильтра — существует и используется `database-call`
  (`DatabaseCallBlock.svelte:402`), видна безусловно в режиме правки и показывает «+ Фильтр», когда
  фильтров нет. Просто нигде не было сказано, что она закрывает требование. У `data-table` место
  зарезервировано собственным комментарием `TableControlBar`. То есть это «повторить у второго
  виджета то, что у первого есть», а не новый интерфейс.
- **Правило первичного действия, а не список:** оно есть у виджета только если **создаёт или
  добавляет то, ради чего блок читают**. Фильтрация, сортировка, настройка вида и навигация
  первичными не бывают. У чарта и Stats первичного действия **нет** — и это ответ, а не пробел.
- **Три веса вместо нынешней плоскости:** заливка акцентом с текстовой подписью (первичное),
  всегда видимая приглушённая иконка (меню `⋯`), скрытая до наведения бледная иконка (cog и
  остальные). Меню `⋯` не трогается — это клавиатурный вход, закреплённый `A169`.
- **Проход архитектора сделан 2026-09-03** — `PLAN_169_WIDGET_WEIGHT_2026-09-03.md`. Его выводы
  проверены по дереву перед реализацией, и три из них оказались неверны. Это записано целиком,
  потому что каждый из них изменил бы работу.
- **Опровержение 1, крупное: полоса фильтра у `data-table` уже стоит.** И тикет, и спецификация
  дизайнера, и план архитектора читали `data-table` как близнеца, которому полосы не досталось.
  Досталось: `data-table` рисуется **через** `DatabaseCallBlock` (`widgetComponentRegistry`,
  `restoreDataTableConfig`), а блок монтирует `BlockFilterBar` в ветке, куда попадает при наличии
  хотя бы одной вкладки — а `restoreDataTableConfig` всегда выдаёт ровно одну. Единственный вентиль
  на полосе это `tabs.length === 0`, и он никогда не срабатывает. Целый запланированный коммит
  («принять полосу») отменён: он добавил бы **вторую** поверхность фильтра в блок, то есть ровно ту
  параллельную реализацию, которую проект ловит ратчетами. Закреплено с двух независимых сторон в
  `A169_widgetWeight` — по логике конфига и по позиции монтажа в исходнике.
- **Опровержение 2: `.ppp-dt-content` больше не объявляет `container-type`** — это снял #166.
  План строил на нём весь довод в пользу именованного запроса. Вывод («писать `@container widget`,
  а не безымянный») остаётся верным, но механизм другой: не «перехватит ближайший контейнер
  сегодня», а «не даст ему перехватить завтра».
- **Поправка 3: запаса LOC везде на строку меньше**, чем измерил архитектор: ратчет считает
  `split("
").length`, а не переводы строк. `DataTableContent` стоял ровно на 280/280 — ноль, а не
  единица. Из-за этого пришлось разгрузить на один файл больше.
- **Сделано: собственное действие блока тяжелее его настроек.** `WidgetPrimaryAction.svelte` —
  отдельный компонент, и это структура, а не оформление: спецификация просит «своим элементом,
  никогда внутри hover-группы», а соседний файл делает это тем, что следующая правка не сможет
  потерять, дописав один селектор в список. Заливка акцентом, иконка **и текстовая подпись** (у всех
  соседей голый глиф — на этом и держится большая часть разницы веса), видимость в покое.
- **Действие не выполняется в заголовке, и это несущее решение.** Заголовок поднимает счётчик,
  блок решает, каким из **уже существующих** путей создать запись: есть строки — открывается
  встроенная «+ New», нет — модалка пустого состояния. Если бы `WidgetHost` звал `createNamedRecord`
  сам, общей была бы функция, но не взаимодействие: у блока появился бы второй, иначе ведущий себя
  способ создать запись рядом с первым.
- **Дефект, которого не видят ворота, закрыт до того, как случился.** У свёрнутого виджета заголовок
  есть, а содержимого нет вовсе — клик молча не делал бы ничего. Обработчик разворачивает **до**
  подъёма сигнала, оба приёмника сравнивают с нулём (значение, пришедшее при монтаже, считается
  нажатием), а хост гасит сигнал на каждом сворачивании — иначе повторное разворачивание само
  открывало бы строку ввода. Строка ввода к тому же скроллится в видимую зону: она стоит после
  оконного списка, и фокус вне экрана выглядит как «кнопка не сработала».
- **Схлопывание в узком контейнере сделано не так, как просила спецификация, и вот почему.** Она
  просила пилюли «только иконка». По существу это неверный размен: пилюля без текста сообщает лишь,
  что **какой-то** фильтр есть, а какой — нет; нечитаемый фильтр хуже переносящегося, а полоса и так
  переносит. Поэтому подпись не исчезает, а **уступает место в единицах контейнера**: `8rem` выше
  привязан к корню документа и режет подпись одинаково в виджете шириной в колонку и во весь холст,
  а `40cqi` — доля этого виджета. Подпись самой кнопки при этом всё же убирается: она дублируется
  иконкой «+» и `title`, то есть теряется оформление, а не смысл.
- **Ратчеты названы до того, как сработали, и один сработал.** Реестр перевалил за свой потолок 260
  — сборка контекста уехала в `renderContext.ts`, а предикаты заголовка в `headerChrome.ts`
  (последний ещё и потому, что импорт реестра тянет весь граф компонентов и юнит-тест падал на
  модалке, которую в нём не собрать). R0.16 прибит ровно к измерению, поэтому весь новый CSS в
  `em`/`%`/`cqi`, включая порог `24em` внутри условия `@container`: `rem` там считался бы как размер.
- **Два инварианта пришлось провести за переехавшей проводкой**, а не ослабить: `R_filterOrder` и
  `linkedSourceWrites` читали `dbCallScopeApplied` и `dbCallUsesLinkedSource` из `WidgetHost`.
  Композиция не изменилась ни на символ — изменился адрес, и тесты теперь читают его там, где он
  собирается.
- **Состязательное ревью нашло две настоящие вещи, и обе исправлены** (`codex-reports/CX-ADV-169.md`).
  **P1, крупная:** кнопка предлагалась каждому локальному `database-call`, а он держит вкладки Board,
  Calendar и Gallery, каждая из которых создаёт запись **с контекстом того, на что смотришь** —
  значение колонки, день, по которому кликнули, активный фильтр. Мой приёмник проваливал их в общую
  модалку без контекста, то есть запись, созданная из заголовка на вкладке Calendar, **не появилась
  бы в том календаре, который её создал**. Прямое противоречие моему же обоснованию («ярлык к уже
  существующему взаимодействию»). Исправлено по правилу, а не в обход: `database-call` действия в
  заголовке не получает вовсе, создание там принадлежит виду. У `data-table` вопроса нет — это
  всегда ровно одна табличная вкладка. **P2:** проверки свёрнутого состояния искали порядок
  операторов в исходнике, тогда как рискованная половина здесь — тайминг, которого текст не покажет.
  Добавлен настоящий монтаж компонента (`tableNewRowSignal.test.ts`, 5 тестов); проверено, что
  падает — замена начального нуля на входящее значение роняет три из пяти.
- **Что ворота не проверяют, названо явно:** половина «под `:hover`» утверждается по тексту стиля и
  помечена в тесте как проверка исходника — синтетическое событие `:hover` не даёт. Остаётся одна
  ручная проверка в OBStests: свернуть `data-table`, нажать «добавить запись» в заголовке (развернуть
  → доскроллить → фокус → заметка появилась в папке проекта), затем сузить виджет на сетке при
  широком окне.

- **Лишняя остановка Tab у `WidgetShell` не тронута и вот почему:** `accessibility.ts` даёт виджету
  `tabindex` намеренно — это roving tabindex сетки, где ровно одна ячейка фокусируема. Снять её
  значит тронуть клавиатурную модель сетки целиком; это не «лишняя остановка», а часть контракта.
...
- Status: 🔄 ПОЧТИ — типы, чистые функции и тесты есть; **потребителей нет ни у одного из них**
  (проверено 2026-09-04, подробности в #184), поэтому в продукте возможности пока нет. Создание
  выборки и подключение потребителя — #184 | Milestone: M-MATRYOSHKA | Priority: **P1** | Complexity: M (решение)
...
- **РЕШЕНО 2026-09-03: вариант C — именованные источники в контейнере.** Полное сравнение трёх
  вариантов — `SAVED_SELECTION_BRIEF_159.md`, раздел «Revision 3».
- **Посылка тикета оказалась ложной, и это главное.** И #170, и сам референс утверждали, что уровня
  контейнера у нас нет. **Проверено по коду 2026-09-03: он есть и поставлен** —
  `ProjectDefinition.additionalSources` (`v3/settings.ts:239`), разрешается в
  `DataFrameProvider.svelte:75-85`, редактируется в `CreateProject.svelte`. У источников нет не
  существования, а **имени**: `{ kind, config }` без `id` и `name`. Больше того,
  `NativeQueryDataSource` (#048) — это уже сохранённая выборка, хранимая как источник, безымянная.
  Утверждение референса было помечено `[вывод]`, а не `[код]`; поправка внесена в сам референс,
  иначе следующий читатель заново выведет вариант B.
- **Решение пользователя от 2026-08-27 переживает по существу и не переживает по форме.** Требование
  было — выборка должна быть **адресуемой**: с именем, открываемой как источник другого вида.
  Вариант C даёт ровно эту фразу. Не переживает невысказанная посылка «значит, нужна четвёртая
  сущность»: она стояла на том, что у проекта один источник, а их N.
- **Цена переворачивается.** B оценивался как «одна новая сущность против того, что ей негде жить».
  Настоящий счёт: B ставит **второй список производителей записей** рядом с уже поставленным,
  который удалить нельзя, и требует размеченного объединения на `WidgetSourceConfig` с обязательным
  нормализатором — иначе #136 заново, тихий показ чужого проекта. C добавляет два необязательных
  поля и **не мигрирует ничего**.
- **Ничего аналитического не выброшено:** ответы ревизии 2 о том, *когда* вычисляется membership —
  обогащённый кадр на оси A, verify-after-write, три состояния, глубокая копия при промоушене —
  переносятся в шаг 2 варианта C целиком.
- **Продуктовое решение, принятое здесь же:** сохранённый фильтр — это **четвёртый вид рядом с
  Folder / Tag / Query**, а не отдельная секция. Отдельная секция восстановила бы в интерфейсе ту
  самую модель «четвёртой сущности», которой хранилище только что избежало.
- **Источники стали адресуемыми на деле, а не только в типах (2026-09-03).** Шаг 1 добавил `id` и
  `name`, но их никто не заполнял — значит адресовать было нечего, и возможность оставалась
  недостижимой. Теперь новый источник получает `id` при создании, имя редактируется, а **при смене
  вида идентичность сохраняется**: блок, указывающий сюда, имеет в виду «второй источник этого
  проекта», и замена папки на тег не делает его другим — выдать новый `id` значило бы молча сломать
  каждый указывающий блок. Пустое имя удаляет ключ, а не хранит пустую строку.
- **Создание выборки из редактора проекта отвергнуто, и это не откладывание.** Модалка проекта
  **не знает полей** — у неё нет ни кадра, ни записей, только определение. Фильтр там не
  отредактировать, а созданная с пустым фильтром выборка равна своему основанию, то есть бесполезна
  и сбивает с толку. Правильное место — **там, где фильтр уже виден и уже сработал**: «сохранить
  этот фильтр как источник» в полосе фильтра вида. Это ровно ответ ревизии 2 (verify-after-write):
  имя даётся фильтру, который пользователь уже видел работающим. Отдельный тикет, поверхность
  меняется — по маршрутизации нужен свой проход.
- **Шаг 2 сделан 2026-09-03: `kind: "derived"` — сохранённый фильтр как источник.**
  - **Он не приобретается, и в этом весь замысел.** Все остальные источники читают хранилище; этот
    сужает записи, которые у проекта уже есть, и разрешается **на оси A потребляющего вида, поверх
    обогащённого кадра**. Разрешать его в слое источников значило бы сделать его фильтром
    приобретения, а такие срабатывают до того, как кадр существует: условие по роллапу — «клиенты,
    у кого последняя сессия давно», собственный пример эссе — не нашло бы **ничего** и молча.
    Это опровержение Gate 0, убившее ревизию 1; **тест закрепляет его, чтобы оно не вернулось как
    оптимизация**.
  - **Три состояния, а не два:** «ничего не найдено» и «сломан» на экране выглядят одинаково —
    пустой таблицей — и требуют противоположных действий от пользователя. Сломанный называет,
    что именно не разрешилось: конфигурацию человек починит, а «0 записей» отправит его смотреть
    данные, то есть не туда. Третье — `pending`: обогащённый кадр приходит после внешних, и пустая
    таблица в этом окне была бы третьей вещью с тем же лицом.
  - **Исключение из приобретения сделано явно**, а не по счастливой случайности: производный
    источник иначе провалился бы в «неразрешимый» и был бы отброшен — работает, пока кто-нибудь не
    добавит ветку по умолчанию.
  - **Компилятор поймал недостающее место показа.** Новый вид сломал `switch` в `Archives.svelte`,
    где источники описываются пользователю. Это система типов за работой: новый хранимый вид обязан
    быть описан везде, где источники показывают.
- **Шаг 1 сделан 2026-09-03, и он действительно не вводит ни одного нового понятия.**
  `DataSourceIdentity` (`id?` / `name?`) на четырёх вариантах источника, `sourceId?` у
  `WidgetSourceConfig`, провайдер держит происхождение рядом со слиянием (`frameParts`), выбор — в
  чистой функции `selectSourceFrame`, а не в компоненте.
- **Обратная совместимость доказана первой и жёстче остального** (`sourceSelection.test.ts`), потому
  что на ней стоит всё решение: вариант C выбран в том числе за «не мигрирует ничего», а это верно
  только если конфиг без `sourceId` по-прежнему даёт слитый кадр. Пустая строка адресом тоже не
  считается, и проект, чьи источники никогда не именовали, продолжает сливаться.
- **Одно решение принято по ходу и записано:** `sourceId`, который больше ничего не называет
  (источник удалили, а блок на него ссылается), даёт **весь проект**, а не пустой кадр. Пустая
  таблица выглядит как фильтр, который ничего не нашёл, и пользователь пошёл бы искать не тот баг;
  `sourceExists` позволяет отличить одно от другого тому, кто спросит.
- **Безымянный источник подписывается тем, что он читает** — путём папки или тегом, а не «Источник 2»:
  это то, что пользователь выбрал, когда добавлял его.
- **Разбито на два шага:** шаг 1 (S) — имена у источников и `sourceId` у блока, **без единого нового
  понятия**; шаг 2 (M) — `kind: "derived"`, сам сохранённый фильтр.
...
### #184 — «Сохранить этот фильтр как источник»: выборка рождается там, где фильтр уже сработал
- Status: ✅ DONE 2026-09-04 — оба шага слиты; прогон в хранилище выполнен частично
  (`UNTESTABLE_FEATURES_2026-09-04.md`), визуальная половина остаётся за пользователем
  | Milestone: M-MATRYOSHKA | Priority: P2 | Complexity: **L** (два шага, потребитель + создание)
- analysis_required: false — решение о МЕСТЕ принято в #170; проход архитектора нужен по
  маршрутизации (2+ модуля и изменение поведения), а не по нерешённому вопросу
- Vision scene: 5 | User outcome: фильтр, который ты уже видел работающим, получает имя и становится источником
- **Почему это отдельный тикет, а не хвост #170.** Хранилище (`kind: "derived"`), разрешение
  (`resolveDerived` над **обогащённым** кадром, три состояния вместо двух) и адресуемость источников
  (`id`/`name`, переживающие смену вида) сделаны и слиты. Не сделано ровно одно: **создать** такую
  выборку. Это меняет поверхность вида, а не хранилище, — по маршрутизации нужен свой проход.
- **Место выбрано и обосновано отрицанием, а не вкусом.** Редактор проекта **не знает полей**: у него
  есть определение, но нет ни кадра, ни записей, поэтому условие там не построить, а выборка,
  созданная с пустым фильтром, равна своему основанию — то есть бесполезна и сбивает с толку.
  Правильное место — **полоса фильтра вида**, где фильтр уже виден и уже что-то отобрал: «сохранить
  этот фильтр как источник». Это же и ответ ревизии 2 брифа (verify-after-write): имя даётся тому,
  что пользователь уже наблюдал работающим.
- **ПОСЫЛКА ЭТОГО ТИКЕТА ОКАЗАЛАСЬ ЛОЖНОЙ, проверено по коду 2026-09-04.** Он был написан так,
  будто осталась «только поверхность». Осталась не только поверхность: **у всего #170 нет ни одного
  потребителя**, и сохранённая выборка сегодня не появилась бы нигде.
  - `resolveDerived` — ноль вызовов в живом пути. Единственное, что подключено из `derivedSource.ts`,
    это `isAcquirable` в `DataFrameProvider.svelte:86`, то есть строка, которая derived-источник
    **исключает**.
  - `selectSourceFrame`, `sourceExists`, `sourceLabel` — ноль вызовов.
  - `frameParts` (`stores/dataframe.ts:80`) пишется в `DataFrameProvider` и **не читается никем**.
  - `WidgetSourceConfig.sourceId` (`Dashboard/types.ts:73`) объявлен и не читается никем. Хуже:
    единственный, кто пишет `sourceConfig`, — `DatabaseCallSettings.handleSourceChange`, и он
    собирает `{ projectId }` заново, то есть **затёр бы** сохранённый `sourceId`. И список там —
    соседние **проекты** (`DashboardCanvas.svelte:99`), а не источники внутри проекта.
  - Ни один UI не создаёт `kind: "derived"`. Единственное упоминание вне ядра — ветка `case
    "derived"` в `Archives.svelte`, добавленная потому, что `switch` перестал быть исчерпывающим.
- **Что это меняет: порядок.** Сначала сохранённая выборка должна что-то **делать**, и только потом
  пользователю дают её создать. Иначе он именует фильтр и получает объект, которого не видно ни в
  одном блоке, — ровно «видимое обещание без эффекта», которое ищет flow-render. Поэтому тикет из
  «M, только поверхность» становится двухшаговым, как и сам #170:
  - **Шаг 1 — потребитель.** Блок умеет смотреть на ОДИН источник проекта: `sourceId` читается,
    `frameParts` читается, `selectSourceFrame` выбирает, `resolveDerived` разрешает derived, а
    состояния `broken` и `pending` рисуются отдельно от `empty`. Плюс выбор источника в панели
    блока — рядом с выбором проекта и не вместо него.
  - **Шаг 2 — создание.** «Сохранить этот фильтр как источник» в полосе фильтра вида.
- **Что действительно готово и переиспользуется:** `sourceLabel` даёт имя источнику без имени,
  `sourceExists` отличает удалённый источник от пустого результата, `resolveDerived` различает
  `ok / empty / broken / pending`. Это чистые функции с тестами (`datasources/__tests__/`) — их не
  надо писать заново, их надо **позвать**.
- **Ловушка, названная заранее:** сохранённая выборка читает **обогащённый** кадр, поэтому имя ей
  дают в тот момент, когда обогащение уже прошло. Кнопка, поставленная на поверхность, которая
  рисуется до обогащения, сохранит фильтр, который затем ничего не найдёт, — и это будет выглядеть
  как «фильтр не работает», а не как «сохранено слишком рано».
- **Шаг 1 сделан 2026-09-04, три коммита.** `namedSource.ts` — первый потребитель, который у #170
  вообще появился. Он не переписывает ни одной чужой функции; он добавляет **порядок**, и порядок
  здесь несущий дважды, причём обе ошибки молчаливые:
  - **derived проверяется РАНЬШЕ `sourceExists`.** Тот смотрит только в `parts`, а derived в `parts`
    не попадает никогда (его исключает `isAcquirable`, и правильно — у него нет запроса к хранилищу).
    Спросить `sourceExists` первым — и каждая сохранённая выборка вечно «сломана», с сообщением не
    про то. Перестановка проверок роняет пять тестов.
  - **Строки берутся из ОБОГАЩЁННОГО кадра, а не из part.** `selectSourceFrame` возвращает то, что
    вернул запрос к хранилищу: ни бэклинков, ни rollup, ни колонок, добавленных видом. Отрисовать
    его — молча лишить блок производных колонок ровно в момент, когда на источник указали. Part даёт
    **идентификаторы**, строки берутся из обогащённого кадра — тем же приёмом, что внутри
    `resolveDerived`.
- **Разрешение стоит в голове оси A**, между `enrich` и собственным фильтром блока: оно решает,
  **про какие записи** виджет, до того как что-либо их сужает. Записано в `FILTER_ORDER_ADR.md`
  **после** подстроки, которую пиннит `R_filterOrder`, а не внутрь неё.
- **Математика кадров уехала в `hostFrames.ts`, и канонический порядок стал доказуемым.** Он пиннился
  поиском подстроки в тексте хоста; это ловит ту однострочную регрессию, ради которой писалось, и
  больше ничего — подстрока не может показать, что перестановка вызовов меняет ответ. Как композиция
  чистых функций может: `hostFrames.test.ts` берёт кадр, где корзина X содержит строку, которую
  фильтр оставляет (10), и строку, которую убирает (100). Сначала фильтр — 10; сначала группировка —
  110, и никакая последующая фильтрация 100 из суммы уже не вынет. Разворот порядка красит три теста
  из семи.
- **Состязательное ревью нашло три вещи, все настоящие** (`codex-reports/CX-ADV-184-step1.md`).
  **Первичный источник проекта не мог быть выбран вообще** — `handleDataSourceChange` пересобирал его
  без `id` во всех четырёх ветках, так что в проекте «первичный + дополнительный» выбирался только
  дополнительный, а свежий проект вдобавок врал подсказкой про «источники без имён». Теперь
  идентичность выдаётся и переживает смену вида. **`sourceId` действует на все типы виджетов**, тогда
  как `projectId` гейтится по типу: разобрано и оставлено как осознанная асимметрия — «из какого
  проекта читать» умеет только многовидовой блок, «про какой из источников этого проекта я» осмысленно
  и для чарта; записано комментарием на самом ключе и закреплено с двух сторон. **У состояния
  `pending` не было ни одного производителя** — провайдер рисует дерево виджетов только внутри
  `{:then}` и пишет оба store одним синхронным шагом, так что хост никогда не работает без кадра.
  Экран был недостижим, а мой тест «покрывал» его вызовом чистой функции с `undefined`, то есть
  доказывал отображение, а не путь на экран. Состояние удалено, `enriched` стал обязательным. Тем же
  разбором опровергнут риск 1 плана («мигание broken при смене проекта»): рендера между двумя `set`
  нет.
- **Шаг 2 сделан 2026-09-04.** «Сохранить как источник» живёт в полосе фильтра вида, и место
  обосновано отрицанием: в редакторе проекта нет ни кадра, ни записей, поэтому условие там не
  построить, а выборка с пустым фильтром равна своему основанию. Здесь фильтр уже виден и уже сузил
  то, что на экране, — это и есть verify-after-write из ревизии 2 брифа.
- **Действие появляется только когда есть что именовать:** не «`conditions.length > 0`», а «есть
  включённое условие» — выключенное не сужает ничего, и пользователь такой фильтр работающим не
  видел. В режиме чтения действия нет вовсе. Полоса не узнаёт, что такое проект: она спрашивает имя
  и испускает событие, запись делает `App` через `settings.updateProject` — тот же путь, которым
  пишет редактор проекта. Пустое имя это отмена, а не безымянный источник. Именование **не трогает**
  сам фильтр, и на это есть отдельный тест: случайное `change` переписало бы фильтр вида как побочный
  эффект сохранения. Компонент монтируется в jest, поэтому все 10 тестов гоняют настоящий, а не текст.
- **D-7 записан в `VISION_DEVIATIONS.md`:** полоса фильтра вида редактирует **необогащённый** кадр,
  поэтому условие по rollup здесь не построить. Хранение и разрешение такого ограничения не имеют —
  ради этого #170 и воевал на Gate 0. Узка только создающая поверхность.
- **Состязательное ревью шага 2: три находки, все верны, ответы разные**
  (`codex-reports/CX-ADV-184-step2.md`). **Notice утверждал успех до того, как запись удалась** —
  настройки пишутся подпиской `void saveData(...).catch(console.error)`, и провал диска молчит.
  Надпись исправлена (описывает наблюдаемое, а не персистентность), а сам механизм вынесен в **#185**:
  чинить его внутри #184 значило бы менять каждый путь записи в приложении под видом тикета про
  фильтры. **Дубликаты имён** делали выборку неразличимой при том, что интерфейс обещает обратное —
  добавлен `sourceNameTaken`, сравнивающий с `sourceLabel` всех источников (выборка «Archive» иначе
  сливалась бы с безымянной папкой, которая так подписана), регистр и пробелы игнорируются, дубликат
  отклоняется с объяснением. **Выборка композируется с фильтром вида-потребителя** (A ∩ B, а не A) —
  наблюдение верное, поведение оставлено и закреплено тестом: дашборд сам является видом, каждый
  виджет на холсте уже рисует кадр вида, и источник, игнорирующий его, был бы единственным элементом,
  показывающим скрытые видом записи.
- **Живой прогон в OBStests 2026-09-04** (`UNTESTABLE_FEATURES_2026-09-04.md`). Плагин собран,
  задеплоен, перезагружен — 10 команд, ассерт §2 пайплайна выполнен. Поставлен стенд `T184`: два
  источника записей, сохранённая выборка и четыре блока, включая один с заведомо битой ссылкой.
  **Что REST действительно доказал, и Jest не мог:** плагин **сам переписал** `data.json` при старте
  (2823 → 3868 байт, порядок ключей изменился, добавлены `dateFormat`/`autosave`), то есть
  конфигурация прошла загрузку, миграцию v3→v4 и сериализацию — и при этом `kind: "derived"` со всей
  вложенной `where` **не отброшен**, `sourceId` уцелел во всех трёх блоках вместе с `projectId: ""`,
  а **битая ссылка `src-deleted` не была молча «починена»**. Последнее важнее всего: нормализация,
  вычищающая неизвестный `sourceId`, сделала бы состояние `broken` недостижимым, и пользователь терял
  бы конфигурацию без единого признака.
- **Чего прогон не проверил, и это не считается сделанным:** сколько строк в каждой таблице, что
  блок с исчезнувшим источником показывает сообщение, а не пустую таблицу, и вся полоса «сохранить
  как источник». REST не отдаёт отрисованное. Чек-лист U1–U16 в отчёте, стенд оставлен в хранилище.
- **Ограничение, названное явно:** UI выбора источника есть только у `database-call` — шестерёнка
  панели открывается только для него. `data-table` **читает** `sourceId`, но выбрать его пока негде.
  Расширение шестерёнки меняет поведение существующего виджета и заслуживает своего прохода.
- **Работа:** шаг 1 — чтение `sourceId` через `selectSourceFrame`/`resolveDerived` над `frameParts`,
  выбор источника в панели блока, отрисовка `broken`/`pending` отдельно от `empty`; шаг 2 — действие
...
- **Найдено при ревью #184, проверено по коду.** `main.ts` подписан на store настроек и пишет
  `void this.saveData(value).catch((err) => console.error(...))`. То есть **любое** изменение проекта,
  вида или виджета считается применённым в момент обновления in-memory store, а провал записи на диск
  не доходит до пользователя никак — только в консоль разработчика.
- **Почему это не косметика.** Всё, что рисует UI после изменения, описывает состояние store, а не
  диска. При неудачной записи пользователь видит свой результат, работает дальше и теряет его при
  следующей перезагрузке, без единого сигнала. Это ровно тот класс «видимое обещание без эффекта»,
  который проект ловит flow-render — но здесь он спрятан в успешном пути.
- **Что уже сделано в #184 и чего недостаточно.** Notice после сохранения выборки переформулирован
  так, чтобы утверждать только наблюдаемое («источник теперь в проекте — выберите его в настройках
  блока»), а не «сохранено». Это честно, но лечит одну надпись, а не механизм.
- **ПРОХОД АРХИТЕКТОРА НАШЁЛ ВТОРОЙ ДЕФЕКТ ТОГО ЖЕ МЕХАНИЗМА, И ОН ТЕРЯЕТ ДАННЫЕ СЕЙЧАС.**
  Проверено мной по коду: `settings.subscribe` в `main.ts:344` — Svelte-store, он вызывает колбэк
  **немедленно текущим значением**, а `loadSettings()` отработал раньше (`:69`) и между ними store не
  мутируется. Значит первое срабатывание всегда пишет на диск эхо загруженного значения.
  В пути порчи настроек это **уничтожение улики**: `loadSettings` пишет судебную копию
  `__broken_backup*` поверх дефолтов, показывает Notice «original payload backed up inside data.json
  under `__broken_backup`» и делает `settings.set(DEFAULT_SETTINGS)`. Подписка тут же вызывает
  `saveData(DEFAULT_SETTINGS)` — **без** этих ключей, а `saveData` пишет файл целиком. К моменту,
  когда пользователь туда посмотрит, копии уже нет. Приоритет поднят до P1.
- **И ловушка при починке:** та же эхо-запись — единственное, что сегодня сохраняет на диск результат
  миграции v1→v4. «Просто пропустить первое срабатывание» выглядит верной однострочной правкой и молча
  перестаёт сохранять миграцию. Эхо снимается только вместе с явной записью.
- **Механизм выбран:** один писатель с коалесингом, статус наружу отдельным store, **состояние не
  откатывается**. `settings.*` промисов не возвращают — под коалесингом промис отдельного вызова не
  соответствует ни одной операции записи, то есть был бы тем же враньём, что Notice «Saved …».
- **Что не обещается, прямым текстом:** если пользователь закроет Obsidian, пока запись падает,
  изменение теряется, и #185 этого не предотвращает. Он даёт другое — гарантию, что пользователь был
  предупреждён непропадающей пометкой **прежде**, чем закрыл.
- **Работа:** довести результат записи до вызывающего (промис или store состояния сохранения),
...
- analysis_required: false
- Vision scene: none — корректность | User outcome: модалка открывается тогда, когда ты её попросил, и только тогда
- **Симптом, увиденный пользователем:** «При любом переходе между проектами и вкладками постоянно
  открывается это окно» — модалка «Создать новое поле». Воспроизводится после **одного** нажатия
  «добавить поле» и дальше преследует пользователя по всему приложению.
- **Причина, точная.** `commandBus` — это `writable`, хранящий **последнее** сообщение и никогда его
  не гасящий, а Svelte-store отдаёт текущее значение каждому новому подписчику немедленно.
  `subscribeCanvasCommands` заводил гейт как `lastCommandTs = 0`, поэтому это переигранное значение
  проходило как свежая команда. `DashboardCanvas` монтируется при каждой смене проекта и вкладки —
  отсюда «постоянно».
- **Исправление в одну строку:** гейт стартует с текущего значения шины
  (`get(commandBus)?.ts ?? 0`). Команда, отправленная до того, как ты подписался, не твоя — она уже
  доставлена тому, кому предназначалась.
- **Тест, которого не было.** Файл вынесен из `DashboardCanvas` со словами «to allow isolated unit
  testing» и не имел ни одного теста. Теперь их шесть, и два падают на прежнем коде.
- **Класс ошибки, а не случай.** Это то же «значение при монтаже принято за событие», что и в #169 с
  `openSignal` — только там оно было **нужно** (свёрнутый виджет, сигнал приходит до монтажа) и
  поэтому гасится при сворачивании. Разница между двумя случаями — гасит ли отправитель своё
  сообщение. Стоит помнить при следующем сигнале через store.

### #187 — Группировка по полю: недочёты оформления групп
- Status: ✅ DONE 2026-09-07 — минимальный шаг спецификации сделан | Milestone: (next) | Priority: P3 | Complexity: S
- **Сделано:** группа без значения получила подпись-запаску вместо пустоты, пониженной
  насыщенности — это подпись отсутствия, а не значение. Заодно `var(--font-medium, 500)`
  заменено на существующий токен: запасное значение подставлялось вместо переменной Obsidian,
  которой нет.
- **Локали:** `en` и `ru` получили ключ; в `uk` и `zh-CN` блока `table-v2` вообще нет — они
  отстают на 139 ключей и живут на `defaultValue`. Класть ключ в отсутствующий блок значило бы
  чинить не тот дефект.
- **Отложено спецификацией и не сделано:** липкий вертикальный заголовок и более высокая строка
  заголовка (§3c) — они меняют позиционирование в общем контейнере прокрутки и требуют прохода
  архитектора.
- **Не установлено воротами:** как это выглядит в живой таблице с реальными пустыми значениями.
- **Механизм найден, а не описано впечатление** (проход `designer` 2026-09-05):
  `groupRows.ts:209-215` отдаёт пустую строку для записей без значения в поле группировки, и эта
  пустота доезжает до `TableGroupSection.svelte:21` без запасной подписи. Заголовок такой группы —
  шеврон, пустота и счётчик, то есть ровно та «пустая строка-разделитель без заголовка»,
  которую видел пользователь. Семантический режим (`:131-168`) это уже решает подписью-запаской;
  обычный режим — нет.
- **Скриншота с группировкой в папке не оказалось** — два имеющихся снимка показывают другое.
  Спецификация построена по коду, и совпадение с жалобой подтверждается механизмом, а не снимком.
- **Решено здесь, чтобы не блокировать реализацию:** подпись-запаска — «Без значения»
  (en `No value`, uk `Без значення`, zh-CN `无值`), той же формы, что уже живёт в семантическом режиме.
  Это обратимо и меняется одной строкой в локалях, если пользователь захочет другое слово.
- analysis_required: false — нужен проход `designer`, а не разбор
- Vision scene: 3 | User outcome: сгруппированная таблица читается как сгруппированная
- **Наблюдение пользователя** (скриншот 2026-09-04 10:29): «группировка по полю вызывает некоторые
  недочёты дизайна группировки». На снимке между группами стоит пустая строка-разделитель без
  заголовка группы, из-за чего непонятно, где кончается одна группа и начинается другая.
- Поверхность: `TableGroupSection.svelte` (потолок R0.6 = 100 строк). Конкретику собирает `designer`;
  здесь зафиксировано только то, что видел пользователь, чтобы наблюдение не потерялось.

### #188 — «Фильтровать по полю» пусто и не объясняет, что делать дальше
- Status: ✅ DONE 2026-09-04 | Milestone: (next) | Priority: P3 | Complexity: S
- analysis_required: false
- Vision scene: 4 | User outcome: если выбрать нечего, тебе говорят не только почему, но и что сделать
- **Наблюдение пользователя** (скриншоты 10:32, 10:35): пометка «не даёт выбрать» у селекта
  «Фильтровать по полю». Рядом уже есть объяснение — «No Relation field points at the linked
  block's project. Add a Relation field in the schema editor first» (это #151, он и добавлял текст).
  То есть **сообщение есть и оно верное**, но пользователь всё равно прочитал ситуацию как «сломано».
- **Что здесь на самом деле не так:** объяснение на английском в русском интерфейсе (ключа в локали
  нет, работает `defaultValue`), и оно называет действие, но не даёт до него дойти — редактор схемы
  открывается отдельно. Плюс над таблицей висит второе предупреждение «Relation '' is broken
  (missing-relation)», говорящее о том же другими словами.
- **Сделано 2026-09-04.** Три вещи, и первая важнее остальных.
- **Подсказка теперь заканчивается там, где делается работа.** Она называла действие («добавьте
  поле-связь в редакторе схемы») и оставляла пользователя искать редактор самому. Рядом появилась
  кнопка «Открыть редактор схемы», и она идёт через ту же шину команд, что и палитра, — то есть
  способ открыть редактор один, а не два.
- **«Не настроено» отделено от «сломано».** Пользователь видел `Relation '' is broken
  (missing-relation)` для связи, которую никогда не настраивал: пустое имя в кавычках, технический код
  причины и слово «сломано». Теперь у невыбранного поля своя ветка и своя фраза, и она стоит **раньше**
  ветки про поломку — иначе та снова её поглотит.
- **Ключи заведены в локали.** `relation-missing-hint` и `relation-open-schema` есть в `en` и `ru`.
  **Записан и пробел:** в `ru` вообще отсутствует блок `filter-label` — все эти строки работают на
  `defaultValue`, как и несколько других. Это не недосмотр тикета и им не чинится; закреплено тестом,
  чтобы пробел был виден #140, а не открывался заново.
- **Ограничение, названное честно:** утверждения проверяются по тексту исходника — панель настроек и
  блок нельзя смонтировать в jest (`DatabaseCallBlock` замыкает цикл импортов через `BoardView`).
  Так и написано в шапке теста.

### #189 — Обычный клик по записи открывает ШАПКУ, а не заметку. Это наше изменение, и пользователь его отверг
- Status: ✅ DONE 2026-09-04 (ветка `feat/189-record-open-choice`) | Milestone: (next) | Priority: **P1** | Complexity: M
- analysis_required: false — решение принято пользователем: сделать отдельной опцией
- Vision scene: 3 | User outcome: клик по записи открывает запись; просмотр полей — отдельное осознанное действие
- **Формулировка пользователя:** «При открытии заметки открывается просмотр фронтматтера в боковом
  окне, это должна быть отдельная опция; просмотр фронтматтера (шапки) не есть просмотр заметки».
- **Это регресс от нашей же работы, и он однострочный.** `PLAIN_MODE` в
  `src/lib/record/openRecord.ts` был перевёрнут с `"same"` на `"peek"` 2026-09-03 (шаг b контракта
  открытия записи). Вся смена поведения — эта строка, что и было её целью: миграция сначала, флип
  потом. Значит и возврат — тоже одна строка.
- **Но возврат «в лоб» неправильный.** Панель просмотра полей (`recordPeek`) сама по себе полезна;
  ошибка в том, что она заняла **умолчание** и вытеснила открытие заметки, которое пользователь
  считает основным действием. Нужны оба, и различимо.
- **РЕШЕНО пользователем 2026-09-04: «в модификатор, но отдельным пунктом».** То есть:
  1. `PLAIN_MODE` возвращается к открытию **заметки** — обычный клик снова открывает запись;
  2. просмотр полей вешается на **модификатор** (а не на умолчание);
  3. и одновременно получает **отдельный пункт** — явную строку в меню записи, чтобы действие было
     обнаружимым, а не только «если знаешь сочетание».
  Настройка «что считать основным» **не делается**: пользователь выбрал модификатор, а не опцию, и
  добавлять третий способ настроить одно и то же значило бы усложнять там, где решение уже принято.
- **Ограничение, которое надо соблюсти:** три поверхности уже согласованно трактуют модификаторы
  (`EditNote`, `AllDayEventStrip`, `MultiDayEventStrip`): `shift → window`, `ctrl/meta → tab`. Оба
  занятых модификатора трогать нельзя, значит просмотру полей достаётся свободный — и выбор надо
  обосновать, а не взять первый попавшийся.
- **Сделано 2026-09-04 через протокол** (`implementer` → `auditor`, отчёт `codex-reports/CX-AUDIT-189.md`).
- **Модификатор — `alt`, и это не «первый свободный».** `ctrl+shift` занят самим Obsidian (открытие
  ссылки в сплите) — брать его значило бы спорить с хостом на его же жесте. `alt` в `src/` встречается
  ровно дважды, оба раза на `WheelEvent` в календарях (проверено). Проверяется **после** `shift` и
  `ctrl`: на европейских раскладках AltGr сообщает `ctrlKey` и `altKey` вместе, и это обязано остаться
  «открыть во вкладке», а не стать просмотром. И порядок, и причина закреплены тестом.
- **Ошибка в пакете, которую исполнитель не стал обходить:** модификатор в `modeFromEvent` был бы
  мёртвым кодом — `TableRow` диспатчила `openRecord` без события, а `DataTableContent` звала
  `PLAIN_MODE` напрямую. Строка теперь передаёт `MouseEvent`. Два лишних файла, и обязательных.
- **Аудит вернул BLOCKED, и по делу:** на readonly-строке кнопки `⋯` не было вовсе (`{#if !readonly}`),
  то есть обещанный пункт там недостижим, а `alt` остаётся скрытым способом — и именно там, где
  просмотр полей нужнее всего. Меню теперь доступно, но `Дублировать`/`Удалить` **отсутствуют, а не
  погашены**: погашенный «Удалить» всё равно сообщает, что строку можно удалить, а для заимствованного
  источника это неправда.
- **Вторая находка аудита отклонена с проверкой** (числа ворот в сообщениях коммитов): R0.7 сканирует
  `.claude`/`.codex`/`.github` и два файла правил, сообщения коммитов туда не входят; правило защищает
  от дрейфа конфигурации, а историческая запись разойтись с будущим измерением не может. Предложенное
  «переписать сообщения» — переписывание истории, инвариант 11.
- **РЕШЕНО пользователем 2026-09-05: так и задумано.** Просмотр полей — возможность **таблицы
  дашборда**, а не общее свойство открытия записи. Состязательное ревью справедливо заметило, что
  «назвать асимметрию» — не то же самое, что решить; решение принято, и формулировки приведены к нему,
  чтобы «просмотр полей» нигде не читался как обещание для всех поверхностей.
- **Асимметрия, названная заранее:** просмотр достижим только в таблице дашборда. Gallery, Board,
  Calendar и EditNote снова корректно открывают заметку, но не передают событие, поэтому `alt` там
  ничего не делает и пункта меню нет. Это соответствует поверхности тикета; распространять ли —
  отдельное решение, а не недосмотр.
- **Не сделано:** живого прогона не было. Ни `alt`-клик, ни кнопка `⋯` на readonly-строке, ни
  положение панели в Obsidian не видены (последнее — #190).

### #190 — Панель просмотра уезжает под шапку плагина
- Status: ✅ DONE 2026-09-04 — слито в `main` (`b6e090a`), аудит `CX-AUDIT-190.md`; четыре
  ворот зелёные, живого прогона НЕ было | Milestone: (next) | Priority: P2 | Complexity: S
- analysis_required: false — нужен проход `designer`
- Vision scene: 3 | User outcome: панель не перекрывается тем, поверх чего она открылась
- **Формулировка пользователя:** «Само открытие окна немного некорректно располагается относительно
  шапки плагина, уходя под неё без смещения».
- Видно на скриншотах 2026-09-04 (панель записи справа, её заголовок вплотную к шапке вида).
- **Чего flow-auditor доказать не смог, и это записано честно:** геометрия перекрытия из кода не
  выводится. Роль вернула гипотезу о поверхности и её монтаже (`View.svelte`, `RecordCardView`), но
  сама сказала, что реальный рендер, перекрытие шапки и адаптивность кодом не доказываются.
  Поэтому здесь нужен замер, а не рассуждение — `renderProbe` или снимок с линейкой.
- **Замер сделан.** `SPEC_PEEK_ANCHORING_2026-09-04.md` (дизайн) и
  `PLAN_190_PEEK_ANCHORING_2026-09-04.md` (исполнение); сьют
  `src/__tests__/A190_peekAnchoring.acceptance.test.ts` (13 утверждений). Механизм: строка сетки, а
  не вычисленное смещение — `.projects-container` стала гридом, слой `.ppp-app-overlay` делит вторую
  строку с `.projects-main`, узлы панели переезжают в него действием `portalToOverlay`. Ни одной
  длины, выражающей высоту шапки. Найденный по дороге третий дефект (в разделённой области панель
  рисовалась поверх соседнего листа и хрома Obsidian) закрыт тем же механизмом — исчезновением
  `position: fixed`.
- **Что доказано измерением:** верхний край панели = нижний край шапки при ДВУХ разных высотах шапки
  (один прогон прошёл бы и при жёстком смещении); ширина панели = ширина контейнера в узком
  контейнере (сегодняшний `100vw` даёт 448 против 300 — этот тест и был красным до правки);
  подложка кончается там же, где панель; `z-index: 100` внутри вида не рисуется поверх панели;
  `position: fixed` в компоненте не осталось.
- **Чего проверить нельзя, и это не смягчается:**
  1. **Хром Obsidian не перекрыт** — страница пробы содержит только поддерево плагина. Механизм
     доказан, снимка нет.
  2. **Разделённая область** — дефект с наибольшим радиусом; нужен человек с двумя листами.
  3. **Что цель портала находится при любом монтировании** — статически недоказуемо; смягчено
     веткой отказа, которая гарантирует «ниже шапки и шириной с контейнер», но НЕ «неподвижна при
     прокрутке».
  4. **Obsidian mobile (WebKit)** — проба говорит только за Chromium; `overflow: clip` здесь самое
     молодое свойство.
  5. **Что это выглядит правильно** с темой или сниппетом.
  6. **Порядок слоя против `SettingsMenuPopover`** — оба объявляют `--ppp-z-overlay`, решает порядок
     в DOM; попап рендерится по условию, поэтому ни один тест этого не видит. Требование записано
     комментарием в `App.svelte`.
- **Оставлено сознательно, отдельными тикетами:** сырой `z-index: 90` у `.compact-navbar` (инертен
  после перехода в сетку); модальность панели (ловушка фокуса + `aria-modal` + подложка меняются
  только вместе); консолидация пяти порталов; два листа показывают одну запись — поведение не
  менялось.

### #191 — Шаблоны: два разных механизма под одним словом, и мёртвая ветка внутри
- Status: ✅ DONE 2026-09-04 (ветка `feat/191-drop-dashboard-templates`) | Milestone: (next) | Priority: P3 | Complexity: M
- analysis_required: false — карта использования уже снята (`codex-reports/CX-MAP-191.md`)
- Vision scene: none — сопровождаемость | User outcome: в интерфейсе нет поверхностей, которые ничего не делают
- **Формулировка пользователя:** «есть устаревшие артефакты, такие как например шаблоны, функционал
  которых и потребность совершенно устарели».
- **Карта (`code-mapper`, 2026-09-04) говорит: «шаблоны» — это ДВА несвязанных механизма**, и путать
  их при удалении опасно:
  1. **Шаблоны записи** — `ProjectDefinition.templates` (пути к .md) → выбор в `CreateNote.svelte`
     → `DataApi.createNote` читает файл, подставляет `title`/`date`/`time` и создаёт заметку.
     **Живой путь целиком**, у него есть потребитель и эффект.
  2. **Шаблоны дашборда** — раскладки виджетов в коде (`widgetTemplates.ts`) → `dashboardTemplates.ts`
     → пишут `DatabaseViewConfig.widgets`. Тоже живые, плюс достижимы через сохранённое быстрое
     действие `kind: "apply-template"` по `templateId`.
- **Что действительно мертво:** `getVerticalTemplates` (`widgetTemplates.ts:559`) — ноль вызовов и
  импортов. Это можно удалять по образцу #178/#183 (карта + сравнение бандла).
- **Чего карта НЕ смогла установить:** какие `templateId` реально лежат в хранилищах пользователей —
  быстрые действия хранят их строкой, статический разбор их не перечислит. Значит удаление шаблонов
  дашборда без проверки хранилища сломает чью-то кнопку молча.
- **РЕШЕНО пользователем 2026-09-04: лишние — шаблоны ДАШБОРДА.** Шаблоны записи
  (`ProjectDefinition.templates` → содержимое новой заметки) остаются: это работающий путь, которым
  пользуются.
- **Что это значит по объёму, из карты:** удаляются раскладки виджетов в коде (`widgetTemplates.ts`),
  контроллер (`dashboardTemplates.ts`) и три поверхности, которые их предлагают —
  `WidgetGrid.svelte:70` (пустой холст), `WidgetToolbar.svelte:26`, `DashboardCanvas.svelte:134`.
- **Ловушка, названная картой заранее и требующая проверки хранилища:** быстрые действия могут
  хранить `kind: "apply-template"` с `templateId` строкой (`dashboardTemplates.ts:80,85`). Статический
  разбор их не перечислит. Значит перед удалением надо **проверить настоящие хранилища** и решить,
  что делать с уже сохранённой кнопкой: удалять её вместе с механизмом или оставлять неработающей
  (второе — недопустимо, это как раз «видимое обещание без эффекта»).
- **Сделано 2026-09-04.** План: `PLAN_191_DASHBOARD_TEMPLATES_2026-09-04.md`, архив:
  `archive/DASHBOARD_WIDGET_TEMPLATES.md` (три файла дословно рядом).
- **Проверка хранилищ выполнена и подтвердила худшее из карты:** `migrateTableConfig` **писала**
  кнопку `qa-overview` в каждый мигрируемый дашборд, и она лежит в настоящем хранилище пользователя.
  То есть это было не удаление мёртвого кода, а изменение хранимых данных.
- **Кнопка отбрасывается при чтении**, а не остаётся отключённой: отключённая сообщает, что раньше
  что-то было, и не даёт ничего сделать — тот же дефект в вежливой обёртке. `data.json` чистится сам,
  когда вид сохраняется по уже существующему пути. Фильтр по `kind`, а не по списку известных
  `templateId`: карта записала, что статический разбор идентификаторы из чужих хранилищ не перечислит.
- **Полноту удаления обеспечил компилятор:** `ApplyTemplateQuickAction` вышел из союза, и
  `migration.ts` не собирался, пока из него не убрали генерацию артефакта. Дешевле и строже текстового
  ратчета.
- **Уточнение плана, найденное реализацией.** План ставил фильтр только в миграцию. Тест на
  смонтированном холсте показал, что этого мало: после удаления диспетчера по `kind` **любая** кнопка
  запускала бы единственное оставшееся действие, то есть «Обзорный пресет» переключал бы панель
  формул. Фильтр нужен в обоих местах: миграция чистит файл, компонент решает, что рисовать.
- **Своя ошибка, пойманная проверкой diff:** первый проход по локалям снёс блок шаблонов **записи**
  («Templates to choose from when you create new notes») — ровно то, что решение пользователя
  исключает. Откачено, скрипт переписан на различение двух блоков по содержимому.
- **Числа перемерены, а не взяты из плана:** `PX_BUDGET` 151 → **143** (план оценивал 149),
  `REM_IN_CONTAINER_BUDGET` 807 → **804** (совпало), потолок `DashboardCanvas` 200 → **195**.
  `main.js` 2 820 099 → **2 789 438** (−30 661 байт): в отличие от #178/#183 это был runtime-код, и
  равенство байтов здесь было бы плохим знаком.
- **Ослабление ратчета названо прямо:** `TemplateConfirmDialog` был единственным входом
  `WINDOW_ANCHORED` внутри области R0.16, и на нём держалось доказательство, что список работает.
  Замену искать нельзя — оставшиеся кандидаты исключены докстрингом самого ратчета. Утверждение
  сужено до правды: внутри области освобождённых нет.
- **База тестов упала на один сьют** (`widgetTemplates.test.ts` — тест предмета, которого больше нет)
  и на три теста подтверждения замены; при этом добавлено шесть на `dropTemplateQuickActions` и один
  рендерный. Итог 207 → **206 сьютов**, 3348 → **3336 тестов**. Прецедент #160/#183: тест, чей предмет
  удалён, — не покрытие, и падение называется вслух.
- **Что осталось невидимым для ворот:** живого прогона не было. Надо увидеть в Obsidian, что
  «Обзорный пресет» исчез, «Formula Builder» работает, пустой холст предлагает «Добавить блок данных»,
  и что `data.json` почистился после открытия и сохранения вида.

### #192 — Пять порталов делают одно и то же по-разному
- Status: ✅ DONE 2026-09-07 — шаги 0–5, слито в `main` | Milestone: (next) | Priority: **P1** (был P3) | Complexity: M
- **Дефект оказался четырёхслойным, и каждый слой находился только после закрытия предыдущего:**
  (1) узел уезжал в главное окно; (2) слушатели и измерения остались там же — попап в оторванном
  окне нельзя было закрыть ни Escape, ни кликом вне; (3) привязка делалась один раз и не
  переезжала, хотя оба компонента монтируются раньше, чем становится известен их документ;
  (4) захваченная блокировка прокрутки тоже принадлежит документу — узкое отдельное окно
  показывало лист с незаблокированной прокруткой, а главное оставалось заблокированным.
- **Ни один слой не был виден тестам:** в jsdom окно одно. Отсюда правило, закреплённое
  `A192_popoutBinding.test.ts`: компонент, способный оказаться в другом окне, не держит ничего
  глобального — ни слушателей, ни измерений, ни захваченных стилей чужого документа.
- **S8 измерен, а не выведен:** Svelte 3.59.2 отсоединяет элемент ДО вызова `destroy` действия,
  поэтому ветка восстановления в `AgendaSidebar` была мёртвым кодом и удалена со ссылкой на замер.
- **Отклонение от плана, названное явно:** `DayPopup` не переведён на примитив. Он не переносит
  узел в существующую цель, а создаёт слой и владеет им; примитив стал бы менеджером слоёв ради
  одного вызывающего. Общий урок применён: документ берётся у узла.
- **Осталось отдельным тикетом:** `FilterRow` — три самодельных поповера, которые строят DOM
  вручную мимо Svelte. Их починка — переезд на общий попап, другая приёмочная поверхность.
- **Не установлено воротами:** прогон в оторванном окне Obsidian. Ни один тест не поднимает
  второе окно.
- **Приоритет поднят: внутри долга лежит дефект.** Проход архитектора 2026-09-05:
  `FloatingPopup.svelte:258` берёт `document`, а не документ своего узла. В оторванном окне Obsidian
  попап уезжает в главное окно, позиционируется по чужому вьюпорту (`:114-124`) и не закрывается
  кликом вне (`:89-92` слушает чужой документ). Потребителей у компонента тринадцать.
  `CONTRIBUTING.md:85` уже запрещает `document.*` — правило было, проверки не было.
- **Гипотеза о фиксе была неверной, и это важнее самого дефекта:** `activeDocument` — документ
  С ФОКУСОМ (`obsidian.d.ts:255-264`), поэтому компонент, монтирующийся пока фокус в другом окне,
  уедет туда же. Верная цель — `node.ownerDocument`, и так уже делает ровно одно место:
  `AgendaSidebar.svelte:496`.
- **«Пять» в заголовке — недосчёт.** Шесть действий и девять мест вызова;
  `HeaderStripsSection` и `TimelineView` — одна и та же функция дважды, вместе с комментарием.
  `FilterRow` вынесен из тикета: он не переносит чужой узел, а строит свой DOM и владеет
  им целиком — это отдельный тикет.
- **Имя примитива задано ратчетом, а не вкусом:** исключение R0.16 для `FloatingPopup`
  срабатывает на слове `use:portal`, так что действие должно называться `portal` и жить в
  `src/ui/portal.ts`.
- analysis_required: false
- Vision scene: none — сопровождаемость | User outcome: перенос узла ведёт себя одинаково везде
- **Найдено ревью решений** (`codex-reports/CX-ADV-DAY-2026-09-04.md`, п.4). В дереве пять действий,
  переносящих DOM-узел: `FloatingPopup:257`, `HeaderStripsSection:29`, `TimelineView:20`,
  `AgendaSidebar:494` (все — в `document.body`) и `overlayPortal.ts` из #190 (в слой своего листа).
  Каждое повторяет жизненный цикл и очистку по-своему.
- **Почему это не сделано в #190:** консолидация переписывает три календарных компонента ради панели
  и смешивает рефакторинг с исправлением геометрии. Новый портал намеренно повторяет форму
  `FloatingPopup`, чтобы объединение было механическим.
- **Работа:** один примитив со стратегией цели (`body` / ближайший слой листа), пять вызовов на него.

### #193 — «Which source» обещает уровень проекта, а даёт пересечение с фильтром вида
- Status: ✅ DONE 2026-09-05 — вместе с #194 | Milestone: M-MATRYOSHKA | Priority: P3 | Complexity: S
- analysis_required: false — поведение подтверждено решением #184, меняется только копирайт
- Vision scene: 5 | User outcome: подпись говорит то, что происходит
- **Поведение верное и оставлено намеренно:** дашборд сам является видом, каждый виджет уже рисует его
  кадр, и источник, игнорирующий фильтр вида, был бы единственным элементом, показывающим скрытые
  записи. Закреплено тестом в #184.
- **Проблема в слове.** Пикер говорит «Which source» и «All sources of this project» — то есть обещает
  проектный уровень, а даёт пересечение с текущим видом. Честнее сказать «в пределах этого вида».
- **Работа:** копирайт селектора и подсказки в четырёх локалях. Если понадобится настоящая
  переносимость — это другое решение и другой тикет.

### #195 — Судебная копия повреждённых настроек лежит внутри файла, который переписывается целиком
- Status: ✅ DONE 2026-09-05 — аудит `CX-AUDIT-195.md`, починка проверена живым прогоном | Milestone: (next) | Priority: **P1** | Complexity: S
- analysis_required: false
- Vision scene: none — надёжность | User outcome: если настройки испортились, копия старых лежит там, где её не затрёт следующая же запись
- **Как найдено.** Прогон в OBStests 2026-09-05: подложен `data.json` с верным JSON и неверной
  схемой (`projects` строкой вместо массива), Obsidian запущен. После загрузки файл содержит
  дефолты и демо-проект, а ключа `__broken_backup` в нём нет. Уведомление отправляет пользователя
  за копией, которой к этому моменту уже не существует.
- **Механизм, подтверждённый по коду.** `loadSettings` (`main.ts:616-626`) кладёт копию ВНУТРЬ
  `data.json` соседними ключами. `saveData` пишет файл целиком, поэтому любая последующая штатная
  запись стирает копию. #185 закрыл один маршрут такой записи — эхо подписки, — но не все:
  после порчи список проектов пуст, вид предлагает онбординг, `createDemoProject`
  (`demoProject.ts:594`) вызывает `settings.addProject`, и эта запись затирает копию.
- **Почему это не «ещё один маршрут», а неверное место хранения.** Пока копия живёт в том же
  файле, каждая будущая запись — новый способ её потерять, и закрывать их придётся по одному.
  Копия должна лежать в отдельном файле рядом (`data.broken-<ISO>.json` через адаптер хранилища),
  которого штатная запись не касается; уведомление указывает на него по имени.
- **Что подтвердил тот же прогон, и это хорошая новость:** файл с ОБОРВАННЫМ JSON после загрузки
  остался нетронутым. До #185 подписка записала бы поверх него дефолты, то есть уничтожила бы
  единственную копию данных пользователя. Прайминг снял эту запись — проверено на живом
  приложении, а не воротами.
- **Второй факт того же прогона:** оборванный JSON идёт другой веткой, чем схемная порча
  (`loadData` бросает → `main.ts:591-599`), и копии там не делается вовсе. Файл при этом цел,
  но уведомление об этом не говорит, а первое же изменение настроек его перепишет.

- **Сделано 2026-09-05.** Копия уехала из `data.json` в соседний файл `data.broken-<отметка>.json`,
  которого штатная запись не касается; уведомление называет его по имени и ветвится по тому,
  удалась ли запись. Ветка оборванного JSON теперь тоже делает копию — раньше не делала вовсе,
  хотя это самый частый вид порчи. Логика вынесена в `src/lib/settings/brokenBackup.ts` с
  инъекцией адаптера, потому что у `main.ts` в этом дереве нет сьюта.
- **Проверено на живом приложении, а не воротами:** копия появляется отдельным файлом, `data.json`
  остаётся нетронутым, и после штатной записи настроек (создание демо-проекта, переписывающее файл
  целиком) копия цела с исходным payload внутри. Это ровно тот шаг, на котором улика терялась.
- **Аудит нашёл HIGH и он принят:** имя копии выводилось из миллисекундной отметки и писалось без
  проверки, свободно ли оно, при доступном `exists`. Теперь имя проверяется, потолок — десять
  попыток.
- **Остаётся долгом:** уведомления этого пути — английские литералы, тогда как локалей четыре.
  Долг существовал до тикета и им расширен; отдельным пунктом при следующей ревизии текстов.

### #196 — Приёмочный сьют на headless Chrome не стартует под полной параллельной нагрузкой
- Status: ✅ DONE 2026-09-06 — причина измерена, лечение проверено | Milestone: (next) | Priority: P2 | Complexity: S
- **Причина оказалась не той, что предполагалась.** Не тайм-аут сам по себе: пять приёмочных
  сьютов поднимают headless Chrome, Jest запускает их параллельно, и каждому нужны сотни мегабайт.
  Замерено в сессии 2026-09-06: свободной памяти около полутора гигабайт, при этом браузер
  пользователя держит гигабайт. Те же три сьюта с `--runInBand` прошли полностью.
- **Лечение:** запуски браузера сериализованы файловой блокировкой между воркерами (`renderProbe.ts`),
  повтор при тайм-ауте оставлен на случай, когда и сериализованный запуск голодает. Два
  предохранителя: брошенная блокировка стареет и снимается, ожидание сверх срока всё равно
  запускает браузер — застрявшая блокировка не должна стоить дороже той конкуренции, от которой
  защищает.
- **Проверено:** пять проб в одном параллельном прогоне — зелено, затем полный прогон вместе с
  ними — зелено.
- analysis_required: false
- Vision scene: none — доверие к воротам | User outcome: зелёные ворота значат «прошло», а не «повезло»
- **Наблюдение, дважды.** `A190_peekAnchoring.acceptance.test.ts` дважды за день не стартовал
  (`Test suite failed to run`) под полным `npx jest`, и оба раза проходил при одиночном запуске
  (все свои проверки) и при повторном полном прогоне. Он поднимает headless Chrome.
- **Почему это не «просто флейк».** Ворота — единственное, что отделяет «сделано» от «кажется».
  Сьют, который падает по причине, не связанной с его предметом, учит игнорировать красный цвет,
  а это ровно та привычка, из-за которой пропускают настоящие падения.
- **Работа:** выяснить, чего именно не хватает под нагрузкой (порт, процессы, память), и либо
  сериализовать такие сьюты, либо дать пробе повтор с явной причиной в сообщении. Прогон в
  одиночку и в полном наборе должен давать один и тот же ответ.

### #197 — Проекты уходят из шапки в настройки целиком
- Status: ✅ DONE 2026-09-06 — слито в `main`, инвариант `navbarHasNoProjects.test.ts` | Milestone: (next) | Priority: P2 | Complexity: S
- analysis_required: false — решение принято, нужен план удаления, а не разбор
- Vision scene: none — функциональный минимализм | User outcome: шапка показывает виды и действия, а проектами управляешь там же, где и всеми остальными настройками
- **РЕШЕНИЕ ПОЛЬЗОВАТЕЛЯ (визуальный прогон 2026-09-06, четыре скриншота).** Дословно: «из шапки
  нам необходимо убрать излишнее отображение проектов, а текущий дизайн можно полностью
  переместить в настройки, где мы в общем-то и так управляем всеми настройками». Ориентир —
  функциональный минимализм.
- **Наблюдение пользователя, объясняющее половину проблемы:** «чипы проектов появляются только
  после того, как их становится больше одного». Так и есть по коду: `CompactNavBar.svelte:62`
  показывает селектор при `projects.length > 1`, иначе (`:71`) — статичную надпись с именем. То
  есть шапка меняет форму в зависимости от количества проектов, и ни одна из двух форм не нужна.
- **Дубль уже существует, и он полнее.** `SettingsMenu/tabs/ProjectTab.svelte` — это listbox со
  выбором активного проекта (`:37`), правкой (`:44`), удалением (`:52`) и созданием (`:63`).
  Шапка умеет только выбрать. То есть переносить нечего: надо удалить, а не перенести.
- **Работа:** убрать из `CompactNavBar` проектный блок целиком (триггер, статичную надпись,
  попап-селектор и хардкод `hideProjectName === "Демо-проект"` на `:23`), перестать передавать
  `projects` / `projectId` / `on:projectChange` из `App.svelte:338-345`, снять мёртвый ключ
  `nav.project-switcher` из четырёх локалей.
- **Компромисс, названный прямо:** после этого в шапке не видно, в каком проекте ты находишься.
  Активный проект виден в настройках на вкладке «Проекты», и вкладка Obsidian называется
  «Projects». Пользователь выбрал минимализм явно; если контекст «где я» окажется нужен, он
  возвращается одной надписью, а не селектором.

### #198 — Демо-проект сообщает, что часть заметок не записалась, и делает это по-английски
- Status: ✅ DONE 2026-09-06 — причина найдена в консоли пользователя | Milestone: (next) | Priority: P3 | Complexity: S
- **Причина, названная самим хостом:** `demo note could not be created … Overdue: budget review —
  Storefront Nimbus.md Error: File name cannot contain any of the following characters: * " \ / < > : | ?`
  Двоеточие в имени заметки. Obsidian отказывает, заметка не создаётся, и первое, что видит новый
  пользователь, — сообщение о неполном примере.
- **Почему причина нашлась за минуту:** #156 перестал глотать эту ошибку и оставил её в консоли.
  Ровно та ситуация, ради которой он делался.
- **Починено не одной строкой:** имена демо-заметок пишут прозой, а в прозе бывают двоеточия,
  поэтому очистка имени вынесена в `noteName.ts` и применяется ко всему набору. Тест прогоняет
  весь демо-набор через поддельное хранилище, которое отказывает по правилу хоста, — он падает на
  коде до правки.
- **Осталось долгом:** текст самого уведомления английский, тогда как локалей четыре. Это часть
  общего долга уведомлений этого пути (см. #199).
- analysis_required: false
- Vision scene: none — первое впечатление | User outcome: демо-проект либо создаётся целиком, либо честно говорит, чего не хватает, и на языке интерфейса
- **Наблюдение (скриншот 2026-09-06 00:33).** Уведомление: «The demo project was created, but 1
  notes could not be written. See the console for the list.» Два дефекта в одной строке: часть
  демо-контента не создалась (то есть первое, что видит новый пользователь, — неполный пример), и
  текст английский при четырёх локалях в дереве.
- **Работа:** выяснить, какая заметка не пишется и почему (столкновение имён? запрещённый символ?
  уже существующий файл?), и либо чинить запись, либо называть файл в уведомлении. Текст — через
  ключ локали, а не литералом.

### #199 — Успешная запись настроек ничего не значит: хост докладывает об успехе, которого не было
- Status: ✅ DONE 2026-09-06 — механизм #185 подтверждён живым прогоном на честном стенде | Milestone: (next) | Priority: P2 (был P0) | Complexity: M
- **ЧИСТЫЙ ПРОГОН СОСТОЯЛСЯ 2026-09-06 16:20, и он всё расставил по местам.** Запрет через ACL
  (запись, удаление и создание в каталоге), изменение настроек — и появилось ровно то, что обещал
  #185: уведомление на русском и стоящая пометка в шапке. В консоли: `settings write starting`,
  три `Could not read settings back to verify: EPERM` и `settings write attempt 3 failed`.
- **Значит #185 работает, а прошлое «молчание» объяснялось негодным стендом** — атрибут «только
  чтение» хост обходил, и записи просто не было. Проверка чтением при этом отработала честно:
  когда файл недоступен и на чтение, она сообщает о провале, а не выдумывает успех.
- **ПОПРАВКА 2026-09-06, ВАЖНЕЕ САМОГО ТИКЕТА: исходный вывод не подтвердился, потому что
  эксперимент был негодным.** Запись блокировалась атрибутом «только чтение» (`IsReadOnly`).
  Позже выяснилось: файл всё-таки был перезаписан (15:38), и атрибут при этом ИСЧЕЗ. То есть хост
  обходит атрибут, пересоздавая файл, и «запись не прошла» на самом деле означало «запись ещё не
  случилась» — например, потому что изменение не было доведено до сохранения.
- **Что это значит.** Утверждение «`saveData` докладывает об успехе, которого не было» НЕ доказано.
  Оно было выведено из того, что файл не менялся, а ошибок в консоли не было, — и обе половины
  объясняются негодной блокировкой. Приоритет снижен с P0 до P2.
- **Что остаётся верным.** `fs.writeFile` из Node на том же файле бросал `EPERM` — но это Node, не
  Obsidian, и на другом пути записи. Диагностических строк в консоли не было вовсе, и это
  подтверждённый пробел, закрытый в том же изменении.
- **Что сделано и почему оставлено.** После записи файл перечитывается и сравнивается с тем, что
  писали; неподтверждённая запись идёт путём провала. Это страховка, а не исправление
  подтверждённого дефекта: если хост когда-нибудь солжёт, пользователь узнает. Цена — одно чтение
  файла настроек после каждой записи и три исхода вместо двух (см. аудит `CX-AUDIT-199.md`).
  **Решение оставить или откатить — за пользователем**; код обратим одним revert.
- **Чистый эксперимент подготовлен, но не проведён:** запрет через ACL (`icacls /deny` на запись,
  удаление и создание в каталоге) — его хост обойти не может, проверено из Node по всем трём путям.
  Результат ждёт прогона в приложении.
- **Урок, который стоит держать отдельно от вердикта:** негодный эксперимент дал уверенный вывод,
  и этот вывод успел стать тикетом P0, коммитом и записью в отчёте. Проверять надо не только код,
  но и то, что стенд действительно делает то, что заявлено.
- analysis_required: false
- Vision scene: none — надёжность | User outcome: «сохранено» означает, что оно на диске, а не что хост так сказал
- **Как найдено.** Прогон #185 в OBStests 2026-09-06: на `data.json` снят флаг записи, пользователь
  переименовал проект и сделал ещё несколько изменений. Ни уведомления, ни стоящей пометки не
  появилось. Файл на диске при этом не изменился — ни размер, ни время правки.
- **Что показала консоль.** Добавленная диагностика дала `settings write starting` и **ни одной**
  строки `settings write attempt N failed`. То есть писатель начал запись, `saveData` вернул
  успех, и файл не изменился.
- **Что проверено отдельно, чтобы не гадать:** тот же файл, та же машина, `fs.writeFile` из Node
  бросает `EPERM`. Значит отказ на этом пути существует и обнаружим — теряется он внутри хоста.
- **Почему это P0 и почему это шире #185.** #185 строил всю видимость провала на том, что
  неудачная запись отклоняет промис. Живой прогон показал, что она может его не отклонить. Тогда
  вся конструкция — повторы, пометка, уведомление — не срабатывает ни разу, и пользователь теряет
  настройки ровно так же молча, как до #185. Хуже: теперь есть механизм, который выглядит рабочим.
- **Чего не поймал никто, и это стоит записать буквально.** Четыре зелёных ворот, аудит, ревью
  перед слиянием и девять юнит-тестов писателя — все они проверяли `save`, который падает по
  команде. Ни один не мог заметить `save`, который лжёт. Поймал первый же прогон в живом
  приложении.
- **Работа:** не верить результату записи, а проверять его. После успешного `saveData` перечитать
  файл и сравнить с тем, что писали; несовпадение — это провал записи со всеми последствиями
  (повтор, пометка, уведомление). Сравнение должно быть устойчиво к порядку ключей, иначе оно
  само станет источником ложных тревог.

### #200 — Внешнее изменение data.json не подхватывается и теряется при следующей записи
- Status: ✅ DONE 2026-09-08 — все пять шагов; приёмка на живом хосте 10/10 (`ACCEPTANCE_200_RESULTS_2026-09-08.md`) | Milestone: (next) | Priority: P2 | Complexity: M
- **RESOLVED 2026-09-07 (решение пользователя): `uniqueIdCounter` переносится как `max`.** При
  принятии файла с диска счётчик каждого проекта берётся как `max(память, диск)` по `id`, для
  `projects` и `archives`; это единственное сливаемое поле. Причина: счётчик монотонен и питает
  `UniqueId`, которые уже записаны в заметки, — откат назад даёт дубли в хранилище, и копии, из
  которой их вернуть, не существует. Альтернативы (строго целиком; откат счётчика = конфликт)
  рассмотрены и отклонены.
- **Сделано:** план `PLAN_200_SETTINGS_OWNERSHIP_2026-09-07.md` §11a (спайк) и §11b (шаги 1–4).
  `verify` возвращает вердикт вместо булева; `settingsReconcile.ts` — чистая таблица решений;
  `onExternalSettingsChange` проведён; чужая версия уезжает в `data.conflict-*.json`; знак
  `diverged` не предлагает повтор. Коды `PPP-105` и `PPP-106`.
- **Шаг 5 пройден 2026-09-08 на живом хосте** (Obsidian 1.9.12, `OBStests`, внешний писатель —
  отдельный процесс Node). Десять пунктов из десяти; главный — A2: внешнее изменение пережило
  следующую запись плагина, то есть наблюдение спайка инвертировано. Обе правки после
  состязательного ревью подтверждены живым входом: огрызок `{"version": 4}` сохранён копией и
  конфигурация цела (A5), битый файл скопирован через 2,07 с (A6). Ловушка 1.3 снята: хук на
  непарсящийся файл **приходит**.
- **Прогон нашёл три чужих дефекта, ни один не про сохранность данных:** #207 (в консоли печатается
  сырой `{{path}}`), #208 (причины кодов есть только по-английски — 32 из 32 ключей), #209 (пометка
  «Изменён извне» живёт ~200 мс вопреки обещанию быть стоячей).
- **Осталось:** merge в `main` и push — за пользователем.
- analysis_required: true — нужен архитектор: это про владение файлом, а не про одно место в коде
- Vision scene: none — надёжность | User outcome: изменения, сделанные в другом окне или синхронизатором, не пропадают молча
- **Найдено аудитом #199, поведение существовало всегда.** Плагин пишет `data.json` целиком из
  своего состояния в памяти. Если файл изменил кто-то другой — второе окно Obsidian, Remotely Save,
  правка руками — плагин об этом не знает, и первая же его запись затирает чужое.
- **#199 сделал это заметным, но не решил.** Проверка записи различает «наша запись не легла» и
  «файл заменил кто-то другой»; во втором случае повтор не делается и пользователь получает
  уведомление. Но состояние в памяти остаётся своим, и следующее изменение всё равно запишет файл
  целиком поверх чужого.
- **Дырка, названная аудитом прямо:** если хост молча не записал НАШЕ изменение и одновременно
  чужой писатель заменил файл, исход неотличим от чистого «чужой записал». Уведомление в этом
  случае говорит правду («последнее изменение могло не сохраниться»), но пометки «Не сохранено»
  не будет.
- **Работа:** решить, кому принадлежит файл. Варианты: следить за изменением `data.json` и
  перечитывать настройки; хранить версию или отпечаток и предлагать выбор при расхождении; либо
  объявить единственного владельца и блокировать запись при расхождении. Это выбор модели, а не
  правка — отсюда `analysis_required: true`.

### #201 — Пометка «Не сохранено»: текста не видно, и она притворяется кнопкой
- Status: ✅ DONE 2026-09-06 — `SPEC_201_SAVE_STATUS_2026-09-06.md` | Milestone: (next) | Priority: P2 | Complexity: S
- **Сделано:** заливки в покое нет, рамка волосяная в цвете текста, наведение и нажатие — оттенок,
  выведенный из того же цвета (`color-mix`), поэтому по светлоте он не может совпасть с текстом,
  как совпадали два независимых токена ошибки. Элемент остался `<button>`: «не в виде кнопки» —
  про форму, а отказ от элемента отнял бы фокус, Enter и сам повтор.
- **Место под код ошибки предусмотрено** — композиция знака оставляет слот, который займёт #202.
- analysis_required: false — нужен проход `designer`
- Vision scene: none — функциональный минимализм | User outcome: по пометке сразу понятно, что случилось, и она не выглядит действием, которого не совершали
- **Найдено живым прогоном 2026-09-06** (скриншоты 16:20). Пометка появляется там, где должна, и
  тултип с объяснением работает. Но текста на ней не видно: `SaveStatusChip.svelte` красит фон в
  `--background-modifier-error-hover`, а текст — в `--text-error`; в тёмной теме это красное по
  красному. Пользователь видит пустой красный прямоугольник.
- **Решение пользователя, дословно:** «для плашки возле плюса стоит доработать общий дизайн (не
  делать его в виде кнопки и наладить корректное отображение текста)».
- **Работа:** `designer` даёт спецификацию: пометка как ЗНАК состояния, а не кнопка; контраст,
  который держится в светлой и тёмной теме; место для кода ошибки (см. #202) и тултип с причиной.
  Повтор по клику остаётся — но выглядеть должно как состояние, а не как действие.

### #202 — У ошибок нет кодов, а у кодов — места, где о них написано
- Status: ✅ DONE 2026-09-07 — все шесть шагов, аудит и два ревью пройдены | Milestone: (next) | Priority: P2 | Complexity: M
- **Все 30 кодов подключены, меток `pending` не осталось; консоль говорит одним префиксом.**
- **Шаг 6 нашёл вдвое больше, чем инвентаризация.** План насчитал пять префиксов; ратчет `R0.23`
  сразу нашёл ещё пять — `FilterEngine`, `Performance`, `dataFrame`, `i18n`, `CalendarView`.
  Поиск по известным префиксам находит только те, о которых кто-то уже знал; правило о ФОРМЕ
  находит те, которых никто не перечислял.
- **Ревью поймало две вещи, которых я не увидел:** дублирование префикса в календарном логгере
  (существовало и раньше, шаг 6 сделал его длиннее) и то, что ратчет ловил чужой тег, но молчал
  на строках вовсе без тега — а их было шесть, и «Failed to save …» ищется в консоли ровно так же
  плохо, как «[Widget] …».
- **Что осталось долгом и вынесено отдельно:** #203 (страница кодов по-русски, пути к ней нет),
  #204 (ратчеты читают текст, а не поток данных), #205 (русская и английская формулировки одного
  кода разошлись).
- **Все 30 кодов подключены, меток `pending` не осталось.** Шаг 6 (единый префикс в консоли) —
  единственный незакрытый.
- **Отгруженный дефект закрыт:** 16 русских литералов в `CalendarView.svelte` уехали в `ru.json`,
  английский текст берётся из реестра. Русский пользователь читает то же, что читал; остальные
  перестали видеть чужой язык. Закреплено ратчетом `R0_22_noHardcodedUiText.test.ts`.
- **Состязательное ревью нашло регрессию, которую внёс я:** в доске и галерее сообщение о неудачном
  переименовании раньше несло текст исключения, а после подключения кода — только общую фразу, и
  причина не попадала даже в консоль. Код называет СОБЫТИЕ и не должен проглатывать ПРИЧИНУ:
  столкновение имён, отказ прав и недопустимое имя файла требуют от пользователя разного. Причина
  вернулась в консоль.
- **Открытые вопросы из того же ревью вынесены отдельно:** #203 (страница кодов русская, а коды
  видят все), #204 (ратчет видит только прямой литерал), #205 (русская и английская формулировки
  одного кода разошлись).
- **Выдано 30 кодов на 37 мест**, шесть подтверждений успеха номеров не получили. Часть мест
  оказалась одним событием: «переименование не удалось» жило в четырёх файлах, «редактор заметки
  не смог сохранить» — в трёх. Разные строки, одно событие для пользователя, теперь один код.
- **Область настроек подключена целиком:** один и тот же код виден в уведомлении, в пометке и в
  строке консоли.
- **Аудит нашёл дыру в самом ратчете, и это ценнее находки:** он проверял, зарегистрирован ли
  код, встреченный в `src/`, — и реестр удовлетворял эту проверку сам. Код, который страница
  объясняет, а показать некому, ничем не отличался от рабочего. Теперь такие коды помечены
  `pending`, и метка стоит денег: непомеченный обязан иметь производителя, помеченный — не иметь.
  Проверено снятием метки: тест краснеет.
- **Проверка подписей поймала дефект без подсадки:** четыре подписи `1xx` были переписаны
  «покрасивее» и разошлись с отгруженным текстом.
- **Отклонено из спецификации:** скрывать подпись пометки контейнерным запросом. Над шапкой никто
  не объявляет `container-type`, правило никогда бы не сработало, а мёртвое правило, выглядящее
  как защита, хуже отсутствия.
- **Осталось (шаги 4–6):** области `2xx`–`7xx` не подключены; там же живёт отгруженный дефект —
  16 русских литералов в `CalendarView.svelte`, которые видят все локали.
- **План поправил мои же цифры, и это важно.** Не «6 против 21», а 21 сообщение через локали
  против 22 голых литералов, причём **16 из 22 — захардкоженный русский в `CalendarView.svelte`**,
  который сегодня видят англоязычный, украинский и китайский пользователи. Это отгруженный дефект,
  а не гигиена. Живых уведомлений 43 в 11 файлах, консольных мест 40 с пятью разными префиксами,
  поверхностей пять, а не три. Коды нужны 37 местам: успехи не нумеруются.
- **Шаг 1 сделан: механизм раньше первого кода.** `src/lib/errors/errorCodes.ts` (чистые данные,
  ноль импортов), страница `docs/ERROR_CODES.md` и ратчет `R0_21_errorCodeRegistry.test.ts`.
  Ратчет проверен в обе стороны: на пустом реестре зелёный, на подложенном коде без секции —
  краснеет. Механизм, принятый вместе с первым пользователем, проверить нельзя — в него можно
  только поверить.
- **Ратчет проверяет структуру и биекцию, никогда не прозу** — урок `R_filterOrder`: тест,
  запрещающий документу стать точнее, хуже отсутствия теста. Плюс он пинит два правила плана:
  модули настроек и движка не импортируют резолвер (вес, а не циклы), и локали сохраняют BOM.
- analysis_required: true — затрагивает каждое место, где плагин что-то сообщает
- Vision scene: none — качество продукта | User outcome: у ошибки есть короткий код, по которому её можно найти и назвать, а при наведении — объяснение причины
- **Решение пользователя, дословно:** «удобнее всего для всех сообщений сделать кодировки — ошибка
  номер такая-то, но именно при наведении всплывашка, чтобы разъясняла причину ошибки; ну и в целом
  упорядочить все подобные записи в документации и отсылках было бы хорошим тоном и показателем
  качества разработки».
- **Почему это не косметика.** Сегодня одно и то же событие звучит по-разному в трёх местах:
  уведомление, пометка и строка в консоли. Пользователь не может сослаться на ошибку, а в отчёте
  о проблеме нет ничего, что можно найти поиском. Разбор #199 занял день ровно потому, что
  сопоставлять пришлось по текстам.
- **Работа:** единый реестр кодов (модуль, а не разрозненные литералы): код, короткая подпись,
  причина для тултипа, и одна страница документации, где каждый код описан. Уведомление, пометка
  и консольная строка берут текст из одного места, поэтому расходиться не могут. Нужен план
  архитектора: это касается каждого `Notice` в дереве и всех четырёх локалей.

### #203 — Страница кодов ошибок русская, а коды видят все локали
- Status: 🚧 ПОЛОВИНА СДЕЛАНА 2026-09-07 — страница двуязычная; путь из интерфейса остаётся | Milestone: (next) | Priority: P3 | Complexity: S
- **Сделано:** каждая из 30 секций получила английскую половину. Она не переведена заново, а взята
  из самого реестра — это те же слова, что показывает уведомление и печатает консоль, поэтому
  страница не может разойтись с продуктом. Написана здесь только строка «что делать»: такого поля
  в реестре нет.
- **Закреплено ратчетом:** `R0.21` требует у каждой секции обе половины — структурно, никогда не
  проверяя слова. Проверено посадкой: убрал английскую половину одной секции, ратчет покраснел.
- **Осталось: пути к странице из интерфейса нет.** Код показан, а куда с ним идти — не сказано
  нигде. Это решение об интерфейсе (ссылка в уведомлении, в тултипе пометки или в настройках
  плагина), и по стеку оно идёт через `designer`, а не делается по ходу.
- analysis_required: false
- Vision scene: none — качество продукта | User outcome: код в сообщении ведёт к объяснению на языке, который ты понимаешь
- **Найдено состязательным ревью.** `docs/ERROR_CODES.md` написана по-русски, а код `PPP-nnn`
  теперь видят англоязычный, украинский и китайский пользователи. Для них токен сегодня — способ
  сослаться в поддержке, а не найти ответ самому.
- **Второе, о чём ревью спросило прямо:** в интерфейсе нет пути к этой странице. Код показан, а
  куда с ним идти — не сказано нигде.
- **Работа:** английская версия страницы (она первична для кода — подписи в реестре английские),
  и решение, как на неё попасть: ссылка в уведомлении, в тултипе пометки или в настройках плагина.

### #204 — Ратчет текстов видит только прямой литерал в `Notice`
- Status: 📋 TODO — состязательное ревью #202 2026-09-07 | Milestone: (next) | Priority: P3 | Complexity: M
- analysis_required: true — нужен разбор: проверка потока данных, а не регулярное выражение
- Vision scene: none — доверие к воротам | User outcome: сообщение на чужом языке не проедет мимо ворот
- **R0.22 ловит ровно ту форму, которую имел дефект** — кавычку сразу после `new Notice(`. Литерал,
  доехавший до уведомления через переменную, конкатенацию или `defaultValue`, проходит мимо;
  `EditNote.svelte:186` — живой пример.
- **Формулировка ратчета уже сужена до проверяемого** (ревью указало, что широкое название учит
  доверять там, где ратчет слеп). Осталась сама проверка.
- **Работа:** проверка по потоку данных или по AST для всех поверхностей, показывающих текст
  пользователю: `Notice`, баннер виджета, панель `ErrorBoundary`, строка ошибки в модалке.

### #205 — Русская и английская формулировки одного кода разошлись по смыслу
- Status: 📋 ВОПРОС К ПОЛЬЗОВАТЕЛЮ — состязательное ревью #202 2026-09-07 | Milestone: (next) | Priority: P3 | Complexity: XS
- analysis_required: false — нужно решение о словах, а не разбор
- Vision scene: none | User outcome: одно и то же событие называется одинаково на всех языках
- **Пример из ревью:** `PPP-305` по-английски — «The checkbox could not be changed», по-русски
  сохранён исторический текст «Ошибка при изменении статуса». Это не дефект, а расхождение смысла:
  «отметка» и «статус» — разные слова о разном.
- **Почему не исправлено молча:** русский текст — тот, который пользователи читали до сих пор, и
  переписать его — продуктовое решение, а не перевод. Ревью назвало это прямо: нужно решение о
  языке, а не автоматическая ретрансляция.
- **Работа:** пройти 14 кодов календаря и решить по каждому, какая формулировка верна, — и
  выровнять вторую под неё.

### #206 — `uniqueIdCounter` не увеличивается ни на одном живом пути
- Status: 📋 TODO — найдено при подготовке приёмки #200 2026-09-07 | Milestone: (next) | Priority: P3 | Complexity: S
- analysis_required: false — сначала выяснить, что задумывалось NPLAN-A2, потом решать
- Vision scene: none — целостность данных | User outcome: поле «уникальный идентификатор» выдаёт номера, а не пустоту
- **Факт:** `settings.bumpUniqueId` (`src/lib/stores/settings.ts:75`) не вызывается **нигде** в
  `src/` — проверено по всему дереву. Тип поля `UniqueId` в интерфейсе выбрать можно
  (`ConfigureField.svelte:835`, `dataFieldTypeOptions.ts:36`), префикс настраивается, а счётчик за
  ним не двигается: значит либо номера не выдаются вовсе, либо выдаются мимо счётчика.
- **Как всплыло:** #200 принял решение пользователя переносить `uniqueIdCounter` как `max` при
  принятии чужого файла — и при подготовке живого прогона выяснилось, что риск, от которого эта
  страховка защищает, сегодняшний код сам создать не может. Страховка остаётся верной (поле в схеме
  есть, файл переживает версии плагина, дубли в заметках необратимы), но обещание интерфейса —
  пустое.
- **Работа:** пройти путь `UniqueId` от выбора типа до записи значения в заметку и установить, где
  он обрывается. Дальше — либо дописать выдачу через `bumpUniqueId`, либо снять тип из списка, если
  он не задумывался рабочим. Демо-проект в этот тип полей сегодня не заводит, поэтому пробел не
  виден на демо-данных.

### #210 — Конфликтная копия на 0 байт: «сохранили то, что там было» сохраняет ничто
- Status: ✅ DONE 2026-09-08 — исправлено на той же ветке | Milestone: (next) | Priority: P2 | Complexity: XS
- **Сделано:** решение вынесено в `settingsReconcile.ts` — `carriesAVersion` и отдельная причина
  `keep: "empty"`, чтобы «пусто» и «испорчено» перестали быть одним случаем. Пустой захват больше не
  снимается и копия не пишется; ветка с испорченными байтами не тронута. Правило по длине, а не по
  парсингу: пустой файл и через две секунды остаётся оборванной записью, а не чьей-то версией.
  Пять новых проверок в `settingsReconcile.test.ts`, включая границу между двумя причинами.
- analysis_required: false
- Vision scene: none — целостность данных | User outcome: уведомление о сохранённой чужой версии ведёт к файлу, в котором она есть
- **Факт прогона:** в инвентаре A10 копия `data.conflict-2026-09-08T06-25-50-387Z-42ce3e.json`
  размером **0 байт**. Происхождение подтверждено двумя следами: строка консоли `the unparsable
  settings file was replaced by our own write; preserving what it held`, следом `PPP-105 … reason:
  unparsable-overwritten; other version kept at …42ce3e.json`.
- **Причина:** `onUnparsableSettlement` (`src/main.ts:827`) сохраняет захваченные байты при условии
  `seen !== null`. Пустая строка это условие проходит. Внешний писатель, пишущий не атомарно
  (`writeFileSync` усекает файл перед записью — так пишет и часть синхронизаторов), на миг оставляет
  `data.json` нулевой длины; плагин успевает прочитать именно этот миг и объявляет пустоту чужой
  версией. Тот же пробел в соседней ветке `unparsable` тем же способом.
- **Почему это стоит починить, хотя данные не теряются:** копия попадает под синхронизацию (риск №3
  из §10 плана), а уведомление PPP-105 отправляет человека к файлу, в котором ничего нет — ровно та
  сломанная ссылка, против которой #195 и заводил разбор «копия не удалась».
- **Работа:** считать пустой захват отсутствием версии — не писать копию и не поднимать PPP-105.
  Осторожно с границей: «файл существует и пуст» через 2 секунды это тоже не чужая версия, а
  оборванная запись, поэтому условие про длину, а не про парсинг.
- **Как воспроизвести:** внешний писатель перезаписывает `data.json` каждые 60 мс в течение ~2 с,
  одновременно с командой плагина, меняющей настройки; среди копий появляется нулевая.

### #211 — Гонки на ветке конфликта: чужая версия могла быть затёрта раньше, чем сохранена
- Status: ✅ DONE 2026-09-08 — `fix/211-conflict-races` | Milestone: (next) | Priority: P1 | Complexity: S
- **Найдено догоняющим ревью** (`CX-REVIEW-200-followup.md`): предмерджевый гейт прошёл на `f49e988`,
  а после него в `main` уехали два коммита с кодом — реализация находок того же ревью (`83abc43`) и
  фикс #210 (`8d13d84`). Три находки, все верные.
- **Ускорение чужой записи (P1, регресс вчерашней правки).** Дренаж очереди после `diverged` не
  различал, чья это запись: обычная правка пользователя уходила на диск в тот самый миг, когда мы
  узнали о чужом файле, обгоняя хук, который собирался эту чужую версию сохранить. Теперь немедленно
  дописывается только значение, поставленное `pushImmediate` (восстановление реконсиляции); обычной
  правке возвращается её дебаунс — и она при этом не теряется, иначе вернулось бы то самое зависание,
  ради которого дренаж и делался.
- **`hold()` не останавливает запись в полёте (P1).** Он снимает таймеры, а уже начатая запись
  завершится и ляжет поверх. Хук теперь ждёт `settled()` перед решением конфликта: ничего нашего не
  ложится между решением и сообщением о нём. `flush()` для этого не годится — он пишет отложенное.
- **`PPP-106` указывал на файл, которого уже могло не быть (P2).** Инструкция «скопируйте `data.json`
  руками» неверна, если наша запись успела его заменить. Текст переписан во всех локалях и на
  странице кодов: чужая версия выводится **целиком в консоль**, и уведомление отправляет туда. Это
  последний канал, который у ветки остаётся, когда файл записать не удалось.
- **Второй проход ревью по самой этой ветке — ещё две находки, обе верные.** Перевод обычной правки
  на её дебаунс был всё ещё гонкой: 400 мс против хука, которому нужно прочитать файл и записать
  копию. Значение теперь **удерживается** до `resume()`, который зовёт каждая ветка хука, закончив
  решать; страховка от зависания — `SETTINGS_RECONCILE_BACKSTOP_MS` (10 с), после которых значение
  уходит на диск само, даже если хук не пришёл вовсе. Вторая находка: признак «немедленной» записи
  жил на писателе, а не на значении, поэтому обычная правка, сделанная поверх поставленного
  `pushImmediate`, наследовала право обойти дебаунс — теперь `push` его снимает.
- **Третий проход — ещё две, обе верные.** `flush()` на выключении игнорировал удержание: он пишет
  принудительно, и на quit/unload правка легла бы поверх чужого файла без единой копии. Теперь
  удержанное значение на выключении **не пишется** — оно теряется при перезагрузке, ровно как #185 и
  обещает для незаписанного изменения, а стоячая пометка предупреждает заранее. Вторая: у `PPP-106`
  реестр и локаль разошлись — `resolveError` берёт перевод, а не дефолт, поэтому подсказка знака
  ещё звала к `data.json`. Текст выровнен, и **R0.21 усилен**: причина теперь сверяется с английским
  дефолтом так же, как заголовок, — этой проверки не было, потому расхождение и проехало.
- **Четвёртый проход — одна находка, верная: у `PPP-106` не было пути восстановления на телефоне.**
  `isDesktopOnly: false`, а консоли в мобильном Obsidian нет — значит «заберите из консоли» для этих
  пользователей не путь, а его отсутствие. Добавлен запасной вариант **внутри хранилища**: заметка
  `Projects+ settings conflict <отметка>-<токен>.md` в корне, чужая версия внутри в блоке кода, с
  инструкцией, куда её вернуть. Она же закрывает слепое пятно плана: при неизвестном `manifest.dir`
  писать рядом с файлом настроек некуда, а корень хранилища на месте. Путь к `data.json` **передаётся
  вызывающим**, а не собирается — папка конфигурации настраиваемая, и правило проекта об этом ловит
  такую сборку как ошибку. `PPP-106` остаётся только на случай, когда не пишется вообще ничего, и его
  текст теперь говорит именно это.
- **Пятый проход — одна находка, верная: заметка восстановления попадала в источники.** Проект с
  папкой-источником в корне хранилища забирает её как обычную запись (`FolderDataSource.includes`
  пропускает всё при пустом пути проекта) — а в самой заметке было написано «ничего её не читает».
  Заметка переехала в собственную папку `Projects+ recovery/` (с откатом в корень, если папку
  создать нельзя), а ложная фраза заменена честной: плагин её не читает, но проект, покрывающий эту
  папку, покажет её записью — удалите, когда заберёте нужное.
- **Шестой проход — одна находка, верная и того же рода: удержание применялось после ожидания.**
  `settled()` возвращается сразу, когда записи в полёте нет, и **не снимает уже заведённый дебаунс** —
  а решение конфликта состоит из нескольких await (прочитать файл, записать копию, записать заметку).
  Таймер мог выстрелить в середине и затереть ровно ту версию, которую в этот момент сохраняли; при
  отказе копии `PPP-106` затем утверждал бы, что поверх ничего не записано. Теперь порядок обратный:
  **сначала удержать, потом ждать, потом сохранять**, во всех четырёх местах ветки конфликта.
- **Седьмой проход — снова P1 из моей же заплатки, и это довод против заплаток.** `hold()` сбрасывал
  флаг отсрочки, а `resume()` по нему же и решал, взводить ли таймер: после «удержать → сохранить →
  отпустить» значение оставалось грязным без единого таймера, то есть до следующей правки или до
  выключения — а в прерванном сеансе это его потеря. **Починено сведением, а не латанием:** отсрочка
  и удержание были двумя механизмами с двумя флагами и разными дырами, теперь это одно состояние
  «запись приостановлена». Его ставят обе причины, снимают `resume`, `push` и `prime`, `flush` его
  уважает, а `startWrite` проверяет — так что путь, обошедший таймеры, тоже не пишет. Страховка на
  10 с осталась только у отсрочки: удержание ждёт человека, которому уже сказали.
- **Заодно закрыта дыра того же рода, до которой ревью не дошло:** `hold()` очищал флаг, по которому
  `flush()` отказывался писать, — то есть выход из Obsidian записывал значение поверх файла, ради
  защиты которого удержание и вводилось. Тест на это добавлен и он падал до правки.
- **Восьмой проход — P3 и впервые не гонка:** страница кодов описывала только один из двух путей
  восстановления. Обе половины переписаны на «смотрите путь в уведомлении» с объяснением обоих мест.
- **Девятый — P1 и снова следствие моего сторожа:** `startWrite` стал отказывать при приостановке, а
  ветка «файл не разобрался резолвером» звала `hold()` и затем `pushImmediate()` без `resume()` —
  восстановление молча не происходило. Починено там же, где причина: **`pushImmediate` сам снимает
  приостановку**, потому что это и есть запись реконсиляции, ради которой приостановка вводилась.
  Требовать от вызывающего помнить о порядке — ровно та форма дефекта, которую эта ветка уже дала.
- **Десятый проход — P2, снова из семейства «обещание, которое код не держит»:** ветка конфликта
  удерживает писателя дважды (сначала чтобы решать, потом с кодом, который в итоге сказало
  уведомление), а `sameStatus` считал любые два `diverged` одинаковыми — второе удержание
  проглатывалось, и пометка продолжала утверждать `PPP-105` («чужая версия сохранена»), когда
  уведомление уже сказало `PPP-106` («сохранить не удалось нигде»). Сверяется код.
- **Одиннадцатый проход — P1: пустой файл считался разрешением эпизода.** Ветка «пусто» звала
  `resume()`, то есть отпускала удержанную правку пользователя, — а пустой `data.json` это писатель
  между усечением и заполнением, и его содержимое ещё в пути: наша запись могла лечь поверх него
  раньше, чем что-либо его сохранит. **Починено сведением, а не местом:** «завершает ли решение
  эпизод» стало свойством самого решения (`resolves` в `settingsReconcile.ts`), а хук перестал
  помнить это по веткам — забывчивость и была дефектом. Свойство проверяется юнит-тестами по каждой
  строке таблицы.
- **Ворота:** все четыре зелёные на каждом шаге — tsc 0, jest без падений и выше базиса, lint 0
  ошибок, svelte-check 0/0. Числа сьютов и тестов здесь намеренно не повторяются: их канонический
  дом — `CONTEXT.md`, и копия в живом документе устаревает молча (это же ревью и поймало копию,
  отставшую на девять тестов).
- **СТОП-ПРАВИЛО СРАБОТАЛО.** Одиннадцатый и десятый проходы нашли дефекты, внесённые правками
  предыдущих проходов, — цикл «нашли — залатали» остановлен по правилу из `TWO_MODEL_PROTOCOL.md`
  §Gate 3. Дальше — решение пользователя: merge как есть либо архитектурный проход по координации
  `settingsWriter` ↔ хук внешнего изменения.
- **Не проверено живьём:** ветка `PPP-106` требует отказа записи в папку плагина — поставить это, не
  сломав хранилище, нельзя; покрыта юнит-тестами на отказывающем адаптере.

### #212 — Кто имеет право писать `data.json`: протокол вместо флагов
- Status: 🔧 В РАБОТЕ — архитектурный проход, запущен 2026-09-08 | Milestone: (next) | Priority: P1 | Complexity: L
- analysis_required: **true** — это выбор модели координации, а не правка
- Vision scene: none — надёжность | User outcome: изменения не теряются ни с одной стороны, и это не зависит от того, вспомнил ли код снять флаг
- **РЕШЕНО пользователем 2026-09-08:** после тринадцати проходов ревью по #211 выбран **архитектурный
  проход по координации**, а не merge как есть и не заморозка. Основание — стоп-правило
  `TWO_MODEL_PROTOCOL.md` §Gate 3: два прохода подряд нашли дефекты, внесённые правками предыдущих.
- **Предмет:** право записи в `data.json` распределено между `settingsWriter.ts` (таймеры, очередь,
  приостановка) и хуком `onExternalSettingsChange` в `main.ts` (решение и сохранение чужой версии),
  и согласуется флагами. Шесть из двадцати двух находок эпизода — дефекты именно этой согласовки,
  каждый внесён локально верной правкой предыдущего.
- **Что уже сошлось и должно быть сохранено любым решением:** суждение живёт в чистом модуле
  (`settingsReconcile.ts`, `main.ts` юнит-тестами не покрывается вовсе); одно состояние приостановки
  вместо двух механизмов; «завершает ли решение эпизод» — свойство решения, а не память хука.
- **Ограничения:** ни одна сторона не уничтожается без предварительного сохранения; бессрочная
  блокировка записи отвергнута (`PLAN_200` §2, модель «в»); правка пользователя не пропадает молча;
  модули решений не импортируют `obsidian` и i18n (R0.21).
- **План готов и проверен:** `PLAN_212_WRITE_PERMIT_2026-09-08.md`. Gate 0 (`CX-GATE0-212.md`)
  конструкцию **не опроверг** — ни недостижимых состояний, ни необеспеченных инвариантов, ни ложных
  «поведение не меняется»; единственная находка поправила тяжесть открытого дефекта с P2 на P1:
  верность байтов в памяти не есть сохранность, и при отказе всех мест сохранения запись сеанса
  успевает лечь раньше, чем появится долговечная копия.
- **РЕШЕНО пользователем 2026-09-08: делаем весь план A–F**, а не минимум A+B перед merge. Порядок и
  дельты — `PLAN_212` §4; после F идут ревью по `HEAD` (правило Gate 3) и живой прогон с привязкой к
  сборке.
- **Ветка `fix/211-conflict-races` остаётся вне `main` до конца работ.**
- **Сделано A–F (2026-09-08/09).** A: очередь несёт значение вместо трёх флагов; B: разрешение стало
  одним значением `open/fenced/held/closed` — и это закрыло P1, найденный архитектором; C: чистый
  `settingsEpisode.ts`, исход и код эпизода из одного значения, таблица; D: `withExclusive` — эпизод
  стал лизом со снятием в `finally`, удалены `hold`, `resume`, `settled`, восстановительный
  `pushImmediate`, таймер перечитывания и снимок байтов; **четыре точки ограждения и восемь точек
  снятия стали нулём**; E: структурное сравнение статусов вместо ручного по видам, удалён `Origin`,
  у которого после лиза не осталось читателей; F: ратчет **R0.24** — один писатель `data.json`,
  отставленные методы не возвращаются, модули решений не тянут `obsidian` и i18n.
- **Две вещи вскрыла реализация, а не план:** поле продолжения нельзя называть `then` (объект
  становится thenable, и `await` вызвал бы продолжение при разрешении промиса), и восстановление не
  имеет права затирать правку, поставленную в очередь во время эпизода.
- **Ратчет сразу нашёл, что план был выполнен наполовину:** `hold`, `resume` и `settled` оставались
  объявленными, потому что их ещё звали тесты. Отставлены по-настоящему, тесты переведены на лиз.
- **Ворота после каждого шага:** все четыре зелёные; счёт сьютов и тестов — в `CONTEXT.md`, а не
  здесь.
- **Шаг G, живой прогон: 8 из 8** (`ACCEPTANCE_212_RESULTS_2026-09-09.md`), и он нашёл дефект,
  которого не нашли одиннадцать проходов ревью: чужая версия, записанная пока наша запись стояла в
  дебаунсе, **затиралась молча** — файл перечитывался только после записи и никогда до неё. Правка
  `7c1d262` (проверка перед записью) и `cf0044b` (эпизод запоминает увиденное, иначе восстановление
  отвергает само себя — 237 копий в одном прогоне). Пункт A3, который в прошлый раз «не поставился»,
  теперь проходит с первой попытки: до правки эта ветка была недостижима.
- **Осталось:** кросс-модельное ревью коммитов после `623e600` — у Codex исчерпан лимит
  (`You've hit your usage limit`), гейт недоступен; и решение пользователя о merge.

### #213 — Окно между проверкой файла и записью: нужен compare-and-write
- Status: 📋 TODO — найдено ревью #212 2026-09-09 | Milestone: (next) | Priority: P2 | Complexity: M
- analysis_required: **true** — это смена примитива записи, а не правка
- Vision scene: none — надёжность | User outcome: чужая версия не теряется даже при попадании в
  миллисекундное окно между проверкой и записью
- **Факт.** `beforeWrite` (#212, `7c1d262`) закрыл случай, который нашёл живой прогон: чужая версия
  больше не затирается, если она уже лежала на диске к моменту нашей записи. Но проверка читает файл
  асинхронно, и версия, пришедшая **между** её чтением и началом записи хоста, всё ещё теряется
  молча: пост-проверка сравнивает файл с нашими же байтами и докладывает успех.
- **Почему это не чинится сторожем.** Любая проверка «прочитал — записал» оставляет то же окно.
  Нужен compare-and-write. Ближайшее, что даёт хост, — `DataAdapter.process(path, fn)`,
  документированный как «атомарно прочитать, изменить и сохранить» (`obsidian.d.ts:1005`).
- **Цена, из-за которой это отдельный тикет.** `process` означает, что `data.json` сериализует
  плагин, а не `saveData`: меняется форматирование файла, меняется единственный писатель (ратчет
  R0.24 держит именно `saveData`), и атомарность у него — внутри Obsidian, а не на уровне ОС, так
  что внешний синхронизатор всё равно может влезть. Всё это требует своего плана, своих тестов и
  своего живого прогона — впихивать это в #212 значило бы менять примитив записи под видом правки.
- **Ограничение честно записано** в `PLAN_212` §5 и в комментарии к `beforeWrite`, а не выдано за
  закрытое.

### #207 — В консоли печатается сырой `{{path}}` вместо пути к конфликтной копии
- Status: 📋 TODO — найдено на живом прогоне приёмки #200 2026-09-08 | Milestone: (next) | Priority: P3 | Complexity: XS
- analysis_required: false
- Vision scene: none — диагностика | User outcome: строку из консоли можно вставить в issue и она читается
- **Факт, наблюдённый в DevTools:** `[Projects+] PPP-105 data.json was changed outside this window …
  saved as {{path}}. reason: pending; other version kept at .obsidian/plugins/obs-projects-plus/
  data.conflict-2026-09-08T04-38-32-011Z.json`. Плейсхолдер не подставлен.
- **Причина:** `errorLine` (`src/lib/errors/errorLog.ts`) берёт `entry.caption` из реестра **как
  есть** — реестр хранит текст с `{{path}}`, а параметры подставляет только `resolveError`
  (`errorText.ts`), которым консольная половина намеренно не пользуется (она английская и не тянет
  i18n). Путь при этом не теряется: он дописан следом в `details` из `main.ts:820`.
- **Пользовательский `Notice` не задет** — `main.ts:821` передаёт `{ path: copiedTo }`, и на живом
  прогоне всплывающее уведомление показало настоящий путь. Это дефект только консольной строки.
- **Работа:** либо принимать в `logWarning`/`logError` параметры и подставлять их в caption без
  i18n, либо убрать `{{path}}` из caption тех кодов, где консоль всё равно печатает путь отдельно.

### #208 — Причины кодов ошибок есть только по-английски, и это видно в интерфейсе
- Status: 📋 TODO — найдено на живом прогоне приёмки #200 2026-09-08 | Milestone: (next) | Priority: P2 | Complexity: M
- analysis_required: false
- Vision scene: none — локализация | User outcome: подсказка объясняет причину на языке интерфейса, а не наполовину
- **Живое наблюдение:** подсказка знака `diverged` на русском интерфейсе — «Файл data.json заменили
  вне этого окна. Ваши изменения на месте; повтор записи затёр бы чужую версию, поэтому он не
  предлагается. **Another window or a synchroniser replaced the file, so this window cannot tell
  whether its own change survived.**» Русская фраза — `save-status.diverged.tooltip`, английская —
  `errors.causes.settings-superseded`, которого в `ru.json` нет, и i18next отдаёт `defaultValue`.
- **Масштаб (посчитано по реестру и четырём JSON 2026-09-08):** из 32 кодов с ключами i18n
  `causeKey` отсутствует в **ru — 32, uk — 32, zh-CN — 32** (то есть везде, кроме en);
  `key` (заголовок) отсутствует в ru — 14, uk — 25, zh-CN — 27.
- **Почему это не поймали ворота:** `SaveStatusChip` склеивает подсказку из двух источников
  (`t(...)` + `resolved.cause`), и оба возвращают строку. Ни один тест не спрашивает, на каком она
  языке; #203 закрывал страницу кодов, а не причины.
- **Работа:** перевести `errors.causes.*` в трёх локалях и дописать недостающие заголовки; затем
  ратчет, который валит сборку, если у кода есть `causeKey`, а в локали его нет.

### #209 — Пометка «Изменён извне» живёт доли секунды, хотя объявлена стоячей
- Status: 📋 TODO — найдено на живом прогоне приёмки #200 2026-09-08 | Milestone: (next) | Priority: P3 | Complexity: S
- analysis_required: true — сперва решить, что считать разрешением расхождения
- analysis_done: false
- Vision scene: none — честность поверхности | User outcome: человек успевает увидеть, что его запись перекрыли
- **Наблюдение:** знак пойман опросом DOM каждые 100 мс — он присутствовал **в 2 кадрах из 257**
  (~200 мс). Комментарий компонента обещает обратное: «A standing mark, not a toast: the state it
  reports is standing too» (`SaveStatusChip.svelte`).
- **Механизм:** статус кладёт `onStatus` писателя (`main.ts:433`). После `diverged` плагин не
  повторяет запись, но **следующее принятие чужого файла** проходит через store и возвращает статус
  в рабочий; при серии внешних записей это происходит через десятки миллисекунд.
- **Что решить до правки:** принятие чужой версии действительно разрешает расхождение — тогда сброс
  верен, и чинить надо обещание в комментарии. Но если человек так и не узнал, что его запись
  перекрыли, пометка не выполнила свою работу. Возможный ответ — минимальное время жизни пометки
  либо её снятие только действием человека.
- **Как воспроизвести:** внешний писатель перезаписывает `data.json` каждые 60 мс в течение ~2 с,
  начиная одновременно с `create-demo-project`; в консоли появляется PPP-102.

### #194 — «Область данных»: одна видимая настройка для всех виджетов данных
- Status: ✅ DONE 2026-09-05 (`PLAN_194_DATA_SCOPE_2026-09-05.md`, аудит `CX-AUDIT-194.md`)
  | Milestone: M-MATRYOSHKA | Priority: P2 | Complexity: L
- analysis_required: **true** — пользователь просил не просто реализовать, а переосмыслить: это может
  оказаться техдолгом, а может — багом, который станет фичей
- Vision scene: 5 | User outcome: у любого блока видно и можно снять то, какие записи он показывает
- **Как это всплыло.** Состязательное ревью дня (`codex-reports/CX-ADV-DAY-2026-09-04.md`, п.6):
  `sourceId` читается **всеми** типами виджетов (`hostFrames`), а `projectId` гейтится по типу
  (`linkedSourceState`), при том что UI выбора источника есть только у `database-call`. Следствие,
  которого я не назвал, когда документировал расширение как осознанное: **у чарта, суженного
  `sourceId`, нет способа это снять** — его панель такого селектора не показывает. Записать значение
  сегодня может только ручная правка `data.json`, поэтому вероятность низкая, а форма скверная:
  конфигурация, которую интерфейс не показывает и не отменяет.
- **РЕШЕНО пользователем 2026-09-05: второй вариант — общая настройка для всех виджетов данных.**
  Не ограничивать `sourceId` одним типом (это был мой дефолт и он отвергнут), а дать видимую и
  снимаемую настройку везде, где она уже действует.
- **И прямое указание: отдать архитектору на переработку и осмысление.** Формулировка пользователя —
  «возможно есть технический долг или это баг который станет фичей». То есть проход должен не
  спроектировать селектор, а сперва ответить, чем является нынешнее расхождение.
- **Что уже известно и не надо выяснять заново:** `sourceId` объявлен в `WidgetSourceConfig`
  (`Dashboard/types.ts`) с комментарием, объясняющим расширение; резолвится в `hostFrames` до гейта по
  типу; `projectId` гейтится в `resolveDbCallView`; панель `DatabaseCallSettings` открывается только
  для `database-call` (`WidgetHost`, `hasCog`); пикер источников проекта добавлен в #184 шаг 1.
- **Связанное ограничение, уже записанное:** `data-table` **читает** `sourceId`, но выбрать его негде
  (#184). Этот тикет его поглощает.
- **Сделано 2026-09-05, три коммита, через полный стек** (архитектор → implementer → аудитор).
- **Правило вместо списка:** у виджета есть область данных тогда и только тогда, когда он **выводит
  показываемое из переданного кадра**. Шесть типов из шестнадцати; `cover-banner` кадра не получает,
  `text`/`divider` статичны, снятые legacy рисуют заглушку — пикер на заглушке это хром на трупе.
  Живёт в `headerChrome.ts` рядом с `hasPipelineButton` и `primaryActionFor`.
- **Шестерёнка получила вторую причину появиться, а не переопределённую первую.** `configPanelRegistry`
  не тронут: его `hasCog` означает «у типа есть своя панель», и для `data-table` это по-прежнему
  правда — настройки колонок живут в контекстном меню. Тест реестра остался зелёным.
- **Что не выдержало контакта с планом, и исполнитель сказал сам:** «остальные пять типов» оказались
  четырьмя (шесть имеют область, два уже рисовали уведомление); стили нельзя было перенести вместе с
  селектором — их используют три других поля той же панели, поэтому они уехали в общие примитивы
  `WidgetConfigShell`; названные планом кандидаты на вынос давали 7–9 строк из нужных 15, и честным
  подъёмом оказалась цепочка контента — а она же оказалась правильным домом для уведомления, потому
  что уведомление **заменяет** содержимое.
- **Два решения сверх плана, оба про «не соврать в поверхности, построенной против вранья»:** `select`
  со значением, которого нет среди опций, молча рисует первую — значит чарт на удалённом источнике
  показывал бы «Все источники», храня другое; теперь сохранённый id добавляется отдельной подписанной
  опцией. И проект с единственным источником получает объяснение вместо пустой панели.
- **Аудит вернул BLOCKED на механической находке, и она настоящая:** комментарий, объясняющий запрет
  подавления типов, содержал запрещённый литерал, а хук сканирует `src/` и **не различает запрет и
  употребление**. Исправлено; заодно записано, почему правило не пишется дословно. Правка вывела файл
  на 78 строк при потолке 75, поставленном тем же коммитом, — ужат комментарий, а не потолок.
- **Не проверено вживую:** уведомление на удалённом источнике, шестерёнка у `data-table`, удаление
  ключа из файла. Четыре зелёных ворот ни одного из этих пунктов не устанавливают.
- **Вердикт архитектора: (c) — возможность приехала раньше своей поверхности, её надо доделать.**
  Не баг: ни один путь кода не **пишет** `sourceId` на не-`database-call` (единственный писатель
  монтируется только там). Не техдолг: движок разрешает `sourceId` правильно для всех типов — по
  каноническому порядку, над обогащённым кадром, с гарантией by-reference. Настоящий дефект в другом:
  **один хранимый объект держит два вопроса разного ранга** — `projectId` это *способность* типа
  (только `database-call` умеет чужой проект и имеет защиту записи #139), а `sourceId` это *ось A*,
  осмысленная для чарта и таблицы одинаково. Асимметрия, показавшаяся ревью подозрительной, — эта
  упаковка, просвечивающая наружу.
- **Находка, которой не было в тикете и которая тяжелее исходной жалобы.** Уведомление о сломанном
  источнике доходит только до `data-table` и `database-call` (`namedSource` передаётся ровно двум
  записям реестра). При `kind: "broken"` кадр откатывается к целому проекту — значит **чарт на
  удалённом источнике молча рисует правдоподобно неверные числа**, и «удалён» неотличимо от «не
  задан». Это сильнее, чем «нельзя снять», и это главный аргумент за выбранный вариант.
- **Сцена 5 закрывается попутно и без новой поверхности:** сохранённый фильтр уже элемент того же
  `pickable`, поэтому «именованный фильтр, открытый как источник» получается показом существующего
  пикера там, где его сегодня нет.
- **Ограничение, которое продиктует структуру:** `WidgetHost` — 229 строк при потолке 230, а нужно
  около десяти. Потолок поднимать нельзя, значит коммит обязан вынести из хоста не меньше, чем вносит.
- **Прямой путь заминирован:** включить `hasCog` для `data-table` в реестре нельзя — это роняет
  `configPanelRegistry.test.ts`, где закреплено, что его настройки живут в меню колонки. Шестерёнка
  получает вторую причину появиться в `WidgetHost`, а реестр не трогается.
