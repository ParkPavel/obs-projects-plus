...
- **Работа:** `PLAIN_MODE` возвращается к открытию заметки; просмотр полей получает собственный
  вход (пункт контекстного меню строки и/или модификатор), и — по формулировке пользователя —
  настройку, какое действие считать основным. Контракт `R0_17_recordOpenContract` уже держит все
  поверхности в одной точке, поэтому менять надо ровно одно место, а тест это докажет.
...
- **Работа:** удалить мёртвую ветку; по остальному — решить с пользователем, что именно «устарело»,
  потому что оба механизма работают. Это вопрос продукта, а не кода.
