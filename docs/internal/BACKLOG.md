...
- Status: ✅ DONE 2026-09-02 (ветка `fix/181-ratchet-worktree-exclusion`, `dbb43b8` + `f30f8f7`;
...
- **Сделано:** обход один на троих — `src/__tests__/support/configScan.ts`. Исключения разделены
  ...
  которого всё делается. Симлинк-каталоги (junction) перечисляются, но внутрь обход не идёт.
...
- **Первая редакция была неверной, и это поймал аудит Codex, а не гейты.** Исключение `worktrees`
  по имени на любой глубине спрятало бы обычный конфиг-каталог `.claude/agents/worktrees/` от всех
  трёх ратчетов сразу. Ровно то, о чём предупреждал сам тикет: исключение, написанное шире своего
  предмета, — это способ перестать быть гейтом. Исправлено в `f30f8f7`, случай добавлен в тест.
