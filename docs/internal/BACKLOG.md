...
- Status: ⚠ КОД ГОТОВ, ПРИЁМКА ОТКРЫТА (ветка `feat/165-token-consolidation`, тип `e3f949c`;
  в `main` НЕ слито) | Milestone: M-MATRYOSHKA | Priority: **P1** | Complexity: M (было S)
...
- **Что приёмка ещё не закрыла** (`UNTESTABLE_FEATURES_2026-09-01.md`): работает ли механизм
  второго уровня. Проверка одна — два чарт-виджета разной ширины рядом; в широком подписи и отступы
  должны быть крупнее. Если одинаковы — откатывается **только** `9e870e4`, шаги 1–3 стоят сами.
