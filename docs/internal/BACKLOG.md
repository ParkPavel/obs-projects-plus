...
- Status: ✅ DONE 2026-09-02 (ветка `fix/181-ratchet-worktree-exclusion`, `dbb43b8`, `f30f8f7`, `fbcd0c9`;
  в `main` НЕ слито) | Milestone: (инфраструктура) | Priority: **P1** | Complexity: XS
- analysis_required: false
- Vision scene: none | User outcome: делегирование по маршрутизации перестаёт красить гейты
- **ВНИМАНИЕ ПРИ МЕРЖЕ.** Тикет заведён на ветке `feat/165-token-consolidation` (`7178122`), в
  `main` его текста нет. Эта запись написана на ветке фикса от `main`, поэтому при слиянии обеих
  веток два текста #181 столкнутся в одном месте файла — это сделано намеренно, чтобы git показал
  конфликт, а не создал молча два раздела. Побеждает эта редакция: она описывает выполненную работу.
