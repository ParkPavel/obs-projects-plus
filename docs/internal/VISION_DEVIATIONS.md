...
> **Created:** 2026-08-27 (#157) · **Updated:** 2026-09-04
...
| D-5 | Scene 5 — a saved filter becomes a base with its own views | A named saved filter now exists as a source of its project and a block can be pointed at it (#170, #184) | **Largely met, and the entity turned out to be unnecessary.** The 2026-08-27 decision was to build a fourth entity; the container level was then found to already exist, so a saved filter is a SOURCE (`kind: "derived"`), which needed no migration. Remaining gap is narrower than the row: it is not "its own views" but who can point at it — only `database-call` has the picker UI today. |
| D-7 | Scene 5 — a saved filter may express what the user can see, including computed columns | The view's filter bar edits against the **unenriched** frame, so a condition on a rollup or a backlink cannot be BUILT there — though such a filter resolves correctly once stored | **Accepted 2026-09-04 (#184 step 2), and it is a narrower gap than it looks.** Storage and resolution have no such limit: `resolveNamedSource` runs over the enriched frame precisely so a `where` naming a rollup works, which is the Gate 0 refutation #170 was built on. What is limited is the one surface that can CREATE one. The bar was still the right home — it is the only place a filter is visible and has already narrowed what is on screen, which is the verify-after-write rule the brief settled on. Closing this means enriching the frame the bar edits against, which is a per-view cost nobody has measured. |
