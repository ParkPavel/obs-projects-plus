...
`database-call` блок может открывать **sub-base**: при клике на запись открывается детальная карточка, которая сама является mini-canvas с собственными блоками данных, подтянутыми через wiki-link-связи.

```
Главный канвас
└── [database-call: Клиенты]
    └── Клик на запись → Sub-base карточка клиента
        ├── [database-call: Сеансы этого клиента]  ← relation-фильтр
        ├── [stats: динамика боли]                  ← aggregation
        └── [chart: визит-частота]                  ← cross-base aggregation
```

Глубина — произвольная. Каждый уровень — полноценный canvas.

### 2.3 Канвас — тонкий оркестратор

`DashboardCanvas.svelte` — только layout + routing.
**≤ 200 LOC**. Никаких вызовов engine, никакого fetch данных.

### 2.4 Engine — в lib, не в UI
...
### 2.6 FreeCanvas — удалить полностью
...
`database-call` — главный атом Dashboard. Два режима работы:

### 5.1 Режим списка (List mode)
...
│  Иван Петров  │  Active    │  2026-06-01             │  ← клик → Sub-base
│  Мария Иванов │  Remission │  2026-05-15             │
└─────────────────────────────────────────────────────┘
```
...
### 5.2 Режим Sub-base / Матрёшка (Detail mode)

Клик на запись → **открывается `SubBasePanel`**. Это mini-canvas уровнем ниже.

```
SubBasePanel (Иван Петров)
├── [database-call: Sessions]  ← auto-filter: client == [[Иван Петров]]
│   └── [Table: дата, длительность, боль 0-10, заметки]
├── [stats: динамика боли]     ← агрегат по Sessions.pain_level
└── [chart: частота визитов]   ← aggregation по датам Sessions
```

**Как строятся auto-filter для sub-base:**
- Поле в дочерней базе с типом `Relation`, ссылающееся на родительскую запись
- При открытии SubBasePanel передаётся `recordId` → автоматически создаётся `FilterDefinition` по relation-полю
- Пользователь видит только связанные записи, без ручной настройки

### 5.3 Настройка блока

Каждая вкладка хранит свой `config` — sort/filter/group/columns. Переключение вкладок не сбрасывает состояние соседних.

**Создание в UI**: кнопка "Add Block" → выбор источника (папка/тег/запрос) → автоматически создаётся вкладка Table → пользователь добавляет остальные вкладки по желанию.
...
### 6.3 Engine — filterEvaluator.ts (не трогать)
...
### 6.4 Фильтры как базы (native-query)
...
7. Каждый новый виджет-тип — **тест в widgetRegistry.test.ts**.
8. Каждый новый публичный метод в `dashboard-engine/` — **unit-тест**.
...
### Фаза 4 — DatabaseCallWidget + Матрёшка
...
- [ ] Auto-filter по relation при открытии SubBasePanel
- [ ] Все 4 гейта ✅
...
## 12. Решения (согласовано 2026-06-09)
