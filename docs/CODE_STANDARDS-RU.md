...
> Русский · [English](CODE_STANDARDS.md)

Этот документ — справочник для контрибьюторов: стандарты кодирования, требования Obsidian Community Plugins, которым должен соответствовать код, и решения, из которых эти правила выросли. Он описывает то, что проверяют автоматические проверки: если правило и код расходятся, нужно поправить одно из двух, а не оставлять оба.
...
| Ошибки ESLint | ✅ | 0 |
| Ошибки TypeScript | ✅ | 0 |
| Модульные тесты | ✅ | весь набор проходит |
| Сборка | ✅ | ~7.5с, 1.8MB |
...
**Решение**: Оставить `@ts-ignore` с комментариями. Immer проверен в бою, а корректность во время выполнения покрыта модульными тестами.
...
- [ ] `npm test` — проходят все наборы тестов
...
4. **Полные тесты** — модульные тесты движка, слоя данных и логики интерфейса
