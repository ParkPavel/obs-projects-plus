...
| **Dashboard** | Мульти-виджетный канвас: 8 типов виджетов, 10 типов диаграмм, трансформ-пайплайн (7 шагов + cross-project join), sub-базы с вкладками, условное форматирование, встраивание других видов (ViewPort) |
...
**Dashboard** — мульти-виджетный канвас поверх данных проекта. Каждый из 8 виджетов независим: собственный трансформ-пайплайн (unnest → unpivot → compute → filter → group-by → aggregate → pivot + cross-project join). **DataTable**-виджет поддерживает sub-базы — именованные вкладки с независимой фильтрацией одного набора данных. **ViewPort**-виджет встраивает Таблицу, Доску, Календарь или Галерею внутрь канваса. Два режима раскладки: вертикальный стек и адаптивная 12-колоночная сетка.
...
| ✅ | **Dashboard View** — Database View migrated: базы данных внутри Dashboard canvas | v3.4.0 | Выпущено |
| 🚧 | **Dashboard Engine** — relations, rollup, formula, sub-bases, Dataview adaptive bridge | 3.5.X WIP | В разработке |

> Архитектура V5 — в [docs/internal/MASTER_MAP_V5.md](docs/internal/MASTER_MAP_V5.md).
...
| [Архитектура V5](docs/ARCHITECTURE_V5.md) | **Старт для контрибьюторов** — 4-слойная архитектура, контракты, инварианты |
| [V5 Roadmap](docs/internal/MASTER_MAP_V5.md) | Фазы V5, backlog, карта документов |
