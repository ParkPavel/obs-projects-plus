...
- Use Obsidian built-in UI elements where possible (Notice, Menu via contextMenu helper).
- Dark/light theme compatibility.

**Technical constraints**:
- PX-budget within the `PX_BUDGET` constant in `src/__tests__/R0_3_pxBudget.test.ts` total px values in codebase.
- All new values in `rem`.
- CSS tokens file: `styles.css` (hand-maintained, merged by `esbuild.config.mjs::mergeCSS()`).
...
- A new token is introduced (`--ppp-*` naming must be user-approved for consistency).
- An existing interaction pattern is changed (keyboard, modal behavior).
...
<list any user-facing docs that need updating: tooltips, settings descriptions, help text>
...
- **To `senior-developer`**: if purely token values with no structural changes → state "Token spec ready — senior-developer can implement directly (no architect review needed)."
- **To user**: any new UX pattern not present in Notion or current Obsidian UI → needs user validation. Documentation updates for user-facing features → flag explicitly with proposed text.
