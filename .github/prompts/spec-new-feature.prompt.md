...
Ask the user (one question at a time via ask_user) for any of these you do not have:
...
5. **Affected layers** — Shell / UI / Engine / Data (see `AGENTS.md` Matryoshka)
...
Delegate to `semantic-analyzer` subagent with prompt:
> Read `docs/internal/specs/<feature-name>/SPEC.md`. Identify which existing invariants from `AGENTS.md` and `src.instructions.md` constrain this feature. List affected files, types, and modules. Output: `docs/internal/specs/<feature-name>/CONSTRAINTS.md` with sections: Affected Files, Touched Invariants, Risk Areas, Dependency Map.
...
- **Engine/Data/Shell layer** → delegate to `backend-architect`
- **UI/widget layer** → delegate to `frontend-architect`
...
Convert PLAN.md into ordered tasks in the session database:

```sql
INSERT INTO todos (id, title, description) VALUES
  ('<feature>-01-<task>', '<short>', '<full description with file paths and acceptance criteria>'),
  ...;
INSERT INTO todo_deps (todo_id, depends_on) VALUES (...);
```

Show the user the task list. Ask if they want to:
- (a) Convert to GitHub Issues (use `/backlog-to-issues` prompt afterwards)
- (b) Execute directly via `/run-pipeline`
- (c) Hand back for manual prioritization in `BACKLOG.md`

## Phase 5 — Implementation Handoff

Do NOT call `senior-developer` from within this prompt. The user runs `/run-pipeline` or hands the spec to a coding session. This prompt's job ends at "spec + plan + tasks ready".
