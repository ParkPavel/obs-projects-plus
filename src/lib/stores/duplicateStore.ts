    ...
    init(source: ProcessedRecord, existingData: ProcessedCalendarData): void {
      ...
      if (source.startDate) {
        ...
        for (let i = 0; i < spanDays; i++) {
          const dateKey =
            formatDateForInternal(source.startDate.add(i, "day")) ??
            source.startDate.add(i, "day").format("YYYY-MM-DD");
      ...
      // Вычисляем коллизии
      const collisions = detectCollisionsForPhantoms(
        phantoms,
        existingData.grouped
      );
