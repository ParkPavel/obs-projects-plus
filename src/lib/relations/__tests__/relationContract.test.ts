...
describe("relationContract", () => {
  it("adapts persisted configuration without adding inverse write authority", () => {
    expect(
      adaptRelationFieldConfig("sessions", "client", {
        targetProjectId: "clients",
        displayField: "name",
        inverseFieldName: "sessions",
      })
    ).toEqual({
      source: { projectId: "sessions", fieldName: "client" },
      target: { projectId: "clients", displayField: "name" },
      storage: "wikilink",
      inverse: { fieldName: "sessions", createIfMissing: false },
    });
  });
  ...
  it("resolves paths before legacy basenames and preserves aliases and repeats", () => {
    const index = buildRelationTargetIndex(
      frame([
        { id: "Clients/Acme.md", values: { name: "Acme Studio" } },
        { id: "Archive/Acme.md", values: { name: "Old Acme" } },
      ]),
      ["name"]
    );
    const results = resolveRelationValue(
      ["[[Clients/Acme|ACME]]", "[[Missing]]", "[[Clients/Acme]]"],
      index
    );
    expect(results).toEqual([
      {
        rawLink: "[[Clients/Acme|ACME]]",
        canonicalPath: "Clients/Acme",
        status: "resolved",
        targetRecordId: "Clients/Acme.md",
      },
      { rawLink: "[[Missing]]", canonicalPath: "Missing", status: "unmatched" },
      {
        rawLink: "[[Clients/Acme]]",
        canonicalPath: "Clients/Acme",
        status: "resolved",
        targetRecordId: "Clients/Acme.md",
      },
    ]);
    expect(resolvedRecords(results, index).map((record) => record.id)).toEqual([
      "Clients/Acme.md",
      "Clients/Acme.md",
    ]);
  });
  ...
  it("reports duplicate basename and display fallbacks as ambiguous", () => {
    const index = buildRelationTargetIndex(
      frame([
        { id: "One/Acme.md", values: { name: "Shared" } },
        { id: "Two/Acme.md", values: { name: "Shared" } },
      ]),
      ["name"]
    );
    expect(resolveRelationValue("[[Acme]]", index)[0]?.status).toBe(
      "ambiguous"
    );
    expect(resolveRelationValue("[[Shared]]", index)[0]?.status).toBe(
      "ambiguous"
    );
  });
  ...
  it("normalizes headings, aliases and null values without losing the raw link", () => {
    const index = buildRelationTargetIndex(
      frame([{ id: "Clients/Acme.md", values: { name: "Acme" } }])
    );
    expect(
      resolveRelationValue("[[Clients/Acme#Contacts|Acme contact]]", index)
    ).toEqual([
      {
        rawLink: "[[Clients/Acme#Contacts|Acme contact]]",
        canonicalPath: "Clients/Acme",
        status: "resolved",
        targetRecordId: "Clients/Acme.md",
      },
  ...
  describe("validateLegacyLinkedSelection", () => {
    ...
    it("returns all four legacy diagnostics without importing Dashboard types", () => {
      expect(
        validateLegacyLinkedSelection(undefined, "sessions", "clients", [
          relationField,
        ]).status
      ...
      expect(
        validateLegacyLinkedSelection(
          ...
          "sessions",
          "clients",
          [{ ...relationField, name: "status", type: DataFieldType.Status }]
        ).status
      ).toBe("invalid-field");
      expect(
        validateLegacyLinkedSelection(
