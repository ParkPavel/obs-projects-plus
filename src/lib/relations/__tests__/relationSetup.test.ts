...
describe("relation setup preview", () => {
  test("uses the canonical resolver and never changes source values", () => {
    ...
    expect(preview[1]?.resolutions.map((value) => value.status)).toEqual([
      "unmatched",
      "ambiguous",
    ]);
    expect(source.records[0]?.values["client"]).toBe("[[clients/Ada]]");
    expect(summarizeRelationPreview(preview)).toEqual({
      resolved: 1,
      unmatched: 1,
      ambiguous: 1,
    });
  ...
  test("requires explicit schema choices and only persists opted-in inverse", () => {
    ...
    expect(
      toRelationFieldConfig({
        ...
        displayField: "name",
        createSourceField: false,
        inverse: { enabled: true, fieldName: "sessions" },
      })
    ).toEqual({
      targetProjectId: "clients",
      displayField: "name",
    ...
    expect(
      toRelationFieldConfig({
        ...
        targetProjectId: "clients",
        createSourceField: false,
        inverse: { enabled: false, fieldName: "sessions" },
      })
    ).toEqual({ targetProjectId: "clients" });
  });
});
