...
describe("evaluateFormula — comparisons", () => {
  it("field = string (exact match)", () => {
    ...
    expect(
      evaluateFormula(node, makeRecord({ status: "Done" }), BASE_DATE)
    ).toBe(false);
  });
  ...
  it("field > number", () => {
    ...
    expect(evaluateFormula(node, makeRecord({ priority: 5 }), BASE_DATE)).toBe(
      true
    );
    expect(evaluateFormula(node, makeRecord({ priority: 2 }), BASE_DATE)).toBe(
      false
    );
  });
  ...
  it("field >= and <= number", () => {
    ...
    expect(evaluateFormula(ge, makeRecord({ score: 10 }), BASE_DATE)).toBe(
      true
    );
    expect(evaluateFormula(le, makeRecord({ score: 10 }), BASE_DATE)).toBe(
      true
    );
    expect(evaluateFormula(ge, makeRecord({ score: 9 }), BASE_DATE)).toBe(
      false
    );
    expect(evaluateFormula(le, makeRecord({ score: 11 }), BASE_DATE)).toBe(
      false
    );
  });
  ...
  it("field = boolean", () => {
    ...
    expect(evaluateFormula(node, makeRecord({ done: true }), BASE_DATE)).toBe(
      true
    );
    expect(evaluateFormula(node, makeRecord({ done: false }), BASE_DATE)).toBe(
      false
    );
  });
...
describe("evaluateFormula — AND / OR / NOT", () => {
  it("AND — all true", () => {
    ...
    expect(
      evaluateFormula(
        node,
        makeRecord({ status: "Active", priority: 5 }),
        BASE_DATE
      )
    ).toBe(true);
  });
  ...
  it("AND — one false", () => {
    ...
    expect(
      evaluateFormula(
        node,
        makeRecord({ status: "Active", priority: 1 }),
        BASE_DATE
      )
    ).toBe(false);
  });
  ...
  it("OR — none true", () => {
    const node = parseFormula('OR(status = "Active", status = "Done")');
    expect(
      evaluateFormula(node, makeRecord({ status: "Pending" }), BASE_DATE)
    ).toBe(false);
  });
...
describe("evaluateFormula — string functions", () => {
  it("CONTAINS", () => {
    ...
    expect(
      evaluateFormula(node, makeRecord({ title: "Say Hello World" }), BASE_DATE)
    ).toBe(true);
    expect(
      evaluateFormula(node, makeRecord({ title: "Goodbye" }), BASE_DATE)
    ).toBe(false);
  });
  ...
  it("STARTS_WITH (case-sensitive)", () => {
    ...
    expect(
      evaluateFormula(node, makeRecord({ title: "Hello World" }), BASE_DATE)
    ).toBe(true);
    expect(
      evaluateFormula(node, makeRecord({ title: "Say Hello" }), BASE_DATE)
    ).toBe(false);
    // different case → false
    const node2 = parseFormula('STARTS_WITH(title, "hello")');
    expect(
      evaluateFormula(node2, makeRecord({ title: "Hello World" }), BASE_DATE)
    ).toBe(false);
  });
  ...
  it("IS_EMPTY", () => {
    ...
    expect(evaluateFormula(node, makeRecord({ notes: "" }), BASE_DATE)).toBe(
      true
    );
    expect(evaluateFormula(node, makeRecord({ notes: null }), BASE_DATE)).toBe(
      true
    );
    expect(evaluateFormula(node, makeRecord({}), BASE_DATE)).toBe(true);
    expect(
      evaluateFormula(node, makeRecord({ notes: "text" }), BASE_DATE)
    ).toBe(false);
  });
...
describe("evaluateFormula — date functions", () => {
  ...
  it("IS_TODAY", () => {
    ...
    expect(evaluateFormula(node, makeRecord({ due: today }), BASE_DATE)).toBe(
      true
    );
    expect(
      evaluateFormula(node, makeRecord({ due: yesterday }), BASE_DATE)
    ).toBe(false);
  });
  ...
  it("IS_OVERDUE", () => {
    ...
    expect(
      evaluateFormula(node, makeRecord({ due: pastDate }), BASE_DATE)
    ).toBe(true);
    expect(
      evaluateFormula(node, makeRecord({ due: futureDate }), BASE_DATE)
    ).toBe(false);
  });
  ...
  it("IS_ON_AND_BEFORE", () => {
    ...
    expect(
      evaluateFormula(node, makeRecord({ due: tomorrow }), BASE_DATE)
    ).toBe(false);
  });
  ...
  it("IS_THIS_MONTH", () => {
    ...
    expect(
      evaluateFormula(node, makeRecord({ due: "2026-05-15" }), BASE_DATE)
    ).toBe(false);
  });

  it("returns false for null/empty date fields", () => {
    const node = parseFormula("IS_TODAY(due)");
    expect(evaluateFormula(node, makeRecord({ due: null }), BASE_DATE)).toBe(
      false
    );
...
describe("evaluateFormula — DATE_ADD / DATE_SUB", () => {
  it("DATE_ADD adds days", () => {
    const node = parseFormula(
      'IS_ON_AND_AFTER(due, DATE_ADD("2026-04-10", 5, "day"))'
    );
    // 2026-04-10 + 5 days = 2026-04-15; due=2026-04-20 >= 2026-04-15
    expect(
      evaluateFormula(node, makeRecord({ due: "2026-04-20" }), BASE_DATE)
    ).toBe(true);
    expect(
      evaluateFormula(node, makeRecord({ due: "2026-04-10" }), BASE_DATE)
    ).toBe(false);
  });

  it("DATE_SUB subtracts days", () => {
    const node = parseFormula(
      'IS_BEFORE(due, DATE_SUB("2026-04-10", 3, "day"))'
    );
    // 2026-04-10 - 3 = 2026-04-07; due=2026-04-01 < 2026-04-07
    expect(
      evaluateFormula(node, makeRecord({ due: "2026-04-01" }), BASE_DATE)
    ).toBe(true);
    expect(
      evaluateFormula(node, makeRecord({ due: "2026-04-09" }), BASE_DATE)
    ).toBe(false);
  });
});
...
describe("evaluateFormula — HAS_ANY_OF / HAS_ALL_OF / HAS_NONE_OF", () => {
  it("HAS_ANY_OF returns true when at least one matches", () => {
    ...
    expect(
      evaluateFormula(node, makeRecord({ tags: ["bug", "feature"] }), BASE_DATE)
    ).toBe(true);
    expect(
      evaluateFormula(
        node,
        makeRecord({ tags: ["feature", "docs"] }),
        BASE_DATE
      )
    ).toBe(false);
  });
  ...
  it("HAS_ALL_OF returns true when all match", () => {
    ...
    expect(
      evaluateFormula(
        node,
        makeRecord({ tags: ["bug", "urgent", "critical"] }),
        BASE_DATE
      )
    ).toBe(true);
    expect(
  ...
  it("HAS_NONE_OF returns true when none match", () => {
    ...
    expect(
      evaluateFormula(node, makeRecord({ tags: ["bug", "docs"] }), BASE_DATE)
    ).toBe(false);
  });
  ...
  it("returns false for non-array fields", () => {
    ...
    expect(
      evaluateFormula(node, makeRecord({ title: "a string" }), BASE_DATE)
    ).toBe(false);
  });
});
...
describe("evaluateFormula — arithmetic", () => {
  it("field + number", () => {
    ...
    expect(evaluateFormula(node, makeRecord({ priority: 4 }), BASE_DATE)).toBe(
      false
    );
  });
  ...
  it("field * number", () => {
    ...
    expect(evaluateFormula(node, makeRecord({ score: 5 }), BASE_DATE)).toBe(
      true
    );
  });
});
...
describe("evaluateFormula — edge cases", () => {
  ...
  it("null field = NULL is true", () => {
    ...
    expect(evaluateFormula(node, makeRecord({ field: null }), BASE_DATE)).toBe(
      true
    );
  });
  ...
  it("complex nested formula", () => {
    const node = parseFormula(
      'AND(OR(status = "Active", status = "InProgress"), priority >= 3)'
    );
    expect(
      evaluateFormula(
        node,
        makeRecord({ status: "InProgress", priority: 4 }),
        BASE_DATE
      )
    ).toBe(true);
    expect(
      evaluateFormula(
        node,
        makeRecord({ status: "Done", priority: 5 }),
        BASE_DATE
      )
    ).toBe(false);
  });
...
describe("validateFormula", () => {
  ...
  it("returns error for unknown field", () => {
    const errors = validateFormula('unknown_field = "Active"', [
      "status",
      "priority",
    ]);
