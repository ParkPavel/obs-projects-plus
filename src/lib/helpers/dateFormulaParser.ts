 ...
 */
export function parseDateFormula(
  formula: string,
  baseDate?: Dayjs
): DateFormulaResult {
  ...
  if (!canonical) {
    const supported =
      "today, tomorrow, yesterday, sow, eow, som, eom, soy, eoy";
    return {
      success: false,
      error: `Unknown keyword: "${rawKeyword}". Supported: ${supported}`,
    };
  }
  ...
  const amount = amountStr
    ? parseInt(amountStr, 10) * (sign === "-" ? -1 : 1)
    : 0;
  ...
  try {
    switch (canonical) {
      ...
      default:
        return {
          success: false,
          error: `Internal error: unmapped canonical "${canonical}"`,
        };
    }
