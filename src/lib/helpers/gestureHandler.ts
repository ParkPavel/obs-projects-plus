...
function remToPx(rem: number): number {
  ...
  const rootFontSize = parseFloat(
    getComputedStyle(activeDocument.documentElement).fontSize
  );
...
if (typeof window !== "undefined") {
  const updateCapabilities = () =>
    deviceCapabilities.set(detectDeviceCapabilities());
  window
    .matchMedia("(pointer: fine)")
    .addEventListener("change", updateCapabilities);
  window
 ...
 */
export function createEventGestureHandler(
