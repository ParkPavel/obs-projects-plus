...
describe("detectLinkable", () => {
  ...
  test("detects http/https URLs", () => {
    expect(detectLinkable("https://example.com")).toEqual({
      kind: "url",
      href: "https://example.com",
    });
    expect(detectLinkable("http://example.com/path?q=1")).toEqual({
      kind: "url",
  ...
  test("detects mailto and tel as email/phone", () => {
    expect(detectLinkable("mailto:a@b.io")).toEqual({
      kind: "email",
      href: "mailto:a@b.io",
    });
    expect(detectLinkable("tel:+15551234")).toEqual({
      kind: "phone",
      href: "tel:+15551234",
    });
  });
  ...
  test("normalises bare emails to mailto:", () => {
    expect(detectLinkable("user@example.com")).toEqual({
      kind: "email",
  ...
  test("normalises phone numbers to tel: with stripped formatting", () => {
    expect(detectLinkable("+1 (555) 123-4567")).toEqual({
      kind: "phone",
      href: "tel:+15551234567",
    });
    expect(detectLinkable("555-123-456")).toEqual({
      kind: "phone",
      href: "tel:555123456",
    });
  });
  ...
  test("trims surrounding whitespace before matching", () => {
    expect(detectLinkable("  https://x.io  ")).toEqual({
      kind: "url",
      href: "https://x.io",
    });
  });
