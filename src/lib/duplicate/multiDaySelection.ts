...
export function handleMultiDaySelection(
  ...
  for (let i = 0; i < spanDays; i++) {
    spanDates.push(
      formatDateForInternal(clickedDate.add(i, "day")) ??
        clickedDate.add(i, "day").format("YYYY-MM-DD")
    );
  }
  ...
  if (spanDays > 1) {
    ...
    for (let offset = 0; offset < spanDays; offset++) {
      ...
      for (let i = 0; i < spanDays; i++) {
        testSpan.push(
          formatDateForInternal(potentialStart.add(i, "day")) ??
            potentialStart.add(i, "day").format("YYYY-MM-DD")
        );
      }
    ...
    const clickedDateKey =
      formatDateForInternal(clickedDate) ?? clickedDate.format("YYYY-MM-DD");
...
export function isDateInSelectedSpan(
  ...
  for (let i = 1; i < spanDays; i++) {
    const potentialStart =
      formatDateForInternal(date.subtract(i, "day")) ??
    ...
    if (selectedDates.has(potentialStart)) {
      ...
      for (let j = 0; j < spanDays; j++) {
        const spanDate =
          formatDateForInternal(date.subtract(i - j, "day")) ??
          date.subtract(i - j, "day").format("YYYY-MM-DD");
