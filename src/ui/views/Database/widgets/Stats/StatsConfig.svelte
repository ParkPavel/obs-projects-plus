...
<WidgetConfigShell
  title={$i18n.t("views.database.stats.config.title", { defaultValue: "Stats cards" })}
  subtitle={$i18n.t("views.database.stats.config.subtitle", { defaultValue: "Each card shows one aggregation from the (post-pipeline) data. Use * as field for Count of records." })}
  on:close={() => dispatch("close")}
>
  <div class="ppp-cfg-row">
    <label>
      {$i18n.t("views.database.stats.config.columns", { defaultValue: "Grid columns" })}
  ...
  {#if cards.length === 0}
    <p class="ppp-stats-config-hint">
      {$i18n.t("views.database.stats.config.empty", { defaultValue: "No cards yet. Add one to show a KPI tile." })}
    </p>
  {/if}
  ...
  <div class="ppp-cfg-list">
    {#each cards as card, idx (card.id)}
      <div class="ppp-cfg-item ppp-stats-config-item">
        <label>
          {$i18n.t("views.database.stats.config.label", { defaultValue: "Label" })}
          <input
            type="text"
            value={card.label}
        ...
        <label>
          {$i18n.t("views.database.stats.config.field", { defaultValue: "Field" })}
          <input
            type="text"
        ...
        <label>
          {$i18n.t("views.database.stats.config.aggregation", { defaultValue: "Aggregation" })}
          <select
          ...
          >
            {#each AGGREGATIONS as agg (agg.value)}
              <option value={agg.value}>
                {$i18n.t("views.database.agg." + agg.value, { defaultValue: agg.defaultLabel })}
              </option>
            {/each}
          </select>
        </label>
        ...
        <label>
          {$i18n.t("views.database.stats.config.format", { defaultValue: "Format" })}
          <select
          ...
          >
            {#each FORMATS as fmt (fmt)}
              <option value={fmt}>
                {$i18n.t("views.database.stats.config.fmt-" + fmt, { defaultValue: fmt })}
              </option>
            {/each}
          </select>
        </label>
        ...
        {#if card.format === "currency"}
          <label class="ppp-stats-currency">
            {$i18n.t("views.database.stats.config.currency", { defaultValue: "Symbol" })}
            <input
              type="text"
        ...
        <label class="ppp-stats-sparkline">
          ...
          />
          <span>{$i18n.t("views.database.stats.config.sparkline", { defaultValue: "Sparkline" })}</span>
        </label>
  ...
  <button type="button" class="ppp-cfg-add" on:click={addCard}>
    + {$i18n.t("views.database.stats.config.add", { defaultValue: "Add card" })}
  </button>
