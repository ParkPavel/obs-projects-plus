...
<script lang="ts">
  ...
  function handleSaveNew() {
    const label = window
      .prompt($i18n.t("views.database.field-presets.prompt-new"), "")
      ?.trim();
    if (!label) {
      close();
      return;
  ...
  function handleRename(preset: FieldPreset) {
    const label = window
      .prompt($i18n.t("views.database.field-presets.prompt-rename"), preset.label)
      ?.trim();
  ...
  function handleDelete(preset: FieldPreset) {
    const ok = window.confirm(
      $i18n.t("views.database.field-presets.confirm-delete", {
...
<div class="ppp-fp-root">
  <button
    ...
    aria-haspopup="menu"
    aria-expanded={open}
    aria-label={$i18n.t("views.database.field-presets.aria-label")}
    title={$i18n.t("views.database.field-presets.title")}
  >
    <span aria-hidden="true">⋮⋮</span>
    <span class="ppp-fp-trigger-label">
      {activeId
        ? presets.find((p) => p.id === activeId)?.label ??
          $i18n.t("views.database.field-presets.label")
        : $i18n.t("views.database.field-presets.label")}
    </span>
  </button>
  ...
  {#if open}
    ...
    <div class="ppp-fp-popover" role="menu">
      ...
      >
        <span aria-hidden="true">＋</span>
        {$i18n.t("views.database.field-presets.save-current")}
      </button>
      ...
      {#if presets.length > 0}
        ...
        {#each presets as preset (preset.id)}
          <div class="ppp-fp-row" role="menuitem">
            <button
              ...
              title={$i18n.t("views.database.field-presets.apply")}
            >
            ...
            <button
              ...
              title={$i18n.t("views.database.field-presets.rename")}
              aria-label={$i18n.t("views.database.field-presets.rename")}
            >
              <span aria-hidden="true">✎</span>
            </button>
            <button
              ...
              title={$i18n.t("views.database.field-presets.delete")}
              aria-label={$i18n.t("views.database.field-presets.delete")}
            >
              <span aria-hidden="true">✕</span>
            </button>
          </div>
        ...
        {#if activeId !== undefined}
          ...
          >
            {$i18n.t("views.database.field-presets.detach")}
          </button>
        {/if}
      {/if}
    </div>
