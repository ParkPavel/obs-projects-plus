...
<div
  ...
  role="region"
  aria-label={$i18n.t("views.database.formula.debug-title")}
  ...
  <div class="ppp-formula-debug-header">
    ...
    >
      ...
      <span class="ppp-formula-debug-title">
        {$i18n.t("views.database.formula.debug-title")} — {$i18n.t("views.database.formula.runtime-error")}
      </span>
    </button>
    <div class="ppp-formula-debug-actions">
      <button class="ppp-formula-debug-btn" type="button" on:click={copyError}>
        {$i18n.t("views.database.formula.debug-copy")}
      </button>
      <button
      ...
      >
        {$i18n.t("views.database.formula.debug-dismiss")}
      </button>
    </div>
  </div>
  {#if expanded}
    <div class="ppp-formula-debug-body">
      ...
      {#if cursorPosition >= 0}
        <div class="ppp-formula-debug-cursor">
          {$i18n.t("views.database.formula.debug-cursor")}: {cursorPosition}
        </div>
      {/if}
    </div>
  {/if}
