...
<WidgetConfigShell
  title={$i18n.t("views.database.comparison.config.title", { defaultValue: "Comparison metrics" })}
  subtitle={$i18n.t("views.database.comparison.config.subtitle", { defaultValue: "Each metric sums one numeric field across all records. Add two or more to compare." })}
  on:close={() => dispatch("close")}
>
  <div class="ppp-cfg-row">
    <label>
      {$i18n.t("views.database.comparison.config.mode", { defaultValue: "Mode" })}
      <select value={mode} on:change={onModeChange}>
        <option value="absolute">{$i18n.t("views.database.comparison.config.mode-absolute", { defaultValue: "Absolute" })}</option>
        <option value="percentage">{$i18n.t("views.database.comparison.config.mode-percentage", { defaultValue: "Percentage" })}</option>
        <option value="normalized">{$i18n.t("views.database.comparison.config.mode-normalized", { defaultValue: "Normalized" })}</option>
      </select>
    </label>

    <label>
      {$i18n.t("views.database.comparison.config.orientation", { defaultValue: "Orientation" })}
      <select value={orientation} on:change={onOrientationChange}>
        <option value="horizontal">{$i18n.t("views.database.comparison.config.orient-horizontal", { defaultValue: "Horizontal" })}</option>
        <option value="vertical">{$i18n.t("views.database.comparison.config.orient-vertical", { defaultValue: "Vertical" })}</option>
      </select>
    </label>
    ...
    <label class="ppp-cmp-delta">
      ...
      />
      <span>{$i18n.t("views.database.comparison.config.show-delta", { defaultValue: "Show delta" })}</span>
    </label>
  </div>

  {#if metrics.length === 0}
    <p class="ppp-cmp-hint">
      {$i18n.t("views.database.comparison.config.empty", { defaultValue: "No metrics yet. Add one to pick a numeric field to compare." })}
    </p>
  {/if}
  ...
  <div class="ppp-cfg-list">
    {#each metrics as m, idx (idx)}
      <div class="ppp-cfg-item">
        <label>
          {$i18n.t("views.database.comparison.config.field", { defaultValue: "Field" })}
          <input
            type="text"
            list="ppp-cmp-fields"
            value={m.field}
        ...
        <label>
          {$i18n.t("views.database.comparison.config.label", { defaultValue: "Label" })}
          <input
            type="text"
            value={m.label ?? ""}
            placeholder={m.field}
        ...
        <label class="ppp-cmp-color">
          {$i18n.t("views.database.comparison.config.color", { defaultValue: "Color" })}
          <input
            type="color"
  ...
  <button type="button" class="ppp-cfg-add" on:click={addMetric}>
    + {$i18n.t("views.database.comparison.config.add", { defaultValue: "Add metric" })}
  </button>
