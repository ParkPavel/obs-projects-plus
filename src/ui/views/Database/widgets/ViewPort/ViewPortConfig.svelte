...
<WidgetConfigShell
  title={$i18n.t("views.database.viewport.config.title", { defaultValue: "Embedded view" })}
  subtitle={$i18n.t("views.database.viewport.config.subtitle", { defaultValue: "Pick which saved view renders inside this widget." })}
  on:close={() => dispatch("close")}
>
  <div class="ppp-cfg-list">
    <div class="ppp-cfg-item">
      <label class="ppp-viewport-config__field">
        {$i18n.t("views.database.viewport.config.view", { defaultValue: "View" })}
        <div class="ppp-viewport-config__picker">
          ...
          >
            {viewId
              ? (viewLabel || viewId)
              : $i18n.t("views.database.viewport.select", { defaultValue: "Pick a view…" })}
    ...
    <div class="ppp-cfg-item">
      <label class="ppp-viewport-config__field">
        {$i18n.t("views.database.viewport.config.label", { defaultValue: "Label override" })}
        <input
          type="text"
          value={viewLabel}
          placeholder={$i18n.t("views.database.viewport.config.label-placeholder", { defaultValue: "Shown in widget header" })}
    ...
    <div class="ppp-cfg-item">
      <label class="ppp-viewport-config__toggle">
        ...
        <span>{$i18n.t("views.database.viewport.config.header-visible", { defaultValue: "Show inline header" })}</span>
      </label>
    </div>
  </div>
</WidgetConfigShell>
