...
const SelectionBadge = require("../SelectionBadge.svelte").default;
const { shouldShowSelectionBadge } = require("../SelectionBadge.svelte");
...
const externalSelection = (field: string, value: string): SelectionState => ({
...
describe("shouldShowSelectionBadge — visibility predicate (#044.5)", () => {
  it("hides badge when selection is empty", () => {
    expect(
      shouldShowSelectionBadge({ type: "stats", id: "w1" }, EMPTY_SELECTION)
    ).toBe(false);
  });
  ...
  it("self-skip: hides badge when DataTable widget is itself the driver", () => {
    ...
    expect(
      shouldShowSelectionBadge({ type: "data-table", id: "w1" }, ownSelection)
    ).toBe(false);
  });
  ...
  it("self-skip: hides badge when Stats widget would emit its own selection (v2 driver)", () => {
    ...
    expect(
      shouldShowSelectionBadge({ type: "stats", id: "w1" }, ownSelection)
    ).toBe(false);
  });
  ...
  it("widget-scoped: another DataTable still shows the badge for a sibling's emission", () => {
    ...
    expect(
      shouldShowSelectionBadge({ type: "data-table", id: "w1" }, otherSelection)
    ).toBe(true);
  });
});
...
describe("SelectionBadge.svelte — component (#044.5)", () => {
  it("renders the field label and value text", () => {
    ...
    try {
      const badge = m.target.querySelector(
  ...
  it("sets the title attribute to 'field: value' for hover tooltip (spec §6.2)", () => {
    ...
    try {
      const badge = m.target.querySelector<HTMLSpanElement>(
        '[data-testid="ppp-selection-badge"]'
      );
      expect(badge?.getAttribute("title")).toBe(
        "tags: Some long category label"
      );
    } finally {
      m.destroy();
    }
  ...
  it("dispatches a 'clear' event when the ✕ button is clicked", () => {
    ...
    try {
      ...
      const clearBtn = m.target.querySelector<HTMLButtonElement>(
