<script lang="ts">
  ...
  // -- R2.1b: Column-header context menu Ч property-type override + rollup-mode picker
  ...
  function buildExtraColumnMenuEntries(column: GridColDef): ContextMenuEntry[] {
    ...
    // Rollup mode picker Ч only when the field is configured as a rollup.
  ...
  // R2.2 Ч sub-base partitioning (applied before sort/group)
  ...
  $: columns = sortedFields
    ...
    .map<GridColDef>((field) => {
      ...
      // Stage A.9 fix: hide derived `path` column by default Ч it duplicates
  ...
  // R2.2 Ч subscribe to global `add-sub-base` command. Each DataTable
...
<div class="ppp-datatable-widget">
  ...
  {#if showAggregation}
    <div class="ppp-aggregation-row">
      <div class="ppp-aggregation-cell ppp-aggregation-cell--row-header" role="rowheader" aria-label={$i18n.t("views.dashboard.aggregation.row-label", { defaultValue: "Aggregation row Ч click a cell to pick a function" })} title={$i18n.t("views.dashboard.aggregation.row-label", { defaultValue: "Aggregation row Ч click a cell to pick a function" })}>
  ...
  {#if !isReadonly}
    <!--
      ...
      produced visible duplicate ("+ ƒобавить заметку" from DataGrid,
      then "+ Ќова€ заметка" from this wrapper). DataGrid is the single
      source of truth.
    -->
  {/if}
  </div>
