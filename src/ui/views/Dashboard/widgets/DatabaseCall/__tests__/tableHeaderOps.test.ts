...
describe("tableHeaderOps (F2.4)", () => {
  it("applySortPatch replaces criteria and clears legacy sortField", () => {
    const next = applySortPatch(
      { sortField: "old", sortAsc: true } as DataTableConfig,
      "mrr",
      "desc"
    );
  ...
  it("applyHidePatch hides a field preserving other field config", () => {
    const next = applyHidePatch(
      { fieldConfig: { mrr: { widthRem: 8 } } } as DataTableConfig,
      "mrr"
    );
  ...
  it("applyWidthPatch clamps to ≥4rem and rounds to quarter-rem", () => {
    expect(
      applyWidthPatch(undefined, "x", 1).fieldConfig?.["x"]?.widthRem
    ).toBe(4);
    expect(
      applyWidthPatch(undefined, "x", 8.13).fieldConfig?.["x"]?.widthRem
    ).toBe(8.25);
  });
  ...
  it("calculateOptions offers numeric aggregations only for numeric types", () => {
    expect(calculateOptions(field("mrr", DataFieldType.Number))).toContain(
      "sum"
    );
    expect(calculateOptions(field("name", DataFieldType.String))).not.toContain(
      "sum"
    );
  });
