<script lang="ts">
  ...
  const AGGREGATIONS: { value: ColumnAggregation; labelKey: string; defaultLabel: string }[] = [
    ...
    { value: "median", labelKey: "views.dashboard.agg.median", defaultLabel: "Median" },
    { value: "min", labelKey: "views.dashboard.agg.min", defaultLabel: "Min" },
    { value: "max", labelKey: "views.dashboard.agg.max", defaultLabel: "Max" },
    { value: "range", labelKey: "views.dashboard.agg.range", defaultLabel: "Range" },
    { value: "count_checked", labelKey: "views.dashboard.agg.checked", defaultLabel: "✓ Count" },
    { value: "percent_checked", labelKey: "views.dashboard.agg.pct-checked", defaultLabel: "% ✓" },
    { value: "percent_not_empty", labelKey: "views.dashboard.agg.pct-filled", defaultLabel: "% filled" },
  ];
...
<WidgetConfigShell
  title={$i18n.t("views.dashboard.summary.config.title", { defaultValue: "Summary columns" })}
  subtitle={$i18n.t("views.dashboard.summary.config.subtitle", { defaultValue: "Pick fields and aggregations. Values recompute live after any data/pipeline change." })}
  on:close={() => dispatch("close")}
>
  {#if columns.length === 0}
    <p class="ppp-summary-config-hint">
      {$i18n.t("views.dashboard.summary.config.empty", { defaultValue: "No columns yet. Add one to show an aggregated number next to the records." })}
    </p>
  {/if}
  ...
  <div class="ppp-cfg-list">
    {#each columns as col, idx (idx)}
      <div class="ppp-cfg-item">
        <label>
          {$i18n.t("views.dashboard.summary.config.field", { defaultValue: "Field" })}
          <input
            type="text"
        ...
        <label>
          {$i18n.t("views.dashboard.summary.config.aggregation", { defaultValue: "Aggregation" })}
          <select
        ...
        <label>
          {$i18n.t("views.dashboard.summary.config.format", { defaultValue: "Format" })}
          <select
          ...
          >
            {#each FORMATS as fmt (fmt)}
              <option value={fmt}>
                {$i18n.t("views.dashboard.summary.config.fmt-" + fmt, { defaultValue: fmt })}
              </option>
            {/each}
          </select>
        </label>
        ...
        {#if col.format === "currency"}
          <label class="ppp-cfg-currency">
            {$i18n.t("views.dashboard.summary.config.currency", { defaultValue: "Symbol" })}
            <input
              type="text"
  ...
  <button type="button" class="ppp-cfg-add" on:click={addColumn}>
    + {$i18n.t("views.dashboard.summary.config.add", { defaultValue: "Add column" })}
  </button>
