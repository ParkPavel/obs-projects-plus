<script lang="ts">
  ...
  function aggLabel(agg: ColumnAggregation): string {
    const t = $i18n.t;
    switch (agg) {
      case "sum": return t("views.dashboard.chart.aggregations.sum", { defaultValue: "Sum" });
      case "avg": return t("views.dashboard.chart.aggregations.average", { defaultValue: "Avg" });
      case "min": return t("views.dashboard.chart.aggregations.min", { defaultValue: "Min" });
      case "max": return t("views.dashboard.chart.aggregations.max", { defaultValue: "Max" });
      case "median": return t("views.dashboard.chart.aggregations.median", { defaultValue: "Median" });
      case "count": return t("views.dashboard.chart.aggregations.count", { defaultValue: "Count" });
      case "count_values": return t("views.dashboard.chart.aggregations.count", { defaultValue: "Count" });
      case "count_unique": return t("views.dashboard.chart.aggregations.unique", { defaultValue: "Unique" });
...
<div class="ppp-summary-widget">
  ...
  <div class="ppp-summary-rules" title={$i18n.t("views.dashboard.widget.rules-hint", { defaultValue: "Rules that drive this widget" })}>
    <span class="ppp-summary-rules__item">
      <strong>{source.records.length}</strong>
      {$i18n.t("views.dashboard.widget.records", { defaultValue: "records" })}
    </span>
    <span class="ppp-summary-rules__sep">·</span>
    <span class="ppp-summary-rules__item">
      {$i18n.t("views.dashboard.widget.pipeline-steps", { defaultValue: "pipeline steps" })}: <strong>{pipelineSteps}</strong>
    </span>
    <span class="ppp-summary-rules__sep">·</span>
    <span class="ppp-summary-rules__item">
      {$i18n.t("views.dashboard.widget.columns", { defaultValue: "columns" })}: <strong>{columns.length}</strong>
    </span>
  </div>

  {#if cells.length === 0}
    <div class="ppp-summary-empty">
      {$i18n.t("views.dashboard.widget.summary-empty", { defaultValue: "No columns configured. Open the gear icon to add aggregations." })}
    </div>
  {:else}
