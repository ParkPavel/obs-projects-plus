...
<WidgetConfigShell
  title={$i18n.t("views.dashboard.filter-tabs.config.title", { defaultValue: "Filter tabs" })}
  subtitle={$i18n.t("views.dashboard.filter-tabs.config.subtitle", { defaultValue: "Pick a field — the widget auto-builds tabs from unique values. Or define tabs manually for a fixed set." })}
  on:close={() => dispatch("close")}
>
  <div class="ppp-cfg-row">
    <label>
      {$i18n.t("views.dashboard.filter-tabs.config.field", { defaultValue: "Filter field" })}
      <input
        type="text"
        list="ppp-ft-fields"
        value={field}
    ...
    <label class="ppp-ft-showall">
      ...
      />
      <span>{$i18n.t("views.dashboard.filter-tabs.config.show-all", { defaultValue: "Include \"All\" tab" })}</span>
    </label>
  </div>
  ...
  {#if field && preview.length > 0 && tabs.length === 0}
    <div class="ppp-ft-preview">
      <div class="ppp-ft-preview-head">
        <span>
          {$i18n.t("views.dashboard.filter-tabs.config.auto", { defaultValue: "Auto-detected values" })}
          ({preview.length})
        </span>
        <button
          type="button"
          ...
          title={$i18n.t("views.dashboard.filter-tabs.config.seed-tip", { defaultValue: "Copy values into editable tabs" })}
        >
          {$i18n.t("views.dashboard.filter-tabs.config.seed", { defaultValue: "Use as tabs" })}
        </button>
      </div>
    ...
    <div class="ppp-cfg-list">
      {#each tabs as t, idx (t.id)}
        <div class="ppp-cfg-item">
          <label>
            {$i18n.t("views.dashboard.filter-tabs.config.tab-label", { defaultValue: "Label" })}
            <input
              type="text"
              value={t.label}
          ...
          <label>
            {$i18n.t("views.dashboard.filter-tabs.config.tab-value", { defaultValue: "Value" })}
            <input
              type="text"
              value={t.value}
  ...
  <button type="button" class="ppp-cfg-add" on:click={() => addTab("")}>
    + {$i18n.t("views.dashboard.filter-tabs.config.add", { defaultValue: "Add tab" })}
  </button>
