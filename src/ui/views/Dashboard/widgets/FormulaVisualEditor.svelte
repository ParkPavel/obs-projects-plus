...
<div class="ppp-visual-editor">
  ...
  <div class="ppp-visual-canvas">
    ...
    {:else if parseError}
      <div class="ppp-visual-error">{$i18n.t("views.dashboard.formula.visual-parse-error")}</div>
    {:else}
      <button class="ppp-visual-empty" on:click={() => openPalette(null)}>
        {$i18n.t("views.dashboard.formula.visual-empty")}
      </button>
    {/if}
  </div>
  ...
  {#if showPalette}
    ...
    <div class="ppp-palette-overlay" on:click={() => { showPalette = false; }}>
      ...
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label={$i18n.t("views.dashboard.formula.palette", { defaultValue: "Formula palette" })}
      >
        ...
        <!-- Operators -->
        <div class="ppp-palette-section">
          <div class="ppp-palette-section-title">{$i18n.t("views.dashboard.formula.operators", { defaultValue: "Operators" })}</div>
          <div class="ppp-palette-chips">
        ...
        {#if fields.length > 0}
          <div class="ppp-palette-section">
            <div class="ppp-palette-section-title">{$i18n.t("views.dashboard.formula.fields", { defaultValue: "Fields" })}</div>
            <div class="ppp-palette-chips">
              {#each fields as f}
        ...
        <!-- Literal input -->
        <div class="ppp-palette-section">
          <div class="ppp-palette-section-title">{$i18n.t("views.dashboard.formula.value", { defaultValue: "Value" })}</div>
          <div class="ppp-palette-literal-row">
            <input
              class="ppp-palette-literal-input"
              type="text"
              placeholder={$i18n.t("views.dashboard.formula.enter-value", { defaultValue: "Enter value..." })}
              on:keydown={(e) => {
                if (e.key === "Enter") {
...
<style>
  ...
  .ppp-palette {
    ...
    box-shadow: var(--shadow-s, 0 0.25rem 0.75rem rgba(0,0,0,0.2));
    padding: 0.75rem;
    max-width: 28rem;
    max-height: 24rem;
    overflow-y: auto;
