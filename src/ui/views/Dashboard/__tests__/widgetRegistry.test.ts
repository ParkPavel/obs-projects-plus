...
describe("widgetRegistry", () => {
  ...
  describe("canAddWidget", () => {
    ...
    test("allows multiple data-table widgets (no maxCount cap)", () => {
      // R-phase Bug #2: users may compose several DataTable widgets in a
      // single Database view (each can carry an independent transform
      // pipeline). The cap was lifted so the toolbar no longer disables
      // the option after the first instance.
      const current = [{ type: "data-table" as const }];
      expect(canAddWidget("data-table", current)).toBe(true);
    });
