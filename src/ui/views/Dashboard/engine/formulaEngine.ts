...
/**
 * REFACTOR-103 — adapter that funnels formula args through the canonical
 * `aggregate()` kernel. Flattens array-valued args (e.g. `@column`
 * expansions), forwards numeric coercion to the kernel, and re-maps
 * empty-input semantics so existing formula contracts hold:
 *  - `sum` → 0 on empty (matches kernel)
 *  - `avg` / `min` / `max` / `median` → null on empty (formula contract)
 */
function kernelAggregate(
  ...
  // Detect "no numeric input" for non-sum ops to preserve null contract.
  if (fn !== "sum") {
...
const EXTENDED_FUNCTIONS: Record<string, FormulaFn> = {
  ...
  /**
   * MEDIAN — REFACTOR-103: delegates to `lib/engine/aggregate.ts`.
   * Preserves formula contract of returning `null` (not 0) when no
   * numeric input is found.
   */
  ...
  //
  // REFACTOR-103: SUM/AVG/MIN/MAX delegate to the canonical `aggregate()`
  // kernel in `lib/engine/aggregate.ts`. The local `kernelAggregate`
  // helper flattens arg-or-array inputs and re-maps kernel empty-input
  // semantics (kernel: 0; formula contract: null for AVG/MIN/MAX/MEDIAN,
  // 0 for SUM/COUNT) so existing tests keep passing.
