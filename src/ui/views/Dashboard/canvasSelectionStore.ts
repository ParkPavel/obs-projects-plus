...
export function createSelectionStore(
  ...
  function setSelection(next: SetSelectionInput): void {
    const op: SelectionOp =
      next.op ?? (next.values.length > 1 ? "is-any-of" : "is");
...
function composeLinkedSelectionFilter(args: {
  ...
  if (canvasSelection.source === null || canvasSelection.values.length === 0)
    return null;

  const isMasterSource =
    canvasSelection.source ===
      dataTableSourceId(linkedSelection.sourceWidgetId) ||
...
export function composeEffectiveFilter(args: {
  ...
  readonly linkedSelection?:
    | import("./types").LinkedSelectionConfig
    | undefined;
  ...
  if (linkedSelection && validationResult === "valid") {
    const linked = composeLinkedSelectionFilter({
      linkedSelection,
  ...
  if (
    ...
    if (
      selection.source !== dataTableSourceId(myWidgetId) &&
      selection.source !== chartSourceId(myWidgetId)
    ) {
      ...
      conditions.push({
        ...
        value:
          values.length === 1 ? (values[0] as string) : JSON.stringify(values),
        enabled: true,
      });
    }
  }
