...
import {
  previewRelationSetup,
  summarizeRelationPreview,
  toRelationFieldConfig,
  validateRelationSetupDraft,
  type RelationSetupDraft,
...
export function createRelationSetupController(
  deps: RelationSetupControllerDeps
) {
  ...
  async function refreshPreview(
    ...
    modal: RelationSetupModal
  ): Promise<void> {
    if (!draft.targetProjectId.trim()) {
      modal.setSummary(undefined);
      return;
    }
  ...
  async function open(initial: RelationSetupDraft): Promise<void> {
    ...
    const modal = new RelationSetupModal(
      ...
      (draft) => {
        ...
        void save(draft)
          .then(() => {
            modal.close();
            new Notice(
              deps.t("relation-setup.saved", {
                defaultValue: "Relation saved.",
              })
            );
          })
          .catch((error: unknown) => {
      ...
      (draft) => {
        void refreshPreview(source, draft, modal);
      }
    );
    modal.open();
