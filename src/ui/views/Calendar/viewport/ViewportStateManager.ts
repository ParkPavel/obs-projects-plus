...
export class ViewportStateManager {
   ...
   */
  public pushState(
    state: Omit<ViewportState, "timestamp">,
    force = false
  ): void {
    const now = Date.now();
    ...
    if (this.config.debug) {
      calendarLogger.debug("[ViewportStateManager] State pushed", {
        data: { historySize: this.history.length },
      });
    }
  }
  ...
  public async restoreViewport(
    ...
    if (!state.containerElement) {
      calendarLogger.warn(
        "[ViewportStateManager] Cannot restore: no container element"
      );
      return;
    }
    ...
    try {
      ...
      if (this.config.debug) {
        calendarLogger.debug(
          "[ViewportStateManager] History imported, states: " +
            this.history.length
        );
      }
    } catch (error) {
      calendarLogger.error(
        "[ViewportStateManager] Failed to import history",
        error
      );
    }
  }
