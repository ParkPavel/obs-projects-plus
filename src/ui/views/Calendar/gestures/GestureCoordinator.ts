...
export class GestureCoordinator {
  ...
  private attach(): void {
    ...
    this.element.addEventListener("touchmove", this.boundTouchMove, {
      passive: false,
    }); // Non-passive для preventDefault
  ...
  private handleTouchMove(e: TouchEvent): void {
    ...
    if (
      this.longPressTimer &&
      this.getDistanceFromStart() > this.config.tapMaxDistance
    ) {
    ...
    if (this.state === "active") {
      ...
      if (
        this.currentGestureType === "horizontal-swipe" &&
        this.touchState.zone === "center"
      ) {
        e.preventDefault();
      }
    }
  ...
  private transitionState(newState: GestureState): void {
    ...
    if (!this.isValidTransition(oldState, newState)) {
      calendarLogger.warn(
        `[GestureCoordinator] Invalid transition: ${oldState} → ${newState}`
      );
      return;
    }
  ...
  private createGestureEvent(
    ...
    if (!this.touchState) {
      throw new Error(
        "[GestureCoordinator] Cannot create event without touch state"
      );
    }
...
export function gestureCoordinator(
  ...
  let coordinator = new GestureCoordinator(
    node,
    params.handlers,
    params.config
  );
  ...
  return {
    update(newParams: {
      ...
      coordinator.destroy();
      coordinator = new GestureCoordinator(
        node,
        newParams.handlers,
        newParams.config
      );
