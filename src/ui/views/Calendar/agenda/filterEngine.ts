...
export function evaluateFilter(
  ...
  return matchesCondition(
    cond,
    prepareRecord(record, cond.field, cond.operator),
    baseDate,
    CAL_OPTS
  );
}
...
export function evaluateFilters(
  ...
  return (
    filters.length === 0 ||
    filters.every((f) => evaluateFilter(record, f, baseDate))
  );
}
...
function collectFieldOps(
  group: AgendaFilterGroup,
  out: Map<string, FilterOperator>
): void {
...
export function evaluateFilterGroup(
  ...
  let prepared = record;
  for (const [field, op] of fieldOps)
    prepared = prepareRecord(prepared, field, op);
...
export function filterRecordsForList(
  ...
  if (list.filterMode === "advanced" && list.filterFormula) {
    try {
      ...
      return records.filter((r) => {
        try {
          return evaluateFormula(ast, r, baseDate.toDate());
        } catch (e) {
          calendarLogger.error(
            `[FilterEngine] formula eval failed for ${r?.id}`,
            e
          );
          return false;
        }
      ...
      calendarLogger.error(
        `[FilterEngine] formula parse failed: ${list.filterFormula}`,
        e
      );
      return [];
    }
  ...
  if (list.filterMode === "visual" && list.filterGroup) {
    return records.filter((r) =>
      evaluateFilterGroup(r, list.filterGroup!, baseDate)
    );
  }
  ...
  if ("filters" in list && Array.isArray((list as any).filters)) {
    ...
    return records.filter((r) =>
      evaluateFilters(r, (list as any).filters, baseDate)
    );
  }
  return records;
}
