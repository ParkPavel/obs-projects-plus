...
describe("FormulaConstructor", () => {
  ...
  describe("syntax highlight overlay (#077)", () => {
    ...
    test("highlight on: underlay paints function and field token classes", () => {
      ...
      expect(underlay?.querySelector(".ppp-fc-tok-fn")?.textContent).toBe(
        "SUM"
      );
      expect(underlay?.querySelector(".ppp-fc-tok-field")?.textContent).toBe(
        "amount"
      );
    ...
    test("highlight on: string and number tokens get their classes", () => {
      ...
      const underlay = container.querySelector(".ppp-fc-highlight");
      expect(underlay?.querySelector(".ppp-fc-tok-string")?.textContent).toBe(
        '"hi"'
      );
      expect(underlay?.querySelector(".ppp-fc-tok-number")?.textContent).toBe(
        "42"
      );
    });
