<script lang="ts">
  ...
  function positionPop(el: HTMLElement, anchor: HTMLElement, maxHRem: number) {
    ...
    if (isTouch) {
      ...
      // making initial placement wrong. Show after first resize or fallback.
      el.style.opacity = '0';
  ...
  function makePopover(anchor: HTMLElement, items: { label: string; icon?: string; selected?: boolean; handler: () => void }[], searchable = false) {
    destroyPopover();
    const box = activeDocument.createElement('div');
    box.addClass('ppp-pop-box');
    box.setAttribute('data-settings-dropdown', '');
    const maxHRem = Math.min(items.length * 2.125 + (searchable ? 2.625 : 0) + 0.5, 17.5);
