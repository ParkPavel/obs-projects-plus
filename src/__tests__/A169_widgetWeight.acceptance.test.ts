...
if (!chrome) {
  console.warn(
    "A169 weight: no Chrome found (set CHROME_PATH) — the visual acceptance did NOT run."
  );
}
...
const primaryStyle = unscope(
  svelteStyle("ui/views/Dashboard/widgets/WidgetPrimaryAction.svelte")
);
const actionsStyle = unscope(
  svelteStyle("ui/views/Dashboard/widgets/WidgetHeaderActions.svelte")
);
const shellStyle = unscope(
...
const header = (widthPx: number) =>
  renderProbe({
    css: [
      ...
      actionsStyle,
      primaryStyle,
      `.ppp-widget-host{width:${widthPx}px}`,
    ],
    html: HEADER,
    measure: [
...
describeIfChrome(
  "A169 — the filter bar yields to a narrow widget, in its units",
  () => {
    const barStyle = unscope(
      svelteStyle(
        "ui/views/Dashboard/widgets/DatabaseCall/TableControlBar.svelte"
      )
    );
...
describe("A169 — structure, asserted in source because it is a structural claim", () => {
  it("the primary button is not a member of the hover-only cluster", () => {
    ...
    expect(
      source("ui/views/Dashboard/widgets/WidgetHeaderActions.svelte")
    ).not.toContain("primaryAction");
  });
...
describe("A169 — the filter bar half was already shipped, and this is why", () => {
  ...
  it("and the bar is mounted past that gate, unconditionally", () => {
    const block = source(
      "ui/views/Dashboard/widgets/DatabaseCall/DatabaseCallBlock.svelte"
    );
...
describe("A169 — the collapsed-widget wiring, SOURCE-LEVEL by admission", () => {
  ...
  it("and the signal is spent, so expanding later cannot replay it", () => {
    ...
    expect(
      source("ui/views/Dashboard/widgets/DatabaseCall/TableNewRow.svelte")
    ).toContain("let seenOpenSignal = 0;");
  });
});
