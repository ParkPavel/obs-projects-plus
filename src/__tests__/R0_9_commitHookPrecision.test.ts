  ...
  beforeAll(() => {
    ...
    const cases = [
      ...
      { id: "repo-option", repo: onMain, command: `git -C . commit -m "msg"` },
      {
        id: "steps-onto-main",
        repo: onFeature,
        command: `git checkout main && ${COMMIT} -m "m"`,
      },
      {
      ...
      {
        id: "branch-then-commit",
        repo: onMain,
        command: `git checkout -b feat/y && ${COMMIT} -m "m"`,
      },
      {
        id: "switch-then-commit",
        repo: onMain,
        command: `git switch -c feat/z; ${COMMIT} -m "m"`,
      },
      {
        id: "grep-mentions",
        repo: onMain,
        command: `grep -n "${COMMIT}" .github/workflows/ci.yml`,
      },
      {
        id: "echo-mentions",
        repo: onMain,
        command: `echo "never run ${COMMIT} on main"`,
      },
      {
        id: "words-in-message",
        repo: onMain,
        command: `git checkout -b feat/w && ${COMMIT} -m "on ${COMMIT}"`,
      },
    ];
    ...
    for (const row of JSON.parse(run.stdout) as Array<{
      id: string;
      code: number;
    }>) {
