...
const MOJIBAKE_LEAD: ReadonlySet<number> = new Set([
  ...
  0x0420,
  0x0421,
  0x0422, // capital VE, ER, ES, TE
  0x0432,
  0x0440,
  0x0441,
  0x0442, // small ve, er, es, te
]);
...
function hasMojibake(text: string): boolean {
  ...
  for (let i = 0; i < cps.length - 1; i++) {
    if (
      MOJIBAKE_LEAD.has(cps[i] as number) &&
      isMojibakeTail(cps[i + 1] as number)
    )
      return true;
  }
  return false;
...
whenRunnable("R0.10 - paired working stack integrity", () => {
  ...
  it("no config file carries a U+FFFD replacement character", () => {
    ...
    const offenders = [...textFiles(CLAUDE), ...textFiles(CODEX)]
      .filter((f) =>
        fs.readFileSync(f, "utf8").includes(String.fromCharCode(0xfffd))
      )
      .map((f) => path.relative(ROOT, f));
    expect(offenders).toEqual([]);
  });
  ...
  it("every config file that claims to be JSON parses as JSON", () => {
    const broken: string[] = [];
    for (const file of [...walk(CLAUDE), ...walk(CODEX)].filter((f) =>
      f.endsWith(".json")
    )) {
      try {
        ...
        broken.push(
          `${path.relative(ROOT, file)}: ${(error as Error).message}`
        );
      }
    }
  ...
  it("a hook present in both layers has the same implementation in both", () => {
    ...
    const diverged = fs
      ...
      .filter(
        (f) => f.endsWith(".ps1") && fs.existsSync(path.join(codexHooks, f))
      )
      .filter(
        (f) =>
  ...
  it("the same protective hooks are registered on both sides", () => {
    ...
    const scripts = (blob: string): string[] =>
      [
        ...new Set(
          [...blob.matchAll(/([a-z0-9-]+)\.ps1/gi)].map((m) => m[1] as string)
        ),
      ].sort();
