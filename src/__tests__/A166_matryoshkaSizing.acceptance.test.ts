...
describeIfChrome(
  "A166 step 1 — a container decides the size, and no container means today's size",
  () => {
    ...
    const result = () =>
      renderProbe({
        css: [
          TOKENS(),
          shell,
          ".narrow{width:200px}.wide{width:800px}.plain{width:800px}",
        ],
        html: [
...
describeIfChrome(
  "A166 step 2 — one user unit is one CSS pixel at every width",
  () => {
    ...
    it("the pie is capped at its height and centred, not stretched to the widget", () => {
      ...
      const pie = unscope(
        svelteStyle("ui/views/Dashboard/widgets/Chart/PieChart.svelte")
      );
      const r = renderProbe({
        css: [cell, pie],
        html: "<div class='c w960'><svg id='pie' class='ppp-chart-pie' viewBox='0 0 240 240' width='240' height='240'></svg></div>",
...
describeIfChrome(
  "A166 step 3 — a fixed minimum stops overflowing a narrow container",
  () => {
    ...
    it("the trailing filler takes the slack and never squeezes a fixed track", () => {
      ...
      expect(Number(r["w1"]!["boxWidth"])).toBeCloseTo(
        Number(r["n1"]!["boxWidth"]),
        1
      );
      expect(Number(r["w2"]!["boxWidth"])).toBeCloseTo(
        Number(r["n2"]!["boxWidth"]),
        1
      );
    });
  }
