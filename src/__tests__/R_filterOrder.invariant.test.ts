...
describe("#116 filter-order ADR — the stated contract", () => {
  ...
  it("names the surfaces that make up each axis", () => {
    expect(adr).toContain(
      "`view.filter`, filter-tabs, `block.subFilter`, per-tab filter"
    );
    expect(adr).toContain("`executeTransform` / advanced transform");
    expect(adr).toContain(
      "`composeEffectiveFilter` → `filterByLinkedSelection`"
    );
  });
  ...
  it("describes the order as implemented, not as aspirational", () => {
    ...
    expect(adr).not.toContain(
      "it does not describe the current runtime wiring"
    );
    expect(adr).not.toContain(
      "does not prove that existing runtime wiring implements this order"
    );
  });
...
describe("#128 filter-order — the wiring the ADR describes", () => {
  const host = read(
    resolve(SRC, "ui/views/Dashboard/widgets/WidgetHost.svelte")
  );
  ...
  const context = read(
    resolve(SRC, "ui/views/Dashboard/widgets/renderContext.ts")
  );
  ...
  it("keeps the selection (axis B) downstream of the transform in the block", () => {
    ...
    const selectionAt = block.indexOf(
      "filterByLinkedSelection(subFiltered.records"
    );
