...
export default class ProjectsPlusPlugin extends Plugin {
  ...
  async onload(): Promise<void> {
        ...
        if (file instanceof TFolder) {
          ...
          menu.addItem((item) => {
            item
              ...
              .setIcon("layout-list")
              .onClick(() => {
                emitCommand("open-visualizer-for-file", {
                  filePath: file.path,
                });
              });
          });
    ...
    this.addCommand({
      ...
      callback: () => {
        const existing = get(settings).projects.find(
          (p) => p.name === "Демо-проект"
        );
        if (existing) {
          new Notice(
    ...
    this.unsubscribeSettings = settings.subscribe((value) => {
      ...
      void this.saveData(value).catch((err) =>
  ...
  async maybeReplacePropertiesPane(): Promise<void> {
    ...
    if (
      this.app.workspace.getLeavesOfType(VIEW_TYPE_VISUALIZER_PANE).length === 0
    ) {
  ...
  async loadSettings(): Promise<void> {
    ...
    if (either.isLeft(result)) {
      ...
      try {
        ...
        console.error(
          "[Projects+] Failed to persist broken-payload backup:",
          saveErr
        );
      }
      new Notice(
