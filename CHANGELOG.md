...
> менялась, а поверх неё легло **310 коммитов** — всё, что описано ниже. Версия перестала
> отслеживать реальность на три с половиной месяца, и `3.6.0-alpha` эту задолженность закрывает.
>
> Почему minor, а не patch: в выпуск входят две вехи (M-RELATION-FIRST, M-FILTER-CONSOLIDATION)
> и удаление модели sub-base, которая **поставлялась** в 3.5.0-alpha. Удаление отгруженной
> функции — не patch.
>
> Что при этом НЕ менялось: `ci.yml` по-прежнему публикует на каждый пуш в `main` служебный
> пререлиз с тегом `0.0.0-<номер прогона>` и манифестом `<база>-beta.<номер прогона>`. Этот поток
> остаётся ночными сборками; осмысленные выпуски идут отдельным тегом через `release.yml`.
>
> Раздел «[Unreleased — V5 internal] (3.4.2)» ниже написан ДО того, как версия ушла на
> 3.5.0-alpha / 3.5.1-alpha, и описывает более раннюю работу. Его подпись неточна; содержание
> оставлено как есть, потому что построчно не перепроверялось.

### Added

- **M-RELATION-FIRST — связи как объявленное свойство данных** (#110–#115)
  - `src/lib/relations/relationContract.ts` — канонический контракт: разрешение WikiLink со
    статусами `resolved` / `unmatched` / `ambiguous`, обратная связь, валидация и миграция
    легаси-`linkedSelection` (#111).
  - Мастер настройки связи и правки записи, все 4 точки входа, выбор `displayField` (#112).
  - Поверхность связанных записей со счётчиком, цепочка события `setupRelation`,
    relation-блок в подсказках дашборда (#113).
  - Связь-осознанные взаимодействия дашборда: единый `composeEffectiveFilter`, relation-only
    picker, трёхсоставная метка фильтра (#114).
  - Приёмочный сценарий «Клиенты → Сессии» и раздел 8 ручного пайплайна (#115).
- **M-FILTER-CONSOLIDATION — один движок фильтрации**
  - `FILTER_ORDER_ADR.md` и инвариант-тест порядка `enrich → A (scope) → C (advanced) → B
    (reactive) → sort → render` (#116, #128).
  - Расщепление конвейера трансформаций на advanced-режим плюс миграция сохранённых
    дашбордов (#118).
  - `filter-tabs` переведены на канонический движок (#117).
- **Инфраструктура проверки**
  - Ратчет **R0.8** — целостность поставляемого `styles.css` (после того, как рукописная
    половина ушла в пререлизы незамеченной).
  - Ратчеты R0.5–R0.7: целостность текста, потолки LOC, дрейф конфигов.

### Changed

- **⚠ Порядок фильтрации дашборда.** Блочный фильтр (`subFilter`) теперь сужает данные **до**
  запуска advanced-конвейера, а не после. Дашборд, где блочный фильтр сочетается с шагом
  `filter` в конвейере, может показать другой результат. Данные на диске не меняются, но
  видимое — может (#118; впервые объявлено в разделе 3.4.2 ниже, повторено здесь, потому что
  до пользователя оно доедет именно с этим выпуском).
- **Демо-проект больше не мигрирует сам себя при первом открытии.** Генератор отдавал ведущий
  шаг `filter` в `widget.transform` — ровно ту форму, которую продукт переносит в
  `config.subFilter`. Свежесозданное демо переписывало `data.json` и создавало файл
  восстановления на первом же экране. Теперь условия сразу лежат в `config.subFilter` (#164).
- **Граница внешнего источника.** Блок с внешним источником больше не пишет в родительский
  проект (#139), не рендерит записи чужого проекта (#136), настраивается против собственного
  источника (#137) и получает обогащение обратными ссылками (#138). В Gallery внешний источник
  доступен только для чтения (#142).
- **Исходы записи.** Граница `ViewApi` дожидается записи и возвращает результат (#144);
  вызывающие больше не считают неудачную запись успешной (#161); пакетная запись Board
  компенсируется при отказе (#163); вид обновляет rollup и relation при смене целевого
  проекта (#162).
- **Честность интерфейса.** Проактивные подсказки названы MVP, а не пониманием данных (#155);
  первый запуск объясняет, что делает (#156); модалки создания поля и проекта показывают
  последствие записи до её совершения.

### Fixed

- Rollup, настроенный через UI, не вычислялся никогда (#141).
- Обратная связь обещалась в интерфейсе и не соответствовала ничему в коде — теперь выводится,
  а не пишется; путь записи сохранён в коде, решение обратимо (#143).
- Мастер связи рапортовал об успехе до записи, терял выбранный `displayField`, а его
  предпросмотр считал совпадения по пустому списку записей (#150).
- `linkedSelection` не показывал, какая связь применена, и маскировал поломку (#149).
- Приёмник Stats сравнивал Relation-поля не каноническим ключом (#153).
- `promoteFilterTabToGlobal` терял записи на не-строковых полях (#123); `promoteLocalToGlobal`
  разрушал группы, конъюнкцию `or` и отключённые условия (#125).
- Политика ReDoS была продублирована трижды; третья точка (`unpivot`) пропускала ограничение
  длины шаблона (#126).
- Наборы ключей i18n разошлись по локалям — `en` вернул 42 ключа, бывших в `ru` (#130);
  `uk` и `zh-CN` остаются открытыми (#140, нужен переводчик, машинный перевод отклонён).
- Поставляемый `styles.css` потерял рукописную половину — 685 строк вёрстки — из-за сборки,
  прочитавшей отсутствующий файл как пустой. Галерея, колонки Board и строки поповеров
  рендерились без стилей. Восстановлено; сборка теперь отказывается писать стиль из одних
  токенов вместо того, чтобы делать это молча.

### Removed

- **Модель sub-base удалена целиком** (#160): `subBase.ts`, `subBasePartition.ts`,
  `crossSubBase.ts` и их сюиты, команда `add-sub-base` и её событие шины. Ключи
  `DataTableConfig.subBases` намеренно оставлены в типе: 3.5.0-alpha поставляла виджет
  `SubBaseCanvas`, который их писал, и удаление полей сделало бы существующие данные
  немоделируемыми. Они переносятся, но не интерпретируются.
- `src/archive/dashboard-v1` — мёртвый код V1 (#119).
- Отставленные `WidgetType` и осиротевшие панели конфигурации (#120).
- 12 файлов без единого импортёра, ~1940 LOC (#129).

---

## [3.5.1-alpha] - 2026-05-14

Проставлена коммитом `9034cea` («feat(v7): Dashboard V2 alpha continuation»). Пять коммитов
между 3.5.0-alpha и этой отметкой. **Всё, что описано в разделе [Unreleased] выше, легло поверх
этой версии и ни одной новой отметки не получило.**

## [3.5.0-alpha] - 2026-05-10

Проставлена коммитом `2af8a50` («feat(v6): Dashboard V2 alpha»). Именно эта версия поставила
виджет `SubBaseCanvas`, писавший ключи `subBases` — на неё ссылается обоснование в
`src/ui/views/Dashboard/types.ts`, почему эти ключи нельзя удалить.

Отсутствует в `versions.json` (#175).

## [3.4.2] - 2026-05-05

Проставлена коммитом `83a57d7`. Подробные заметки этого периода — в разделе
«[Unreleased — V5 internal] (3.4.2)» ниже. В `versions.json` присутствует как `3.4.2-alpha.1`.
